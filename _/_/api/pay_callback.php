...
if (strtolower($_SERVER['REQUEST_METHOD']) == 'get') {
    ...
    if (!_checkSign($sign_key, $_GET)) {
        ...
        $error_msg = $_GET['msg'];
        if (empty($error_code) || $error_code == 2) {
            $database_order = D('Order');
            $product_model = M('Product');
            $product_sku = M('Product_sku');
            $nowOrder = M('Order')->findSimple(trim($_GET['order_no']));
            $condition_order['order_no'] = $nowOrder['order_no'];
            if($nowOrder && $nowOrder['status'] == 1){
                $data_order['third_id']       = $_GET['third_id'];
                $data_order['payment_method'] = $_GET['payment_method'];
                $data_order['pay_money']      = $_GET['pay_money'];
                ...
                if(D('Order')->where($condition_order)->data($data_order)->save()){
                    ...
                    $data_store['income']         = $nowStore['income'] + $_GET['pay_money'];
                    $data_store['unbalance']      = $nowStore['unbalance'] + $_GET['pay_money'];
                ...
                //file_put_contents('log.txt', $_GET['order_no']);
                $error_code = 1005;
                $error_msg = '订单不存在';
            }
        }
    }
    header('Location:' . option('config.wap_site_url') . '/payfaildcallback.php?id=' . $_GET['order_no'] . '&pay=failed&error_code=' . $error_code . '&error_msg=' . $error_msg);
    exit;
}
