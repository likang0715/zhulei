...
function shows($store_id,$couponid) {
...
//ajax 获取数据并分页
function get_usercoupon_list($wap_user,$store_id,$couponid) {
	$store_id = isset($store_id) ? $store_id : pigcms_tips('您输入的网址有误','none');
...
switch($action){
	case 'get_usercoupon_list':
		get_usercoupon_list($wap_user,$store_id,$couponid);
		break;
