...
//当前页面的地址
if (!empty($_GET['store_id'])) {
	$now_url = $config['wap_site_url'] . '/pagecat.php?id=' . $nowCategory['cat_id'] . '&store_id=' . $store_id;
} else {
...
if($nowCategory['has_custom']){
	$homeCustomField = M('Custom_field')->getParseFields($nowCategory['store_id'],'page_cat',$nowCategory['cat_id'], $store_id, $now_store['drp_level'], $now_store['drp_diy_store']);
}
...
if($now_store['open_ad'] && !empty($now_store['use_ad_pages'])){
	...
	if(in_array('1',$useAdPagesArr)){
		$pageAdFieldArr = M('Custom_field')->getParseFields($nowCategory['store_id'],'common_ad',$nowCategory['store_id'], $store_id, $now_store['drp_level'], $now_store['drp_diy_store']);
...
if($now_store['open_nav'] && !empty($now_store['use_nav_pages'])){
	...
	if(in_array('2',$useNavPagesArr)){
		$storeNav = M('Store_nav')->getParseNav($nowCategory['store_id'], $store_id, $now_store['drp_diy_store']);
	}
}
