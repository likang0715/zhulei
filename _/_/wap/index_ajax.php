...
switch($action){
	case 'get_category':
		$cat_lists = M('Product_category')->getAllCategory();
		foreach($cat_lists as $key=>$value){
	...
	case 'get_product_list':
        ...
        if(isset($WebUserInfo['city_name']) &&isset($WebUserInfo['area_name'])){
            $where = "city='".$WebUserInfo['city_code']."' and county='".$WebUserInfo['area_code']."'";
        }elseif($WebUserInfo['city_name']) {
            $where = "city=".$WebUserInfo['city_code'];
        }
        if(isset($WebUserInfo['city_name']) || isset($WebUserInfo['area_name'])){
            $store = D('Store_area_record')->where($where)->field('store_id')->select();
            $store_arr = array();
            foreach($store as $k=>$v){
                array_push($store_arr,$v['store_id']);
            }
            $condition['store_id'] = array('in',$store_arr);
        }
		...
		$json_return['list'] = D('Product')->field('`product_id`,`store_id`,`name`,`image`,`price`,`original_price`,`sales`,`give_points`,`open_return_point`,`return_point`')->where($condition)->order($sort_by)->limit((($page-1)*10).',10')->select();
		...
		if($store_ids){
			...
			$store_list = D('Store')->field('store_id,status')->where($where_store)->select();
			if($store_list){
				$open_store_ids = array();
				foreach($store_list as $_store_item){
					if(!in_array($_store_item['store_id'],$open_store_ids)){
						$open_store_ids[] = $_store_item['store_id'];
					}
				}
				if($open_store_ids){
					foreach($json_return['list'] as $_key => $_value){
						if(!in_array($_value['store_id'],$open_store_ids)){
							unset($json_return['list'][$_key]);
							$json_return['count']--;
						}
					}	
				}
	...
	case 'get_shop_list':
		...
		$page = $_POST['page'];
		$prop = $_POST['prop'];
		$sort_by = '`store_id` DESC';
	
		/*$condition['status'] = '1';
		$condition['drp_supplier_id'] = '0';
		$condition['public_display'] = '1';*/
		...
		$where_sql[] = "`s`.`status` = 1 AND `s`.`public_display` = '1' AND `s`.`drp_level`=0 and s.is_point_mall=0";

		if(!empty($keyword)) {
			$where_sql[] = " `s`.`name` LIKE '%" . $keyword . "%'";
		}


		if(!empty($tag_id)) {
			$where_sql[] = " `s`.`tag_id` = ".$tag_id;
		}

		if($page == 1){
			
			//$json_return['count'] = D('Store')->where($condition)->count('store_id');
			$json_return['count'] = M('Store')->getStoreByRoundDistanceCount_new($where_sql,'',0);

		}
		
		
		//$json_return['list'] = D('Store')->where($condition)->order($sort_by)->limit((($page-1)*10).',10')->select();

		$json_return['list'] = M('Store')->getStoreByRoundDistance_new($where_sql,10,($page-1)*10,'s.store_id ','desc','',0);



		foreach($json_return['list'] as $k=>&$v) {
			$v['url'] = option('config.wap_site_url').'/home.php?id='.$v['store_id'];
			if(empty($v['logo'])) {
				$v['logo'] = getAttachmentUrl('images/default_shop_2.jpg', false);
			} else {
				$v['logo'] = getAttachmentUrl($v['logo']);
			}
		}
	...
	case 'nearshops':
		...
		foreach($store_list as &$value){
			...
			$value['product_list'] = $database_product->field('`product_id`,`name`,`image`,`price`')->where($condition_product)->order('`sort` DESC, `product_id` DESC')->limit(3)->select();
	...
	case 'butt_nearshops':
		...
		$database_store_contact = D('Store_contact');
		$store_list = D('')->table(array('Store_contact'=>'sc','Store'=>'s','User'=>'u'))->field("`s`.`store_id`,`s`.`name`,`s`.`intro`, ROUND(6378.138 * 2 * ASIN(SQRT(POW(SIN(({$lat}*PI()/180-`sc`.`lat`*PI()/180)/2),2)+COS({$lat}*PI()/180)*COS(`sc`.`lat`*PI()/180)*POW(SIN(({$long}*PI()/180-`sc`.`long`*PI()/180)/2),2)))*1000) AS juli")->where("`sc`.`store_id`=`s`.`store_id` AND `s`.`status`='1' and `s`.`public_display`='1' AND s.drp_supplier_id = '0' AND `u`.`uid`=`s`.`uid` AND `u`.`source_site_url`='$domain'")->order("`juli` ASC")->limit(20)->select();
		foreach($store_list as $key=>$value){
			if($value['juli'] > 20000){
				unset($store_list[$key]);
			}
	...
	case 'nearstore':
		...
		$database_store_contact = D('Store_contact');
		$store_list = D('')->table(array('Store_contact'=>'sc', 'Store'=>'s'))->field("`s`.`logo`,`s`.`store_id`,`s`.`name`,`s`.`price`,`sc`.`address`,`s`.`intro`, ROUND(6378.138 * 2 * ASIN(SQRT(POW(SIN(({$lat}*PI()/180-`sc`.`lat`*PI()/180)/2),2)+COS({$lat}*PI()/180)*COS(`sc`.`lat`*PI()/180)*POW(SIN(({$long}*PI()/180-`sc`.`long`*PI()/180)/2),2)))*1000) AS juli")->where("`sc`.`store_id`=`s`.`store_id` AND s.drp_supplier_id = '0' AND `s`.`status`='1' and `s`.`public_display`= '1' ")->order("`juli` ASC")->limit($size)->select();
		foreach($store_list as $key=>$value){
		$store['tuan'] = '';
	$tuan = D('Tuan')->field('`name`')->where("store_id = ".$value['store_id'])->order('`id` DESC')->limit(1)->select();
	 foreach($tuan as $k=>$r){
	 $store_list[$key]['tuan'] = $r['name'];
	 }
	$store['hui'] = '';
     $hui= D('Reward')->field('`name`')->where("store_id = ".$value['store_id'])->order('`id` DESC')->limit(1)->select();
	  foreach($hui as $k=>$r){
	 $store_list[$key]['hui'] = $r['name'];
	 }
	...
	case 'nearactive':
		...
		$artivce_list = D('')->table("Activity_recommend as ar")
							...
							->where("`s`.`status`='1' AND `s`.`public_display`=1 and s.drp_supplier_id = '0' and is_rec=1")
							->group ( '`ar`.`id`' )
							->order ( "`juli` asc" )
							->limit($size)
							->select();		
	...
	case 'neargoods':
		...
		$goods_list = D('')->table("Product as p")
							...
							->where("`s`.`status`='1' and `p`.`status`='1' AND `s`.`public_display`='1' AND s.drp_supplier_id = '0' AND p.supplier_id = 0 AND sc.`long`!=''")
							->group ( 'p.store_id' )
							->order ( "`juli` asc, `p`.`is_recommend` desc"  )
							->limit($size)
							->select();	
	...
	case 'nearchahui':
		...
		$database_store_contact = D('Chahui');
		$store_list = D('')->table(array('Store'=>'sc', 'Chahui'=>'s'))->field("`s`.`images`,`s`.`pigcms_id`,`s`.`store_id`,`sc`.`name` as  store_name,`s`.`name`,`s`.`sttime`,`s`.`address`,`s`.`price`,`s`.`description`, ROUND(6378.138 * 2 * ASIN(SQRT(POW(SIN(({$lat}*PI()/180-`s`.`lat`*PI()/180)/2),2)+COS({$lat}*PI()/180)*COS(`s`.`lat`*PI()/180)*POW(SIN(({$long}*PI()/180-`s`.`long`*PI()/180)/2),2)))*1000) AS juli")->where("`sc`.`store_id`=`s`.`store_id` AND sc.drp_supplier_id = '0' AND `sc`.`status`='1' ")->order("`juli` ASC")->limit($size)->select();
		foreach($store_list as $key=>$value){
		if($value['price']>0){
		$store_list[$key]['price']=$value['price'];
		}else{
		$store_list[$key]['price']='免费';
		}
		$store_list[$key]['sttime']=date('m/d H:i',$value['sttime']);
			if($value['juli'] > 200000){
				$store_list[$key]['url'] = $config['wap_site_url'] . '/chahui_show.php?id=' . $value['pigcms_id'];
				if(empty($value['images'])) {
					$store_list[$key]['images'] = getAttachmentUrl('images/default_shop_2.jpg', false);
				} else {
					$store_list[$key]['images'] = getAttachmentUrl($value['images']);
				}				
				//unset($store_list[$key]);
			} else {
				$store_list[$key]['url'] = $config['wap_site_url'] . '/chahui_show.php?id=' . $value['pigcms_id'];
	...
	case 'nearchaguan':
		...
		$database_store_contact = D('Chahui');
		$store_list = D('')->table(array('Store'=>'sc', 'Store_physical'=>'s'))->field("`s`.`images`,`s`.`store_id`,`s`.`pigcms_id`,`sc`.`name` as  store_name,`s`.`name`,`s`.`description`, ROUND(6378.138 * 2 * ASIN(SQRT(POW(SIN(({$lat}*PI()/180-`s`.`lat`*PI()/180)/2),2)+COS({$lat}*PI()/180)*COS(`s`.`lat`*PI()/180)*POW(SIN(({$long}*PI()/180-`s`.`long`*PI()/180)/2),2)))*1000) AS juli")->where("`sc`.`store_id`=`s`.`store_id` AND sc.drp_supplier_id = '0' AND `sc`.`status`='1' ")->order("`juli` ASC")->limit($size)->select();
	
		foreach($store_list as $key=>$value){
			if($value['juli'] > 200000){
				$store_list[$key]['url'] = $config['wap_site_url'] . '/physical_show.php?id=' . $value['pigcms_id'];
				if(empty($value['images'])) {
					$store_list[$key]['images'] = getAttachmentUrl('images/default_shop_2.jpg', false);
				} else {
					$store_list[$key]['images'] = getAttachmentUrl($value['images']);
