...
$qrcode_id = $_GET['login_qrcode_id'];
if(empty($qrcode_id)){
	pigcms_tips('登录失败！请重新扫码~。'.$qrcode_id,$config['wap_site_url']);
}
...
if(!empty($wap_user)){
	if(change_login_qrcode($qrcode_id,$wap_user)){
		...
		pigcms_tips('登录失败！请重新扫码:code:'.$qrcode_id,$config['wap_site_url']);
	}
}else{
	redirect('./login.php?referer='.urlencode('./weixin_bind.php?login_qrcode_id='.$qrcode_id));
}
...
function change_login_qrcode($qrcode_id,$wap_user){
	...
	if($database_login_qrcode->where($condition_login_qrcode)->data($data_login_qrcode)->save()){
		...
		//echo $database_login_qrcode->last_sql;exit;
		return false;
	}
}
?>