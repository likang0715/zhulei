        ...
        <div class="widget-list-filter">
            ...
            <div class="trade-list-nav">
				<ul>
                    ...
                    <li data-id="1" class="status-1"><a href="#trade/1">已付款未发货</a></li>
                    <li data-id="2" class="status-2"><a href="#trade/2">已付款未收货</a></li>
                    <li data-id="3" class="status-3"><a href="#trade/3">已收货未过退货期</a></li>
                    <li data-id="4" class="status-4"><a href="#trade/4">未点击交易完成</a></li>
                </ul>
            </div>
        </div>
    </div>
        ...
        <table class="ui-table ui-table-trade" style="padding: 0px;">
            ...
            <tr class="widget-list-header">
                ...
                <th class="cell-15 text-right">
                    <div class="row-money">流水(元)</div>
                </th>
                <th class="cell-15 text-right">
                    <div class="row-money">收益(元)</div>
                </th>
            ...
            <tr>
                ...
                <td class="text-right">
                    <div class="row-money"><?php if ($record['income'] > 0) { ?><?php echo $record['real_profit']; ?><?php } ?></div>
                </td>
                <td><?php if($record['order_status']==2){ echo '已付款未发货';}else if($record['order_status']==3){ echo '已付款未收货';}else if(($record['order_status']==7)&&((time()<=$record['delivery_time']+option('config.order_return_date')*24*3600) || (time()<=$record['sent_time']+option('config.order_complete_date')*24*3600))){ echo '已收货未过退货期';}else if(($record['order_status']==7)&&((time()>=$record['delivery_time']+option('config.order_return_date')*24*3600) && (time()>=$record['sent_time']+option('config.order_complete_date')*24*3600))){echo '未点击交易完成';}else if($record['order_status']==4){ echo '已完成';}else if($record['order_status']==6){ echo '退款中';}else if($record['order_status']==1){ echo '未支付';}else if($record['order_status']==5){ echo '已取消'; } ?></td>
                <td>
