...
<head>
    <meta charset="utf-8"/>
    <title>添加店铺管理员 - <?php echo $store_session['name']; ?> | <?php if (empty($_SESSION['sync_store'])) { ?><?php echo $config['site_name'];?><?php } else { ?>微店系统<?php } ?></title>
    ...
    <script type="text/javascript">
        var load_url="<?php dourl('substore:load');?>", 
        store_name_check_url = "<?php dourl('store_name_check'); ?>",
        store_setting_url="<?php dourl('store'); ?>",
        store_contact_url="<?php dourl('contact'); ?>",
        add_admin="<?php dourl('substore:add_admin'); ?>",
        store_admin_edit="<?php dourl('substore:store_edit'); ?>",
        store_admin_del="<?php dourl('substore:store_admin_del'); ?>",
