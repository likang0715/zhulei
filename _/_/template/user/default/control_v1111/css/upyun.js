...
function upyunPicUploadCallback(domid,width,height,token,callback){
	art.dialog.data('width', width);
	art.dialog.data('height', height);
	art.dialog.data('domid', domid);
	art.dialog.data('callback', callback);
	art.dialog.data('lastpic', $('#'+domid).val());
	art.dialog.open('?g=User&m=Upyun&a=upload&token='+token+'&width='+width,{lock:true,title:'上传图片',width:600,height:400,yesText:'关闭',background: '#000',opacity: 0.45,close:callback});
}
...
function chooseFile(domid,type,callback){
	art.dialog.data('domid', domid);
	art.dialog.data('width', '600');
	art.dialog.data('callback', callback);
	art.dialog.open('?g=User&m=Attachment&a=index&type='+type,{lock:true,title:'选择文件',width:600,height:400,yesText:'关闭',background: '#000',opacity: 0.45,close:callback});
}
function chooseFileTp(domid,type,callback){
	art.dialog.data('domid', domid);
	art.dialog.data('width', '80%');
	art.dialog.data('callback', callback);
	art.dialog.open('?g=User&m=Attachment&a=my&type='+type,{lock:true,title:'选择文件',width:'70%',height:'80%',yesText:'关闭',background: '#000',opacity: 0.45,close: callback});
}
