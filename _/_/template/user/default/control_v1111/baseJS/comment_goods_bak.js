$(function(){//alert(page_content)
    ...
    $('.js-batch-unload').live('click', function(e){
        ...
        button_box($(this), e, 'right', 'confirm', '确认下架？', function(){
            ...
            $.post(soldout_url, {'comment_id': comment_id}, function(data){
                $('.notifications').html('');
                if (!data.err_code) {
    ...
    $('.js-batch-load').live('click', function(e){
        ...
        button_box($(this), e, 'right', 'confirm', '确认上架？', function(){
            ...
            $.post(putaway_url, {'comment_id': comment_id}, function(data){
                $('.notifications').html('');
                if (!data.err_code) {
                    $('.notifications').html('<div class="alert in fade alert-success">' + data.err_msg + '</div>');
                } else {
    ...
    $('.js-batch-delete').live('click', function(e){
        if ($('.js-check-toggle:checked').length == 0) {
            $('.notifications').html('');
            $('.notifications').html('<div class="alert in fade alert-error"><a href="javascript:;" class="close pull-right">×</a>请选择需要删除的评价</div>');
            $('body').append('<div class="notify-backdrop fade in"></div>');
            return false;
        }
        var comment_id = [];
        ...
        button_box($(this), e, 'right', 'confirm', '确认删除？', function(){
            $.post(del_comment_url, {'comment_id': comment_id}, function(data) {
                close_button_box();
                t = setTimeout('msg_hide()', 3000);
                if (data.err_code == 0) {
                    $('.notifications').html('<div class="alert in fade alert-success">' + data.err_msg + '</div>');
    ...
    $('.js-batch-discount').live('click', function(e){
        ...
        button_box($(this), e, 'top', 'radio', '<label class="radio"><input type="radio" name="discount" value="1" checked="">参与</label><label class="radio"><input type="radio" name="discount" value="0">不参与</label>', function(){
            ...
            $.post(allow_discount_url, {'comment_id': comment_id, 'discount': discount}, function(data){
                $('.notifications').html('');
                if (!data.err_code) {
                    if (discount == 1) {
    ...
    $('.js-promotion-btn').live('click', function(){
        ...
        qrcode_url = './source/qrcode.php?type=good&id=' + comment_id;
        var is_fx = $(this).data('fx');
        $('body').children('.widget-promotion').remove();
        $.post(get_qrcode_activity_url, {'comment_id': comment_id}, function(data){
            if (!is_fx) {
            ...
            if (data) {
                ...
                for (i in data) {
                    ...
                    qrcode_arr[data[i]['pigcms_id']] = '<img class="loading" src="./source/qrcode.php?type=good&id=' + comment_id + '&activity=' + data[i]['pigcms_id'] + '" /><p>' + desc + '</p><div class="clearfix qrcode-links"><a class="pull-left" href="./source/qrcode.php?type=good&id=' + comment_id + '&activity=' + data[i]['pigcms_id'] + '" download="true" target="_blank">下载二维码</a><a class="pull-right js-edit" data-id="' + data[i]['pigcms_id'] + '" href="javascript:;">修改优惠</a></div>';
                }
            }
    ...
    $('.popover-category2 > .popover-footer > .js-btn-confirm').live('click', function(){
        ...
        $('.js-check-toggle:checked').each(function(i){
            ...
            if (group_id != '' && comment_id > 0) {
                data[i] = {'comment_id': comment_id, 'group_id': group_id};
            }
        })
    ...
    $('.js-qrcode-create-content').find('.js-submit-btn').live('click', function(){
        ...
        $.post(save_qrcode_activity_url, {'comment_id': comment_id, 'buy_type': buy_type, 'type': type, 'discount': discount, 'price': amount, 'activity_id': activity_id}, function(data){
            var data = $.parseJSON(data);
            if (data.code) {
                var li = '';
                if(data.data.type == 0) {
                ...
                if (data.type == 'add') {
                    ...
                    qrcode_arr[data.data.pigcms_id] = '<img class="loading" src="./source/qrcode.php?type=good&id=' + comment_id + '&activity=' + data.data.pigcms_id + '" /><p>' + desc + '</p><div class="clearfix qrcode-links"><a class="pull-left" href="./source/qrcode.php?type=good&id=' + comment_id + '&activity=' + data.data.pigcms_id + '" download="true" target="_blank">下载二维码</a><a class="pull-right js-edit" data-id="' + data.data.pigcms_id + '" href="javascript:;">修改优惠</a></div>';
    ...
    //显示复制商品按钮
    /*状元分销www.bestfenxiao.com-QQ：75943938$('.js-list-body-region > tr').live('hover', function(){
        if (event.type == 'mouseover') {
    ...
    $('.js-input-num').live('blur', function(){
        ...
        if (flag) {
            ...
            if (sort != '') {
                $.post(sort_url, {'id': id, 'status': sort}, function(data){})
            }
