...
var p = 1;
$(function(){
    load_page('.app__content', load_url, {page:'my_seller_detail_content','store_id': store_id}, '');

    $('.js-search-btn').live('click', function(){
        var keyword = $.trim($('.js-search').val());
        var approve = $.trim($('.js-search-drp-approve').val());
        var level = $("#level").val();
        load_page('.app__content', load_url, {page:'my_seller_detail_content', 'keyword': keyword, 'approve': approve, 'level':level}, '', function(){
    ...
    $('.ui-nav > ul > li').live('click', function() {
        ...
        var keyword = $.trim($('.js-search').val());
        var approve = $.trim($('.js-search-drp-approve').val());
        load_page('.app__content', load_url, {page:'my_seller_detail_content', 'keyword': keyword, 'approve': approve, 'level':level ,  'store_id': store_id}, '', function(){
            $('.js-search').val(keyword);
            $('.js-search-drp-approve').find("option[value='" + approve + "']").attr("selected",true);
            $('.js-search').focus();
        });
        ...
        button_box($(this), e, 'left', 'confirm', '确认审核？', function(){
            $.post(drp_approve_url, {'seller_id': seller_id}, function(data) {
                ...
                if (data.err_code == 0) {
                    ...
                    load_page('.app__content', load_url, {page:'my_seller_detail_content', 'p': p, 'keyword': keyword, 'approve': approve}, '');
                } else {
    ...
    $('.pagenavi > a').live('click', function(e){
        ...
        level = $('#fx-level').val();
        store_id = $('#fx-store-id').val();

        keyword = $.trim($('.js-search').val());
        $('.app__content').load(load_url, {page: 'my_seller_detail_content', 'p': p, 'keyword': keyword,'level':level,'store_id':store_id}, function(){
            if (keyword != '') {
                $('.js-search').val(keyword);
            }
        });
