...
$(function(){
    ...
    function location_page(mark,dom){
        ...
        switch(mark_arr[0]){
            case '#list':
                ...
                load_page('.app__content', load_url,{page:'warn_stock_list'}, '');
                break;
            case '#warn':
                $('.js-app-nav.warn').addClass('active').siblings('.js-app-nav').removeClass('active');
                load_page('.app__content', load_url,{page:'quantity_warn'}, '');
                break;
            default:
                $(".dianpu_left .list").addClass("active").siblings().removeClass("active");
                $('.js-app-nav.list').addClass('active').siblings('.js-app-nav').removeClass('active');
    ...
    //分页
    $('.js-page-list > a').live('click', function(e){   //TODO + physical
        ...
        //保留门店库存
        var physical_id = 0;
        var physical_id = $("select[name=physical] option:selected").val();
        $('.app__content').load(load_url, {page: page_content, 'p': p, 'keyword': keyword, 'group_id': group_id, 'orderbyfield': orderbyfield, 'orderbymethod': orderbymethod, 'physical_id': physical_id}, function(){
            if (group != '') {
            ...
            $("select[name='physical']").find("option[value='" + physical_id + "']").attr('selected', true);
            set_physical();
        });
    })

    //排序
    $('.orderby').live('click', function(e){        //TODO + physical
    ...
    //回车提交搜索
    $(window).keydown(function(event){      //TODO + physical
    ...
    //商品分组筛选
    $('.active-result').live('click', function(){       //TODO + physical
        var keyword = '';
    ...
    $('.js-show-assign').live('click', function(){
        ...
        $.post(assign_json_url, {'product_id': product_id, 'sku_id' : sku_id}, function(data){
            ...
            for (i in data.store_physical) {
                ...
                html += '                   <td class="tb-num">';
                    html += data.store_physical[i]['phone1'];
                    if (data.store_physical[i]['phone1'].length > 0) {
                        html += '-';
                    }
                    html += data.store_physical[i]['phone2'];
                html += '                   </td>';
    ...
    $('select[name=physical]').live("change", function(){   //TODO + physical
        ...
        $('.app__content').load(load_url, {page: page_content, 'keyword': keyword, 'group_id': group_id, 'orderbyfield': orderbyfield, 'orderbymethod': orderbymethod, 'physical_id': physical_id}, function(){
            if (group != '') {
                $('.chosen-single > span').text(group);
            }
            if (keyword != '') {
    ...
    // 门店库存报警数量
    $('.js-warn-submit').live('click', function(){
        layer.closeAll();
        var formObj = {};
            ...
            switch(i){
                case 'warn_quantity':
                    if(value!= '' && !/^\d+$/.test(value)){
                        layer_tips(1,'库存为数字');
                        $('input[name="warn_sq_quantity"]').focus();
                        return false;
                    }
                    break;
            }
        }
        $.post(warn_status_url,formObj,function(result){
    ...
    $('.js-physical-quantity-edit').live('click',function(event){
        ...
        var self = $(this);
        var spq_id = self.attr("data-id");

        var content_dom = self.closest('tr').find('.data-quantity');

        button_box($(this),event,'left','input',parseInt(content_dom.text()),function(){

            var new_content = $('.js-rename-placeholder').val();

            if (!/^\d+?$/.test(new_content)) {
                layer_tips(1, '修改失败，请输入正整数');
                close_button_box();
                return;
            }

            if(new_content <= 0){
                layer_tips(1,'修改失败，请勿设置负值');
                close_button_box();
                return;
            }else if(parseInt(content_dom.text()) != new_content){
                $.post(physical_quantity_edit_admin,{pigcms_id:spq_id, quantity:new_content},function(result){
...
function set_physical() {
    var physical_id = $("select[name='physical']").val();
    $(".js-physical-quantity").hide();
    $(".physical_"+physical_id).show();
}
