$(function(){
	...
	// 分页
	$(".js-json-page").find('a').live('click', function(){
		var page = $(this).attr('data-page-num');
		var type = $(this).parents(".js-json-page").attr('data-type');
	...
	$('.js-add-first').live('click', function(){
		...
		//		menu_html += '</li>';
				menu_html += '<li>';
				menu_html += '<a class="js-modal-generalize" data-type="generalize" data-alias="推广二维码" data-title="" data-url="" href="javascript:void(0);">获取推广二维码</a>';
				menu_html += '</li>';
				menu_html += '<li>';
	...
	$('.js-select-link a').live('click', function(e){
		...
		if ($(this).attr('class') == 'js-modal-txt') {
			...
			$('.popover').show();
		} else if ($(this).attr('class') == 'js-modal-generalize') {
			var data_id = $(this).parents('.js-select-link').attr('data-id');
			$('#title_'+data_id).find('.h5').text($(this).attr('data-alias'));
			$.post('/user.php?c=weixin&a=save_menu',{'id':data_id, 'title':$(this).attr('data-alias')}, function(data){}, 'json');
...
//			html += '</div>';
			...
			html += '</div>';
			html += '<div class="pagenavi js-json-page">'+response.page+'</div>';
			html += '</div>';
...
function url_list(p, type)
{
	...
	$.post('/user.php?c=weixin&a=get_url_list', {'p':p, 'type':type}, function(response){
		...
		var html = '<div class="modal fade js-modal in" aria-hidden="false">';
			...
			html += '<input type="button" class="btn btn-primary" value="确定使用">';
			html += '</div>';
			html += '<div class="pagenavi js-json-page" data-type="'+type+'">'+response.page+'</div>';
			html += '</div>';
			html += '</div><div class="modal-backdrop fade in"></div>';
			$('body').append(html);
		}, 'json');
