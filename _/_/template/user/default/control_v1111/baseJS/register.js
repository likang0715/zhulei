...
$(function(){
	var get_imgcode=function(){
		...
		$.post('./user.php?c=user&a=register_getcode',function(result){
	...
	$('#phoneValidate').click(function(){
		...
		if(is_validate == false){
			...
			$.post('././user.php?c=user&a=register',{phone:phone,nickname:nickname,password:password},function(result){
				if(result.err_code == 0){
	...
	$('#register').live('click',function(){
		...
		$.post('./user.php?action=register_checkcode',{code:c},function(result){
			if(result.err_code == 0){
