...
<head>
    <meta charset="utf-8"/>
    <title>分销统计 - <?php echo $config['site_name'];?>分销平台 | <?php if (empty($_SESSION['sync_store'])) { ?><?php echo $config['site_name'];?><?php } else { ?>微店系统<?php } ?></title>
    ...
    <script type="text/javascript">var load_url="<?php dourl('load');?>", store_id = "<?php echo !empty($_GET['store_id']) ? trim($_GET['store_id']) : $store_session['store_id']; ?>"</script>
    <script type="text/javascript" src="<?php echo TPL_URL;?>js/fx_statistics.js"></script>
</head>
