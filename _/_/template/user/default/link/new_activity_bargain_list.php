...
<table class="ListProduct" border="0" cellSpacing="0" cellPadding="0" width="100%">
    ...
    <tbody>
    <?php foreach($bargain_list as $k=>$v){ ?>
    <tr>
        <td><?php echo $v['name'];?></td>
        <td><a href="javascript:void(0)" onclick="returnHomepage('<?php echo option("config.wap_site_url")."/bargain.php?action=detail&id=".$v['pigcms_id']."&store_id=".$v['store_id']; ?>')" style="margin-left:14px;">选中</a></td>
    </tr>
    <?php } ?>
    </tbody>
</table>
...
<script>
    ...
    $(".pages a").live("click", function () {
        var p = $(this).data("page-num");console.log(p);

        location.href="<?php echo option("config.site_url")."/user.php?c=link&a=new_activity_list&module=bargain&store_id=".$_REQUEST['store_id']."&p=";?>"+p;
    })
</script>
</body>
</html>