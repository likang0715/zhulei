        ...
        <div>
            ...
            <div class="" style="position: absolute; left: 0; top: 0;">
                <select name="physical">
                    <option value="0">查看库存</option>
                    <?php foreach ($store_physical as $value) { ?>
                    <option value="<?php echo $value['pigcms_id'] ?>" <?php if($physical_id == $value['pigcms_id']) echo 'selected=selected'; ?>><?php echo $value['name'] ?></option>
                    <?php } ?>
                </select>
            </div>
        ...
        <table class="ui-table ui-table-list" style="padding: 0px;">
            ...
            <tr>
                ...
                <th class="cell-8 text-right" style="min-width: 68px; max-width: 68px;">
                    <a href="javascript:;" class="orderby" data-orderby="quantity">主商品库存</a>
                </th>
                <th class="cell-8 text-right" style="min-width: 68px; max-width: 68px;">
                ...
                <?php foreach ($store_physical as $val) { ?>
                    <th class="cell-8 text-right hide js-physical-quantity physical_<?php echo $val['pigcms_id'] ?>" style="min-width: 68px; max-width: 120px;"><?php echo $val['name'] ?><br>(库存)</th>
                <?php } ?>
                <?php } ?>
            ...
            <?php foreach ($products as $product) { ?>
                <tr>
                    ...
                    <?php if (!empty($product['physical'])) { ?>
                        <?php foreach ($product['physical'] as $val) { ?>
                        <td class="hide js-physical-quantity physical_<?php echo $val['pigcms_id'] ?>">
                ...
                <?php if (!empty($product['has_property']) && !empty($product['sku'])) { //商品规格列表 ?>
                    ...
                    <tr>
                        ...
                        <td class="">----</td>
                        <?php if (!empty($product_sku['physical'])) { ?>
                            <?php foreach ($product_sku['physical'] as $val) { ?>
                            <td class="hide js-physical-quantity physical_<?php echo $val['pigcms_id'] ?>">
                                <?php echo $val['quantity'] ?>
                            </td>
                            <?php } ?>
