...
$methodList['goods'] = !empty($methodList['goods']) ? $methodList['goods'] : array();
$methodList['order'] = !empty($methodList['order']) ? $methodList['order'] : array();
$methodList['trade'] = !empty($methodList['trade']) ? $methodList['trade'] : array();
?>
<style type="text/css">
...
<form class="form-horizontal" style="">
        ...
        <div class="controls ui-regions js-regions-wrap">
            <span>
                <select name="item_store" id="store_id" style="width:220px;">
                    <!-- <option value="<?php echo $store_physical_name['pigcms_id'];?>" ><?php echo $store_physical_name['name']?>√</option> -->
                    <?php foreach($store_physical as $store_name):?>
                        <?php if ($store_name['pigcms_id'] == $store_physical_name['pigcms_id']) { ?>
                            <option value="<?php echo $store_name['pigcms_id'];?>" selected=selected><?php echo $store_name['name'];?> √</option>
                        <?php } else { ?>
        ...
        <div class="controls js-goods-tip">
            ...
            <?php foreach ($rbac['goods'] as $method => $nameTxt) { ?>
                <label class="checkbox inline <?php if (in_array($method, $rbacLink['goods'])) echo 'link-color'; ?>">
                    <input  type="checkbox" class="js-not-autoup" name="goods[]" <?php echo in_array($method, $methodList['goods']) ? 'checked' : '';?> value="<?php echo $method ?>"><?php echo $nameTxt ?>
                </label>
            <?php } ?>
        ...
        <div class="controls js-order-tip">
            ...
            <?php foreach ($rbac['order'] as $method => $nameTxt) { ?>
                <label class="checkbox inline <?php if (in_array($method, $rbacLink['order'])) echo 'link-color'; ?>">
                    <input  type="checkbox" class="js-not-autoup" name="order[]" <?php echo in_array($method, $methodList['order']) ? 'checked' : '';?> value="<?php echo $method ?>"><?php echo $nameTxt ?>
                </label>
            <?php } ?>
                <div class="ipt-checkAll">
                    <label>
        ...
        <div class="controls js-express-tip">
            ...
            <?php foreach ($rbac['trade'] as $method => $nameTxt) { ?>
                <label class="checkbox inline <?php if (in_array($method, $rbacLink['trade'])) echo 'link-color'; ?>">
                    <input  type="checkbox" class="js-not-autoup" name="trade[]" <?php echo in_array($method, $methodList['trade']) ? 'checked' : '';?> value="<?php echo $method ?>"><?php echo $nameTxt ?>
                </label>
            <?php } ?>
                <div class="ipt-checkAll">
                    <label>
    ...
    <div class="form-actions" style="margin-top:50px">
        ...
        <button type="button" class="ui-btn js-btn-reset">重置</button>
    </div>
</form>
...
<script>
    $(function(){
        ...
        $('#sub-button').click(function(){
            ...
            if(iptPassword.val() == '' || iptPassword.val()<6)
            {
