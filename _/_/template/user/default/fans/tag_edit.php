...
<div class="app__content">
		<form class="js-page-form form-horizontal ui-form" method="POST" novalidate="novalidate">
			<div class="control-group">
				...
				<div class="controls">
					...
					<ul class="hyname">
						...
						<li><input type="radio" name="rule_type" <?php if($tag['rule_type']=='5'){?> checked="checked" <?php }?> value="5">&nbsp;<input style="display:inline-block;width:70px;" type="text" class="for-post" maxlength="10" name="rule_name" value="<?php if($tag['rule_type']=='5'){ echo $tag['name'];}?>"></li>
					</ul>	
			...
			<div class="control-group">
				...
				<div class="controls">
					<input type="text" id="level_num" name="level_num" value="<?php echo $tag['level_num']?>" placeholder="请输入数字" style="width:100px;">
	...
	<script>
		...
		<div class="control-group">
				...
				<div class="controls">
					<ul class="ico app-image-list js-ico-list">
						<li class="other_li sort">
							<div class="spans">
								<span class="checkico <?php if($tag['level_pic'] == './static/images/huiyuan/1_01.png' ){?>selected-style <?php } else {?>no-selected-style<?php }?>"><i class="icon-ok icon-white"></i>
								<img class="avatar" src="<?php echo STATIC_URL;?>images/huiyuan/1_01.png">
								</span>
							</div>
						</li>
						<li class="other_li sort">
							<div class="spans">
								<span class="checkico <?php if($tag['level_pic'] == './static/images/huiyuan/1_02.png' ){?>selected-style <?php } else {?>no-selected-style<?php }?>"><i class="icon-ok icon-white"></i>
								<img class="avatar" src="<?php echo STATIC_URL;?>images/huiyuan/1_02.png">
								</span>
							</div>
						</li>
						<li class="other_li sort">
							<div class="spans">
								<span class="checkico  <?php if($tag['level_pic'] == './static/images/huiyuan/1_03.png' ){?>selected-style <?php } else {?>no-selected-style<?php }?>"><i class="icon-ok icon-white"></i>
								<img class="avatar" src="<?php echo STATIC_URL;?>images/huiyuan/1_03.png">
								</span>
							</div>
						</li>
						<li class="other_li sort">
							<div class="spans">
								<span class="checkico  <?php if($tag['level_pic'] == './static/images/huiyuan/1_04.png' ){?>selected-style <?php } else {?>no-selected-style<?php }?>"><i class="icon-ok icon-white"></i>
								<img class="avatar" src="<?php echo STATIC_URL;?>images/huiyuan/1_04.png">
								</span>
							</div>
						</li>
						<?php if(preg_match('/^images\//',$tag['level_pic'])) {?>
									
							<li class="sort">
								<div class="spans">
									<span class="checkico selected-style">
										<i class="icon-ok icon-white"></i>
										<img src="<?php echo getAttachmentUrl($tag['level_pic']);?>" class="avatar">
									</span>
			...
			<div class="control-group">
				...
				<!--
				<span class="help-inline"><input type="radio" name="points_exchange_type" value="1" <?php if($tag['exchange_type']==1) {?>checked="checked"<?php }?>> 手工兑换&#12288;<input name="points_exchange_type" type="radio" value="2" <?php if($tag['exchange_type']==2) {?>checked="checked"<?php }?>> 自动升级</span>
				-->
				</div>
			</div>
			...
			<div class="control-group">
				...
				<div class="controls">
						...
						<p class="help-desc" style="clear:none;display:inline-block;font-style:italic">　等级有效期&nbsp; <input name="degree_month" style="width:30px;" type="text" size="2" maxlength="2" value="<?php echo $tag['degree_month'];?>">&nbsp;个月 </p>
				...
				<div class="controls">
					...
					<table class="table" width="100%">
						<tr class="control-group">
							<td class="controls" height="35"><input name="power" value="is_discount" type="checkbox" <?php if($tag['is_discount']){?>checked="checked"<?php }?>>会员折扣
								&#12288;&#12288;
								<div class="input-append">
									<input type="text" id="discount" name="discount" value="<?php echo $tag['discount'];?>" placeholder="请输入折扣" style="width:73px;"><span class="add-on">折</span>
								</div>
								<p class="help-desc" style="clear:none;display:inline-block;">&#12288;留空或10.0，为不打折。</p>
							</td>
						</tr>
					<tr class="control-group"><td  class="controls" height="35">	<input type="checkbox" id="is_postage_free" name="power" value="is_postage_free" <?php if($tag['is_postage_free']){?>checked="checked"<?php }?>>包邮</tr>
					<tr class="control-group"><td  class="controls" height="35"><input type="checkbox" id="is_points_discount_toplimit" name="power" value="is_points_discount_toplimit" <?php if($tag['is_points_discount_toplimit']){?>checked="checked"<?php }?>>积分抵现上限 （每单） 
					&#12288;&#12288;<input style="width:30px" type="text" maxlength="4" name="points_discount_toplimit" class="input-mini for-post" value="<?php echo $tag['points_discount_toplimit'];?>"> 元
					
					<p class="help-desc" style="clear:none;display:inline-block;">&#12288;要比自定义下级上限高。</p>
					</td></tr>
					<tr class="control-group"><td class="controls" height="35">
						<input type="checkbox" id="is_points_discount_ratio" name="power" value="is_points_discount_ratio" <?php if($tag['is_points_discount_ratio']){?>checked="checked"<?php }?>>积分在订单金额抵现比例
					&#12288;&#12288;<input style="width:30px" type="text" name="points_discount_ratio" maxlength="2" size=2 class="input-mini for-post" value="<?php echo $tag['points_discount_ratio'];?>"> %
					<p class="help-desc" style="clear:none;display:inline-block;">&#12288;要比自定义下级上限高</p>
					 </td></tr>
				</table>
				</div>
			...
			<div class="control-group">
				...
				<div class="controls">
					<textarea id="description" class="description"   style="width:65%" name="description" cols="55" rows="4" placeholder="最多可输入100个字符，简述相关会员卡信息，以便会员知晓。" maxlength="300"><?php echo $tag['description'];?></textarea>
				</div>
			</div>			
			<div class="form-actions">
				<input class="btn btn-primary js-btn-edit-save" type="button" value="保 存" data-loading-text="保 存...">
				<input type="hidden" value="<?php echo $tag['id']?>" id="tag" >
