...
<html>
	<head>
		<meta charset="utf-8"/> 
		<title>积分规则管理 - <?php echo $store_session['name']; ?> | <?php if (empty($_SESSION['sync_store'])) { ?><?php echo $config['site_name'];?><?php } else { ?>微店系统<?php } ?></title>
		...
		<script type="text/javascript">
			...
			var location_type = "points";
			var page_create = "points_create";
			var page_edit = "points_edit";
			var page_list_url = "<?php echo url("points")?>";
			var delete_url = "<?php dourl('delete',array('type'=>'points')) ?>";
			var page_content = "points_content";
			var page_product_list = "product_list";

			var disabled_url = "<?php dourl('disabled',array('type'=>'points')) ?>";
			var able_url = "<?php dourl('able',array('type'=>'points')) ?>";
		</script>
		<script type="text/javascript" src="<?php echo TPL_URL;?>js/kehu.js?time=<?php echo time();?>"></script>
	</head>
