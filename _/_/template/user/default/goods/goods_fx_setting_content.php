    ...
    <div class="goods-info-group-inner">
        ...
        <div class="info-group-cont vbox">
            <div class="group-inner">
                ...
                <?php } ?>
                <?php if (empty($open_drp_setting_price)) { ?>
                <?php if ($_SESSION['store']['drp_level'] < 3) { ?>
                <div class="control-group">
                ...
                <?php } ?>
                <?php } else { ?>
                    <input type="hidden" name="unified_price_setting" class="unified-price-setting" value="<?php echo $unified_price_setting; ?>" />
                <?php } ?>
            </div>
        </div>
    </div>
                ...
                <div class="group-inner">
                    ...
                    <div class="js-goods-stock control-group">
                        <div id="stock-region" class="controls sku-stock" style="margin-left: 0;">
                            <div class="sku-price-0 <?php if (!empty($open_drp_setting_price) && !empty($unified_price_setting)) { ?>hide<?php } ?>">
                                <table class="table-sku-stock">
                                    <?php echo $sku_content; ?>
                                </table>
                            </div>
                            <?php if (empty($product['source_product_id'])) { ?>
                            <div class="sku-price-1 <?php if (empty($open_drp_setting_price) || empty($unified_price_setting)) { ?>hide<?php } ?>">
                                <br/>
                                <p class="tip" style="color: orange">* <b>温馨提示：</b>如果您为各级分销商设置了统一的成本价格或分销价格就可以使用微店的多级分销功能。</p>
                                <?php if (empty($drp_level)) { ?>
                                <br/>
                                <b style="color: #0000ff">一级分销商定价(<b class="tip" style="color: red">温馨提示：成本价是您给分销商的价格,分销价是指分销商卖给消费者的终端价</b>)</b>
                                <table class="table-sku-stock table-sku-stock-1" data-level="1">
                                    <?php echo $sku_content2; ?>
                                </table>
                                <br/>
                                <b style="color: #0000ff">二级分销商定价(<b class="tip" style="color: red">温馨提示：成本价是您给分销商的价格,分销价是指分销商卖给消费者的终端价</b>)</b>
                                <table class="table-sku-stock table-sku-stock-2" data-level="2">
                                    <?php echo $sku_content2; ?>
                                </table>
                                <br/>
                                <b style="color: #0000ff">三级分销商定价(<b class="tip" style="color: red">温馨提示：成本价是您给分销商的价格,分销价是指分销商卖给消费者的终端价</b>)</b>
                    ...
                    <div class="control-group">
                        ...
                        <?php } else { ?>
                            ...
                            <div class="cost-price-0 <?php if (!empty($open_drp_setting_price) && !empty($unified_price_setting)) { ?>hide<?php } ?>">
                        ...
                        <?php } ?>
                        <?php if (empty($product['source_product_id'])) { ?>
                            <div class="cost-price-1 <?php if (empty($open_drp_setting_price) || empty($unified_price_setting)) { ?>hide<?php } ?>">
                                <?php if (empty($drp_level)) { ?>
                                    <div class="controls">
                    ...
                    <div class="control-group">
                        ...
                        <div class="price-0 <?php if (!empty($open_drp_setting_price) && !empty($unified_price_setting)) { ?>hide<?php } ?>">
                            <div class="controls" style="margin-top: -5px">
                                <div class="input-prepend">
                                    <span class="add-on">￥</span><input type="text" maxlength="10" name="fx-min-price" class="input-small" <?php if (!$is_supplier) { ?><?php if ($min_fx_price > 0) { ?>data-min-price="<?php echo $min_fx_price; ?>" value="<?php echo $min_fx_price; ?>"<?php } ?><?php } ?> /> <span style="display:inline-block;margin-top:10px">-</span> <span class="add-on">￥</span><input type="text" maxlength="10" name="fx-max-price" class="input-small" <?php if (!$is_supplier) { ?><?php if ($max_fx_price > 0) { ?>data-max-price="<?php echo $max_fx_price; ?>" value="<?php echo $max_fx_price; ?>"<?php } ?><?php } ?> />
                                </div>
                            </div>
                        </div>
                        <?php } ?>
                        <?php if (empty($product['source_product_id'])) { ?>
                        <div class="price-1 <?php if (empty($open_drp_setting_price) || empty($unified_price_setting)) { ?>hide<?php } ?>">
                            <?php if (empty($drp_level)) { ?>
                            <div class="controls">
                                <b style="color:#0000ff">一级分销商</b>
                                <div class="input-prepend">
