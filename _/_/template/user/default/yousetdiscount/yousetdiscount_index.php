...
<html>
	<head>
		<meta charset="utf-8"/> 
		<title>优惠接力 - <?php echo $store_session['name']; ?> | <?php if (empty($_SESSION['sync_store'])) { ?><?php echo $config['site_name'];?><?php } else { ?>微店系统<?php } ?></title>
		...
		<script type="text/javascript" src="<?php echo TPL_URL;?>js/yousetdiscount.js"></script>
		
		<script type="text/javascript">
			var load_url = "<?php dourl('load');?>";
			var yousetdiscount_index_url = "<?php dourl('yousetdiscount:yousetdiscount_index') ?>";

			var yousetdiscount_create_url = "<?php dourl('yousetdiscount:create') ?>";
			var yousetdiscount_edit_url = "<?php dourl('yousetdiscount:doupdate') ?>";
			var yousetdiscount_operate_url = "<?php dourl('yousetdiscount:operate') ?>";

			var yousetdiscount_wap_url = "<?php echo option('config.site_url') ?>/wap/yousetdiscount.php?action=index";	//前台开发后修改
			var yousetdiscount_qrcode_url = "<?php echo option("config.site_url") ?>/source/qrcode.php?type=yousetdiscount";	//前台开发后修改
		</script>
	</head>
