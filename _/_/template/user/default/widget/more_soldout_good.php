			...
			$(function(){
				...
				$('.js-confirm-choose').live('click',function(){
					...
					$.each($('.js-choose.btn-primary'),function(i,item){
						data_arr[i] = {'id':$(item).data('id'),'title':$(item).data('title'),'image':$(item).data('image'),'quantity':$(item).data('quantity'),'price':$(item).data('price'),'url':'<?php echo $config['wap_site_url'];?>/good.php?id='+$(this).data('id')};
					});
				...
				$('.js-page-list a').live('click',function(e){
					if(!$(this).hasClass('active')){
						...
						$('body').load('<?php dourl('do_self_soldout_good',array('type'=>'more'));?>',{p:$(this).data('page-num'),'keyword':input_val},function(){
							$('.js-modal iframe',parent.document).height($('body').height());
						});
					}
				});
				$('.js-modal-search').live('click',function(e){
					var input_val = $('.js-modal-search-input').val();
					$('body').html('<div class="loading-more"><span></span></div>');
					$('body').load('<?php dourl('do_self_soldout_good',array('type'=>'more'));?>',{'keyword':input_val},function(){
						$('.js-modal iframe',parent.document).height($('body').height());
					});
					return false;
				});
	...
	<body style="background-color:#ffffff;">
		<div class="modal-header">
			...
			<ul class="module-nav modal-tab">
				<li class="active"><a href="javascript:void(0);" class="js-modal-tab">仓库中商品</a> <!--|--></li>
						...
						<tbody>
							<?php foreach($products as $product){ ?>
								<tr>
									...
									<td class="opts">
										<div class="td-cont">
											<button class="btn js-choose" data-id="<?php echo $product['product_id'];?>" data-title="<?php echo $product['name']; ?>" data-price="<?php echo $product['price']; ?>" data-quantity="<?php echo $product['quantity']?>" data-image="<?php echo $product['image'];?>">选取</button>
										</div>
									</td>
								</tr>
							<?php } ?>
