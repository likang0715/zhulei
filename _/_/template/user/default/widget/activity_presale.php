		...
		$(function(){
			...
			$('.js-page-list a').live('click', function(e) {
				if (!$(this).hasClass('active')) {
					...
					$('body').load('<?php dourl('activity_module', array('huodong_type' => 'presale', 'type'=>'ajax')); ?>', {p: $(this).data('page-num'), 'keyword': input_val}, function() {
						$('.js-modal iframe', parent.document).height($('body').height());
					});
				}
			});
			...
			$('.js-modal-search').live('click', function(e) {
				...
				$('body').html('<div class="loading-more"><span></span></div>');
				$('body').load('<?php dourl('activity_module',array('huodong_type' => 'presale', 'type'=>'ajax')); ?>', {'keyword': input_val}, function() {
					$('.js-modal iframe', parent.document).height($('body').height());
				});
				return false;
			});
			...
			<div class="tab-content">
				<div id="js-module-feature" class="tab-pane module-feature active">
					...
					<table class="table">
						...
						<thead>
							<tr>
								<th class="title" style="background-color:#f5f5f5;">
									<div class="td-cont">
										<span>预售活动标题</span> <a class="js-update" href="javascript:window.location.reload();">刷新</a>
									</div>
								</th>
						...
						<!-- 表格数据区 -->
						<tbody>
							<?php foreach($presales as $presale){ ?>
								<tr>
									<td class="title" style="max-width:300px;">
										<div class="td-cont">
											<a target="_blank" class="new_window" href=""><?php echo $presale['name']; ?></a>
										</div>
									</td>
									<td class="time">
										<div class="td-cont">
											<span><?php echo date('Y-m-d H:i:s', $presale['starttime']); ?></span>
										</div>
									</td>
									<td class="opts">
										<div class="td-cont">
											<button class="btn js-choose" data-id="<?php echo $presale['id'];?>" data-atype="1" data-title="<?php echo $presale['name'];?>">选取</button>
<!--											<button class="btn js-choose" data-id="--><?php //echo $product['product_id'];?><!--" data-title="--><?php //echo $product['name']; ?><!--" data-price="--><?php //echo $product['price']; ?><!--" data-image="--><?php //echo $product['image'];?><!--">选取</button>-->
		...
		<div class="modal-footer">
			...
			<div class="pagenavi js-page-list" style="margin-top:0;padding-top:2px;"><?php echo $presale_page; ?></div>
		</div>
	</body>
</html>