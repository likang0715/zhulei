			...
			$(function(){
				...
				$('.js-confirm-choose').live('click',function(){
					...
					$.each($('.js-choose.btn-primary'),function(i,item){
						data_arr[i] = {'id':$(item).data('id'),'title':$(item).data('title'),'image':$(item).data('image'),'price':$(item).data('price'),'url':'<?php echo $config['wap_site_url'];?>/chahui_show.php?id='+$(this).data('id')};
					});
				...
				$('.js-page-list a').live('click',function(e){
					if(!$(this).hasClass('active')){
						...
						$('body').load('<?php dourl('chahui',array('type'=>'more'));?>',{p:$(this).data('page-num'),'keyword':input_val, 'no_group' : '<?php echo $_REQUEST['no_group'] ?>'},function(){
							$('.js-modal iframe',parent.document).height($('body').height());
						});
					}
				});
				$('.js-modal-search').live('click',function(e){
					var input_val = $('.js-modal-search-input').val();
					$('body').html('<div class="loading-more"><span></span></div>');
					$('body').load('<?php dourl('chahui',array('type'=>'more'));?>',{'keyword':input_val, 'no_group' : '<?php echo $_REQUEST['no_group'] ?>'},function(){
						$('.js-modal iframe',parent.document).height($('body').height());
					});
					return false;
				});
						...
						<tbody>
							<?php foreach($chahui as $product){ ?>
								<tr>
									...
									<td class="opts">
										<div class="td-cont">
											<button class="btn js-choose" data-id="<?php echo $product['pigcms_id'];?>" data-title="<?php echo $product['name']; ?>" data-price="<?php echo $product['price']; ?>" data-image="<?php echo $product['images'];?>">选取</button>
										</div>
									</td>
								</tr>
							<?php } ?>
