	...
	<head>
		...
		<script type="text/javascript">
			$(function(){
				...
				$('.js-page-list a').live('click',function(e){
					if(!$(this).hasClass('active')){
						...
						$('body').load('<?php dourl('goods_by_sku');?>',{p : $(this).data('page-num'), 'keyword' : input_val, 'is_ajax' : true, 'uid' : '<?php echo $_REQUEST["uid"] ?>', 'status': status, 'group_id': group_id}, function(){
							$('.js-modal iframe',parent.document).height($('body').height());
						});
					}
				});
				$('.js-modal-search').live('click',function(e){
					...
					var group_id = $("input[name='group_id']").val();
					$('body').html('<div class="loading-more"><span></span></div>');
					$('body').load('<?php dourl('goods_by_sku');?>', {'keyword' : input_val, 'is_ajax' : true, 'uid' : '<?php echo $_REQUEST["uid"] ?>', 'status': status, 'group_id': group_id},function(){
						$('.js-modal iframe',parent.document).height($('body').height());
					});
					return false;
				});
			...
			$(".js-modal-tab").live("click", function () {
				...
				$('body').html('<div class="loading-more"><span></span></div>');
				$('body').load('<?php dourl('goods_by_sku');?>', {'is_ajax' : true, 'uid' : '<?php echo $_REQUEST["uid"] ?>', 'status': status},function(){
					$('.js-modal iframe',parent.document).height($('body').height());
				});
				return false;
			});

			$(".js-product_group").live("change", function () {
				var group_id = $(this).val();
				
				$('body').html('<div class="loading-more"><span></span></div>');
				$('body').load('<?php dourl('goods_by_sku');?>', {'is_ajax' : true, 'uid' : '<?php echo $_REQUEST["uid"] ?>', 'status': 3, 'group_id': group_id},function(){
					$('.js-modal iframe',parent.document).height($('body').height());
				});
				return false;
			});
	...
	<body style="background-color:#ffffff;">
		...
		<div class="modal-header">
			...
			<ul class="module-nav modal-tab">
				<li class="active"><a href="javascript:" class="js-modal-tab" data-status="1">出售中商品列表</a> |</li>
				<li><a href="#" class="js-modal-tab" data-status="2">仓库中商品列表</a> |</li>
				<li>
