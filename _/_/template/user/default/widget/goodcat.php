...
/*				$('.js-confirm-choose').live('click',function(){
                 ...
                 */
                 ...
                 $('button.js-choose').live('click',function(){
                    try{
                        parent.login_box_after('<?php echo $_GET['number'];?>','goodcat',[$(this).data('id'),$(this).data('title')],'<?php echo $config['wap_site_url'];?>/goodcat.php?id='+$(this).data('id')+'&selecteditems='+"");
                    }catch(e){
                        var data_arr = [];
                        var group_id = $(this).data('id');
                        var product_list_url = '<?php echo dourl('widget:productList')?>';
                        $.post(product_list_url,{'group_id':group_id,'selecteditems':''},function(data){
                            if(data.err_code == 0){
                                for(var i=0;i<data.err_msg.length;i++){
                                    data_arr[i] = {'image':data.err_msg[i]['image'],'price':data.err_msg[i]['price'],'id':data.err_msg[i]['id'],'title':data.err_msg[i]['title'],'url':'<?php echo $config['wap_site_url'];?>/good.php?id='+data.err_msg[i]['id']};
                                }
                            }
                            parent.widget_box_after('<?php echo $_GET['number'];?>',data_arr);
                        });
				...
				$('button.js-choose').live('click',function(){
                    ...
                    $.post(product_list_url,{'group_id':group_id},function(data){
                        if(data.err_code == 0){
                            for(var i=0;i<data.err_msg.length;i++){
                                data_arr[i] = {'image':data.err_msg[i]['image'],'price':data.err_msg[i]['price'],'id':data.err_msg[i]['id'],'title':data.err_msg[i]['title'],'url':'<?php echo $config['wap_site_url'];?>/good.php?id='+data.err_msg[i]['product_id']};
                            }
                        }
                        <?php if(empty($_GET['only'])){ ?>
                        parent.widget_box_after('<?php echo $_GET['number'];?>',data_arr);
