...
<div class="modal-header">
	...
	<ul class="module-nav modal-tab" style="display:none">
		<li<?php if(empty($_GET['zt'])){?> class="active"<?php } ?>><a href="<?php dourl('chahui',array('number'=>$_REQUEST['number']));?>" class="js-modal-tab">全部茶会</a> |</li>
				
				<?php foreach($category as  $key=>$r){ ?>
				
				<li <?php if($_GET['zt']==$r['cat_id']){?> class="active"<?php } ?>><a href="<?php dourl('chahui',array('zt'=> $r['cat_id'],'number'=>$_REQUEST['number']));?>" class="js-modal-tab"><?php echo $r['cat_name'];?></a> |</li>
							...
							<div class="td-cont" style="padding:7px 0 3px 10px;">
								<form class="form-search" onsubmit="return false;">
									<div class="input-append">
										<input class="input-small js-modal-search-input" type="text" style="border-radius:4px 0px 0px 4px;" value="<?php echo $_POST['keyword']?>"/><a href="javascript:void(0);" class="btn js-fetch-page js-modal-search" style="color:white;border-radius:0 4px 4px 0;margin-left:0px;">搜</a>
									</div>
								</form>
							</div>
						</th>
