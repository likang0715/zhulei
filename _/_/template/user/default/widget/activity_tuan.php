		...
		$(function(){
			...
			$('.js-page-list a').live('click', function(e) {
				if (!$(this).hasClass('active')) {
					var input_val = $('.js-modal-search-input').attr("search_value");
					$('body').html('<div class="loading-more"><span></span></div>');
					$('body').load('<?php dourl('activity_module', array('type'=>'ajax')); ?>', {p: $(this).data('page-num'), 'keyword': input_val}, function() {
						$('.js-modal iframe', parent.document).height($('body').height());
					});
				}
			});
			...
			$('.js-modal-search').live('click', function(e) {
				...
				$('body').html('<div class="loading-more"><span></span></div>');
				$('body').load('<?php dourl('activity_module', array('type'=>'ajax')); ?>', {'keyword': input_val}, function() {
					$('.js-modal iframe', parent.document).height($('body').height());
				});
				return false;
			});
			...
			<table class="table">
				...
				<tr>
					<th class="title" style="background-color:#f5f5f5;">
						<div class="td-cont">
							<span>团购活动名称</span> <a class="js-update" href="javascript:window.location.reload();">刷新</a>
						</div>
					</th>
						...
						<div class="td-cont" style="padding:7px 0 3px 10px;">
							<form class="form-search" onsubmit="return false;">
								<div class="input-append">
									<input class="input-small js-modal-search-input" value="<?php echo htmlspecialchars($_REQUEST['keyword']); ?>"  type="text" style="border-radius:4px 0px 0px 4px;"/><a href="javascript:void(0);" class="btn js-fetch-page js-modal-search" style="color:white;border-radius:0 4px 4px 0;margin-left:0px;">搜</a>
								</div>
							</form>
						</div>
					</th>
					...
					<?php foreach($tuans as $tuan){ ?>
						<tr>
							<td class="title" style="max-width:300px;">
								<div class="td-cont">
									<a target="_blank" class="new_window" href="javascript:"><?php echo $tuan['name']; ?></a>
								</div>
							</td>
							<td class="time">
								<div class="td-cont">
									<span><?php echo date('Y-m-d H:i:s', $tuan['start_time']); ?></span>
								</div>
							</td>
							<td class="opts">
								<div class="td-cont">
									<button class="btn js-choose" data-id="<?php echo $tuan['id'];?>" data-atype="2" data-title="<?php echo $tuan['name'];?>">选取</button>
								</div>
							</td>
						</tr>
					<?php } ?>
...
<div class="modal-footer">
	<div style="display:none;" class="js-confirm-choose left">
		...
		<div class="pagenavi js-page-list" style="margin-top:0;padding-top:2px;"><?php echo $tuan_page; ?></div>
</div>
</body>
</html>