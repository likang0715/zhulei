			...
			$(function(){
				...
				$('button.js-choose').live('click',function(){
					<?php if(empty($_GET['only'])){ ?>
						parent.login_box_after('<?php echo $_GET['number'];?>','page',$(this).data('title'),'<?php echo $config['wap_site_url'];?>/page.php?id='+$(this).data('id'));
					<?php }else{ ?>
						parent.login_box_after('<?php echo $_GET['number'];?>','page',[$(this).data('id'),$(this).data('title')],'<?php echo $config['wap_site_url'];?>/page.php?id='+$(this).data('id'));
					<?php } ?>
				});
