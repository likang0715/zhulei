...
<head>
	...
	<script type="text/javascript">
		$(function(){
			...
			// 选取
			$('button.js-choose').on('click',function(){
			...
			$('.js-page-list a').live('click',function(e){
				if(!$(this).hasClass('active')) {
					...
					$('body').load('<?php dourl('article_module', array()); ?>', {p: $(this).data('page-num'), 'keyword': input_val}, function(){
						$('.js-modal iframe', parent.document).height($('body').height());
					});
				}
			});
			...
			$('.js-modal-search').live('click',function(e) {
				...
				$('body').html('<div class="loading-more"><span></span></div>');
				$('body').load('<?php dourl('article_module', array()); ?>', {'keyword': input_val}, function() {
					$('.js-modal iframe', parent.document).height($('body').height());
				});
				return false;
			});
					...
					?>
						<tr>
							<td class="title" style="max-width:300px;">
								<div class="td-cont">
									<a target="_blank" class="new_window" href="javascript:"><?php echo $article['title']; ?></a>
								</div>
							</td>
							<td class="time">
								<div class="td-cont">
									<span><?php echo date('Y-m-d H:i:s',$article['dateline']); ?></span>
								</div>
							</td>
							<td class="opts">
								<div class="td-cont">
									<button class="btn js-choose" data-id="<?php echo $article['id'];?>" data-atype="8" data-title="<?php echo $article['title'];?>">选取</button>
								</div>
							</td>
						</tr>
					<?php 
