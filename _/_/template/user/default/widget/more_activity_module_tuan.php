...
//				分页
            $('.js-page-list a').live('click',function(e){
                if(!$(this).hasClass('active')){
                    ...
                    $('body').load('<?php dourl('activity_module_tuan',array('type'=>'more'));?>',{p:$(this).data('page-num'),'keyword':input_val},function(){
                        $('.js-modal iframe',parent.document).height($('body').height());
                    });
                }
            });
...
//				搜索
            $('.js-modal-search').live('click',function(e){
                var input_val = $('.js-modal-search-input').val();
                $('body').html('<div class="loading-more"><span></span></div>');
                $('body').load('<?php dourl('activity_module_tuan',array('type'=>'more'));?>',{'keyword':input_val},function(){
                    $('.js-modal iframe',parent.document).height($('body').height());
                });
                return false;
            });
...
<div class="modal-header">
    ...
    <ul class="module-nav modal-tab">
        <li><a href="<?php dourl('activity_module',array('number'=>$_GET['number']));?>" class="js-modal-tab">预售活动</a></li>
        <li class="tuan-active"><a href="<?php dourl('activity_module_tuan',array('number'=>$_GET['number']));?>" class="tuan-js-modal-tab">团购活动</a></li>
            ...
            <table class="table">
                ...
                <tr>
                    <th class="title" style="background-color:#f5f5f5;">
                        <div class="td-cont">
                            <span>团购活动目录</span> <a class="js-update" href="javascript:window.location.reload();">刷新</a>
                        </div>
                    </th>
