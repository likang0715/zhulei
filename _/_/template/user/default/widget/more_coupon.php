...
//				分页
				$('.js-page-list a').live('click',function(e){
					if(!$(this).hasClass('active')){
						...
						$('body').load('<?php dourl('coupon',array('type'=>'more'));?>',{p:$(this).data('page-num'),'keyword':input_val},function(){
							$('.js-modal iframe',parent.document).height($('body').height());
						});
					}
				});
...
//				搜索
				$('.js-modal-search').live('click',function(e){
					var input_val = $('.js-modal-search-input').val();
					$('body').html('<div class="loading-more"><span></span></div>');
					$('body').load('<?php dourl('coupon',array('type'=>'more'));?>',{'keyword':input_val},function(){
						$('.js-modal iframe',parent.document).height($('body').height());
					});
					return false;
				});
						...
						<tbody>
							<?php foreach($coupons as $product){ ?>
								<tr>
									...
									<td class="time">
										<div class="td-cont">
											<span><?php echo $product['number'].'/'.$product['total_amount']; ?></span>
										</div>
									</td>
									<td class="opts">
										<div class="td-cont">
											<button class="btn js-choose" data-id="<?php echo $product['id'];?>" data-facemoney="<?php echo $product['face_money']?>" data-title="<?php echo $product['name']; ?>" data-condition="<?php echo $product['description']; ?>" data-number="<?php echo $product['number'] ?>" data-total_amount="<?php echo $product['total_amount'] ?>">选取</button>
										</div>
									</td>
								</tr>
							<?php } ?>
