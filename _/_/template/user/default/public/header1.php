...
<?php  
if($_SERVER['SERVER_NAME'] != 'dd2.pigcms.com' && $_SERVER['SERVER_NAME'] != 'www.weidian.com'){
?>
<style>
    /*新功能 ，菜单添加class，外网隐藏*/
               ...
               <?php if(in_array(10, $rbac_val) || $package_id == 0):?>
                ...
                <?php if($user_session['type']!=1):?>
                    <?php if (!empty($_SESSION['drp_diy_store'])) { ?>
                        ...
                        <?php } ?>
                    <?php } ?>
                    <?php if (!empty($allow_store_drp) || !empty($allow_platform_drp)) { ?>

                   <?php if(in_array(14, $rbac_val) || in_array(15, $rbac_val) || $package_id == 0):?>
                        <!--<li <?php if($select_nav == 'fx') echo 'class="active"';?>>
                            <?php if(empty($version) && empty($_SESSION['store']['drp_supplier_id'])) { ?>
                                ...
                                <?php else:?>
                                    <a href="<?php echo $_SESSION['store']['drp_level']==0 ? PackageConfig::setDefaultLink(array(14,15),$rbac_val) : dourl('fx:index'); ?>">分销/批发</a>
                                <?php endif;?>
                                ...
                                <?php else:?>
                                    <a href="<?php echo $_SESSION['store']['drp_level']==0 ? PackageConfig::setDefaultLink(array(14,15),$rbac_val) : dourl('fx:index'); ?>">分销</a>
                                <?php endif;?>

                                <?php } else if (!empty($allow_platform_drp)) { ?>
                               
                                <?php if($package_id == 0): ?>
                                    <a href="<?php echo dourl('fx:supplier_market'); ?>">批发</a>
                                <?php else:?>
                                    <a href="<?php echo PackageConfig::setDefaultLink(array(15,15),$rbac_val); ?>">批发</a>
                                <?php endif;?>
                    ...
                    <?php } ?>
                <?php endif;?>-->
        <!--<?php if(in_array(16, $rbac_val) || in_array(17, $rbac_val) || $package_id == 0):?>
                <?php if (!empty($_SESSION['drp_diy_store'])) { ?>
                <?php if(empty($version) && empty($_SESSION['sync_store'])){?>
                <?php if($user_session['type']!=1): ?>
                    <li class=" js-weixin-notify <?php if($select_nav == 'substore') echo 'active';?>">                   
                    <?php if($package_id == 0): ?>
                        <a href="<?php echo dourl('substore:store_list'); ?>">门店仓库物流</a>
                    <?php else:?>
                        <a href="<?php echo PackageConfig::setDefaultLink(array(16,17),$rbac_val); ?>">门店仓库物流</a>
                    <?php endif;?>
                    </li>
                    <li class=" divide">|</li>
                <?php else: ?>   
