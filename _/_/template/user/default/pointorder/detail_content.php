		...
		<div class="pull-left">
			<table>
				<tbody>
					...
					<tr>
						<td>应付金额：</td>
						<td><strong class="ui-money-income">￥<?php echo $order['total']; ?></strong>（含运费 <?php echo $order['postage']; ?> ）
						
							<?php if($order['is_point_order']) {?>
								（含积分： <span class="point_ico"></span><?php echo $order['order_pay_point']; ?> ）
							<?php }?>
...
<table class="table order-goods">
	...
	<tr data-order-id="<?php echo $order['order_id']; ?>">
		...
		<td class="tb-price">
			<?php if($order['is_point_order'] == 1) {?>
				 <span class="point_ico"></span><?php echo (int)$product['pro_price']; ?>
			<?php } else {?>
		...
		<td class="tb-total"><?php echo (int)(number_format($product['pro_num'] * $product['pro_price'], 2, '.', '')); ?></td>
