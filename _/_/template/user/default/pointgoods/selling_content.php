...
<div class="goods-list">
	<div class="js-list-filter-region clearfix ui-box" style="position: relative;">
		<div>
			<h3 class="list-title js-goods-list-title">可兑换中的礼物商品</h3>
		   ...
		   <!-- ----------- -->
			...
			<span style="background-color:#e5e5e5;display:inline-block;">
				<select id="select_level" style="height: auto;line-height: normal;width: auto;font-size: 12px;font-family: Helvetica,STHeiti,'Microsoft YaHei',Verdana,Arial,Tahoma,sans-serif;">
					<option value="selling">导出当前类别礼物商品</option>
					<option value="all">导出全部礼物商品</option>
				</select>
				<input type="button" class="ui-btn ui-btn-primary js-dump-btn" value="礼物商品导出">
			</span>
			</div>
			<?php }?>
			<!-- ----------- -->
		...
		<table class="ui-table ui-table-list" style="padding: 0px;">
			...
			<tr>
				<th class="checkbox cell-35" colspan="3" style="min-width: 332px; max-width: 332px;">
					<label class="checkbox inline"><input type="checkbox" class="js-check-all">礼物商品</label>
					<a href="javascript:;" class="orderby" data-orderby="price">积分</a>
				</th>
			...
			<?php foreach ($products as $product) { ?>
				<tr>
					...
					<td class="goods-meta">
						...
						<p>
							<span class="goods-price" goods-price="<?php echo intval($product['price']); ?>"><span class="point_ico"></span><?php echo intval($product['price']); ?></span>
							<?php if (!empty($allow_platform_drp) && !empty($product['wholesale_product_id'])) { ?>
								<i class="platform-tag" style="background-color: #07d">批发</i>
							<?php } else if (!empty($allow_store_drp) && !empty($product['is_fx'])) { ?>
					...
					<td class="text-right">
						<p>
							...
							<!--  
								<span>-</span>
								<a href="javascript:void(0);" class="js-promotion-btn" <?php if (!empty($product['supplier_id'])) { ?>data-fx="true"<?php } else { ?>data-fx="false"<?php } ?> data="<?php echo $product['product_id']; ?>" data-store-id="<?php echo $_SESSION['store']['store_id']; ?>">推广礼物商品</a>
							-->
						</p>
