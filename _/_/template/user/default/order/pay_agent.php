...
<html>
	<head>
		<meta charset="utf-8"/> 
		<title>代付的订单 - <?php echo $store_session['name'];?> | <?php if (empty($_SESSION['sync_store'])) { ?><?php echo $config['site_name'];?><?php } else { ?>微店系统<?php } ?></title>
		...
		<script type="text/javascript">var load_url="<?php dourl('load');?>", save_bak_url = "<?php dourl('save_bak'); ?>", image_path = "<?php echo TPL_URL; ?>", add_star_url = "<?php dourl('add_star'); ?>", cancel_status_url = "<?php dourl('cancel_status'); ?>", detail_json_url = "<?php dourl('detail_json'); ?>", complate_status_url = "<?php dourl('complate_status'); ?>", float_amount_url = "<?php dourl('float_amount'); ?>", page_content = "pay_agent_content", package_product_url = "<?php dourl('package_product'); ?>", create_package_url = "<?php dourl('create_package'); ?>"</script>
		<script type="text/javascript" src="<?php echo TPL_URL;?>js/order_pay_agent.js"></script>
