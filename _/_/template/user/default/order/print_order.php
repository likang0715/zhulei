...
.hr_style {color: #000000;}
.td01 {font-size: 14pt;}
.font01 {font-size: medium;}
.font02 {font-size: 11pt;}
.font03 {font-size: 11pt;text-align: center;}
.font04 {line-height: 150%;}
#divTest{position: absolute;z-index: 1000;background-color: #D5ECF1;width: 100%;height:30px;left: 0%;bottom:0px;padding-top:10px;text-align:center;}
</style>
</head>
...
<tr>
	<td height="70">
		<table cellpadding="0" cellspacing="0" class="table_style" style="width: 100%">
			<tr>
				<td class="td01"><strong>商品订单 
				&#12288;
				</strong>
				
				<font style="font-size:12px;color:#f00">
					<?php if(in_array($order['type'],array(6,7))) {?>
					(
						<?php if($order['type'] == '6') {?>
							团购订单
						<?php }?>
						...
						<?php if($order['type'] == '7') {?>
							预售订单
							<?php if($order['presale_order_id'] == $order['order_id']) {?>
								尾款支付订单
							<?php }?>
						<?php }?>
						)
