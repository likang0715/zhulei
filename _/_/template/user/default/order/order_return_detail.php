...
<div class="section" style="background-color:#FFF;">
	...
	<h2 class="section-title clearfix" style="height:30px; line-height:30px;">
		...
		<?php 
		if ($return['status'] != 6 && $return['is_fx'] == 0 && empty($return['cancel_dateline']) && $return['product_money'] <= 0 && $return['platform_point'] <= 0) {
		?>
			<span style="padding-right:10px; float:right;"><a href="javascript:" class="ui-btn ui-btn-primary js-apply-check" data-id="<?php echo $return['id'] ?>" data-order_status="<?php echo $return['order_status'] ? $return['order_status'] : $order['status'] ?>">添加审核信息</a></span>
		<?php
		}
		?>
		审核信息
		...
		<div>
			<table>
				<tbody>
					...
					if ($return['product_money'] > 0 || $return['postage_money'] > 0 || $return['platform_point'] > 0) {
					?>
						<tr>
							<td>退货金额</td>
							<td>
								￥<?php echo sprintf('%.2f', $return['product_money'] + $return['postage_money']) ?> = 退货产品金额：<?php echo $return['product_money'] ?> + 退货物流金额：<?php echo $return['postage_money'] ?>
								<?php 
								if ($return['platform_point'] > 0) {
								?>
									<br />退还<?php echo $return['platform_point'] ?>个<?php echo option('credit.platform_credit_name') ?>
								<?php 
								}
								?>
							</td>
					...
					if (empty($return['cancel_dateline']) && $return['product_money'] <= 0 && $return['platform_point'] <= 0) {
					?>
						<tr>
							<td></td>
							<td>
	...
	?>
		...
		<div class="ui-box">
			<table>
				<tbody>
					<?php 
					foreach ($relation_return_list as $relation_return) {
					?>
						<tr>
							<td>
								<a href="<?php echo url('order:order_return') ?>#detail/<?php echo $relation_return['id'] ?>" class="js-relation_return" data-id="<?php echo $relation_return['id'] ?>"><?php echo date('Y-m-d H:i', $relation_return['dateline']) ?>的退货，退货状态：<?php echo $relation_return['status_txt'] ?></a>
							</td>
						</tr>
					<?php 
					}
			...
			<div class="pull-right text-right">
				<table>
					<tbody>
						<tr>
							<td>退款产品总额：</td>
							<td class="js-profit" data-profit="<?php echo $profit ?>" data-edit="<?php echo $is_edit_money ? '1' : '0' ?>" data-discount_money="<?php echo $discount_money ?>" data-name="<?php echo isset($info['name'])?$info['name']:'';?>" data-tel="<?php echo isset($info['tel'])?$info['tel']:'';?>" data-province="<?php echo isset($store_contact['province'])?$store_contact['province']:'';?>" data-city="<?php echo isset($store_contact['city'])?$store_contact['city']:'';?>" data-county="<?php echo isset($store_contact['county'])?$store_contact['county']:'';?>" data-address="<?php echo isset($store_contact['address'])?$store_contact['address']:'';?>">
								￥<?php echo sprintf('%.2f', $profit) ?>
								其中使用<?php echo $platform_point ?>个<?php echo option('credit.platform_credit_name')?>抵<?php echo $platform_point_money ?>元
							</td>
						</tr>
					</tbody>
				</table>
...
<div class="clearfix section-final" style="background-color:#FFF;">
	<div class="pull-right text-right">
		<table>
			...
			?>
				<tr>
					<td>使用<span class="js-platform_point"><?php echo $order['cash_point'] ?></span>个<?php echo option('credit.platform_credit_name') ?>抵扣:</td>
					<td>
			...
			<tr>
				<td>应收款：</td>
				<td><span class="ui-money-income">￥<span class="order-total js-discount_money" data-product_discount_money="<?php echo $profit * (10 - $return_discount) / 10 ?>" data-platform_point="<?php echo $platform_point ?>" data-platform_point_money="<?php echo $platform_point_money ?>"><?php echo $order['total']; ?></span></span></td>
			</tr>
			</tbody>
		</table>
	</div>
