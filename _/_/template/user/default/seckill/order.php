	...
	<table class="ui-table ui-table-list" style="padding:0px;" >
		<thead class="js-list-header-region tableFloatingHeaderOriginal">
			<tr>
				...
				<th class="cell-25">售后</th>
				<th class="cell-15">买家/收货</th>
				<th class="cell-25">下单时间/类型</th>
		...
		<tbody class="js-list-body-region">
			...
			?>
				<tr>
					...
					<td>
						<?php 
						if ($order['return_quantity']) {
						?>
							<a href="javascript:" class="js-return_order" data-order_no="<?php echo option('config.orderid_prefix') . $order['order_no'] ?>" data-pigcms_id="<?php echo $order['return_product_id'] ?>">查看退货</a>
						<?php 
						}
						?>
					</td>
					...
					<td>
						...
						<br />
						<?php 
						if ($order['data_type'] == 1) {
							echo '<span style="color: green">最优开团</span>';
						} else {
							echo '<span style="color: red">人缘开团</span>';
						}
						?>
					</td>
					<td>
						...
						<?php 
						if (empty($order['return_quantity']) && $tuan['tuan_config_id'] > 0 && $order['status'] < 3) {
						?>
							<p>
						...
						<?php 
						} else if (empty($order['return_quantity']) && $tuan['tuan_config_id'] > 0 && $order['status'] == 3 && $order['sent_time'] + $config_order_complete_date * 24 * 3600 >= time()) {
						?>
							<p>
						...
						if ($codpay_status && empty($order['return_quantity']) && $tuan['tuan_config_id'] > 0 && (($order['status'] == 7 && ($order['delivery_time'] + $config_order_return_date * 24 * 3600 < time() || $order['sent_time'] + $config_order_complete_date * 24 * 3600 < time())) || ($order['status'] == 3 && $order['sent_time'] + $config_order_complete_date * 24 * 3600 < time()))) {
						?>
							<p>
			...
			?>
				<thead class="js-list-header-region tableFloatingHeaderOriginal">
					<tr>
						<td colspan="8">
							<div class="pagenavi js-order_list_page">
...
?>
<div class="js-tuan_id" data-tuan_id="<?php echo $tuan_id ?>"></div>
