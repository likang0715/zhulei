...
$(function(){
	...
	$('.js-batch-save').live('click',function(){
		...
		if(js_batch_type == 'price'){
			...
			$("input[name='sku_price']").each(function(i){
				var sale_max_price = parseFloat($("input[name='sale_max_price']").val());
				var sale_min_price = parseFloat($("input[name='sale_min_price']").val());
				var wholesale_product_id = $("input[name='wholesale_product_id']").val();
	...
	$('.js-btn-edit').live('click', function(){
		...
		if (!is_edit_sku) {
			if ($('.table-sku-stock > tbody > .sku').length > 0) {
				$('.table-sku-stock > tbody > .sku').each(function(i){
					...
					skus[i] = {'sku_id': sku_id, 'price': price, 'quantity': quantity, 'weight' : weight, 'code': code, 'properties': properties};
				});
			}
		} else {
			$('.table-sku-stock > tbody > .sku').each(function(i){
				...
				skus[i] = {"sku_id" : sku_id, "properties" : props_str.substring(0, props_str.length - 1), "price" : sku_price, "quantity" : sku_quantity, "weight" : sku_weight, "code" : sku_code};
			});
		}
		...
		$.post(save_url, {'product_id': product_id, 'category_id': category_id,'is_fx': is_fx, 'is_wholesale': is_wholesale,'buy_way': buy_way, 'buy_url': buy_url, 'quantity': quantity, 'show_stock': show_stock, 'code': code, 'name': name, 'price': price, 'original_price': original_price,'after_subscribe_price': after_subscribe_price,'images': images, 'postage_type': postage_type, 'postage': postage, 'postage_tpl_id': postage_tpl_id, 'skus': skus, 'buyer_quota': buyer_quota, 'sold_time': sold_time, 'discount': discount, 'invoice': invoice, 'warranty': warranty, 'status': status, 'intro': intro, 'info': info, 'preview': preview,'sys_fields': sys_fields,  'fields': fields, 'group_ids':group_ids, 'referer': referer, 'weight' : weight, 'is_recommend' :is_recommend, 'recommend_title' : recommend_title, is_edit_sku : is_edit_sku, 'custom':customField.checkEvent(), 'send_other' : send_other, 'send_other_postage' : send_other_postage, 'return_page' : return_page, 'return_keyword' : return_keyword, 'return_p' : return_p, 'return_group_id' : return_group_id,'give_points':give_points,'product_degree_discount_id':product_degree_discount_id,'product_degree_discount_name':product_degree_discount_name,'product_degree_discount':product_degree_discount,'check_give_points':check_give_points,'check_degree_discount':check_degree_discount}, function(data) {
			if (data.err_code == 0) {
...
function create_sku(){
	...
	switch(sku_dom_count){
		...
		case 1:
			...
			html += '<thead><tr><th class="text-center">'+(sku_arr[current_i].siblings('h3').find('span.select2-chosen').html())+'</th><th class="th-price text-right">所需积分</th><th class="th-stock text-right">库存</th><th class="th-stock text-right">重量(克)</th><th class="th-code">商品编码</th><th class="text-right">销量</th></tr></thead>';
			html += '<tbody>';
		...
		case 2:
			...
			html += '<thead><tr><th class="text-center">'+(sku_arr[current_i1].siblings('h3').find('span.select2-chosen').html())+'</th><th class="text-center">'+(sku_arr[current_i2].siblings('h3').find('span.select2-chosen').html())+'</th><th class="th-price text-right">所需积分</th><th class="th-stock text-right">库存</th><th class="th-stock text-right">重量(克)</th><th class="th-code">商品编码</th><th class="text-right">销量</th></tr></thead>';
			html += '<tbody>';
			...
			break;
		case 3:
			html += '<thead><tr><th class="text-center">'+(sku_arr[0].siblings('h3').find('span.select2-chosen').html())+'</th><th class="text-center">'+(sku_arr[1].siblings('h3').find('span.select2-chosen').html())+'</th><th class="text-center">'+(sku_arr[2].siblings('h3').find('span.select2-chosen').html())+'</th><th class="th-price text-right">所需积分</th><th class="th-stock text-right">库存</th><th class="th-stock text-right">重量(克)</th><th class="th-code">商品编码</th><th class="text-right">销量</th></tr></thead>';
			html += '<tbody>';
