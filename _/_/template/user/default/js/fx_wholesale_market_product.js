...
var address_id = 0;
$(function(){
    load_page('.app__content', load_url, {page:'wholesale_market_product_content'}, '', function(){
    ...
    $('.pagenavi > a').live('click', function(e){
		...
		var category = $('.active-result').data('option-array-index');
		...
		text = $('.result-selected').text()?$('.result-selected').text():'所有分类';
        $('.app__content').load(load_url, {page: 'wholesale_market_product_content', 'p': p,'store_name': keyword, 'category_id': category_id, 'category_fid': category_fid}, function(){
    ...
    $('.active-result').live('click', function(){
        ...
        load_page('.app__content', load_url, {page: 'wholesale_market_product_content', 'category_id': category_id, 'category_fid': category_fid, 'keyword': keyword}, '', function(){
            $('.chosen-single > span').text(text);
            $('.active-result').not(index).removeClass('result-selected highlighted')
            $('.active-result').eq(index).addClass('result-selected highlighted');
			$('.js-list-search > .js-keyword').val(keyword);
    ...
    $('.market-search-btn').live('click', function(){
        var keyword = $.trim($('.js-list-search > .js-keyword').val());
		var category = $('.result-selected').data('option-array-index')?$('.result-selected').data('option-array-index'):$('.active-result').data('option-array-index');
		category = category.split('|');
        category_fid = category[0];
        category_id = category[1];
        if (category_fid == 0) {
		...
		text = $('.result-selected').text();
        load_page('.app__content', load_url, {page: 'wholesale_market_product_content', 'keyword': keyword, 'category_id': category_id, 'category_fid': category_fid}, '', function(){
            $('.chosen-single > span').text(text);
            $('.active-result').not(index).removeClass('result-selected highlighted')
            $('.active-result').eq(index).addClass('result-selected highlighted');
            $('.js-list-search > .js-keyword').val(keyword);
    ...
    $('.js-add-to-shop,.js-batch-add-to-shop').live('click', function(){
        ...
        $.post(market_url, {'type': type, 'product_ids': product_ids}, function(data){
            if (data.err_code == 0) {
