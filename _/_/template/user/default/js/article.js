...
function location_page(mark,dom){
	...
	switch(mark_arr[0]){
		case '#create':
			load_page('.app__content', load_url,{page:'creates'}, '',function(){page_create(1);});
			break;
		case "#edit":
			if(mark_arr[1]){
				load_page('.app__content', load_url,{page:'edit',aid:mark_arr[1]},'',function(){
					var editDataDom = $('#edit_data');
					defaultFieldObj = $('.js-config-region .app-field');
					defaultFieldObj.data({'cat_id':editDataDom.attr('cat-id'),'cat_name':editDataDom.attr('cat-name'),'first_sort':editDataDom.attr('first-sort'),'second_sort':editDataDom.attr('second-sort'),'cat-src':editDataDom.attr('cat-src')});
			...
			break;
		default:
			load_page('.app__content', load_url,{page:'lists'}, '');
	}
}
var keyword = '';
$(function(){
	...
	function page_create(emptyData){
		...
		defaultFieldObj = $('.js-config-region .app-field');
		if(emptyData == 1) defaultFieldObj.data({'cat_id':'0','cat_name':'','first_sort':'0','second_sort':'0','src':''});
	...
	$('.js-app-main .js-delete').live('click',function(e){
		...
		button_box(dom,e,'left','confirm','确定删除?',function(){
			var aid = dom.closest('tr').attr('cate_id');
			$.post('/user.php?c=article&a=del',{aid:aid},function(result){
	...
	$('.js-app-main .js-copy-link').live('click',function(e){
		button_box($(this),e,'left','copy',wap_cat_url+'&aid='+$(this).closest('tr').attr('cate_id'),function(){
			layer_tips(0,'复制成功');
		});
	});
	...
	$('.js-page-list a').live('click',function(e){
		if(!$(this).hasClass('active')){
			...
			load_page('.app__content', load_url,{page:'list', 'keyword': keyword, p:$(this).attr('data-page-num')}, '', function() {
    ...
    $(window).keydown(function(event){
        if (event.keyCode == 13 && $('.js-wei_page_txt').is(':focus')) {
            ...
            load_page('.app__content', load_url,{page:'lists', 'keyword': keyword}, '', function() {
