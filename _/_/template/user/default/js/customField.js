...
var customField = {
	 ...
	 */
	 list: function() {
	 	var weipage_menu;
	 	var is_adminuser = is_adminuser?is_adminuser:0;
		//赋值
		if (is_adminuser == 1) {
			weipage_menu = [{
				type: "rich_text",
					...
					if (e.type == 'mousemove') {
						if (dom.data('noFirst') == '1') {
							...
							dom.css({
								...
								'z-index': '1000',
								'top': (dom.offset().top - preview_top - 1) + 'px'
							}).data('noFirst', '1').after('<div class="app-field clearfix editing ui-sortable-placeholder" style="visibility:hidden;height:' + (dom.height()) + 'px;"></div>');
						}
					} else {
						$('body').css({
							'cursor': 'auto',
		 ...
		 */
		 clickArr['component'] = function() {
		 	...
		 	rightHtml.find('.js-add-component').click(function() {
		 		...
		 		modalDom.animate({
		 			'margin-top': ($(window).scrollTop() + $(window).height() * 0.05) + 'px'
		 		}, "slow");
		 	});
$('.js-sidebar-region').empty().html(rightHtml);
};
		 			...
		 			if (navList[num].name != '') {
		 				liHtml.find('.js-delete-link').click(function() {
		 					...
		 					navList[liHtml.data('id')] = {
		 						...
		 						'url': '',
		 						'name': '',
		 						'image': navList[liHtml.data('id')].image
		 					};
		 					domHtml.data('navList', navList);
		 				});
		 			}
		 				...
		 				if (pic_list.length > 0) {
		 					for (var i in pic_list) {
		 						...
		 						image.onload = function() {
		 							...
		 							imageDom.removeClass('add-image').addClass('modify-image').html('重新上传').before('<img src="' + pic_list[i] + '" width="118" height="118" class="thumb-image"/>');
		 							var navList = domHtml.data('navList');
		 							var firstsrc = liHtml.parents('.choices').find('li.choice .thumb-image').eq(0).attr('src');
		 							...
		 							imgtemp.src = firstsrc?firstsrc:"http://www.ediancha.com/upload//images/image_ad_demo.jpg";
		 							imgtemp.onload = function () {
		 								var realWidth = this.width;
		 								var realHeight = this.height;
		 								var newheight =parseInt((realHeight/realWidth)*320);
		 								if ( newheight > domHtml.data('cache_height')) {
		 									domHtml.data('cache_height', newheight);
		 								}
		 								navList[liHtml.data('id')].image = pic_list[i];
		 								domHtml.data('navList', navList);
		 								buildContent();
...
liHtml.find('span.delete').click(function() {
	...
	domHtml.data('cache_height', 0);
	domHtml.data('max_width', 0);
	for (var i in navList) {
		image1.src = navList[i].image;
		image1.onload = function() {
			var newheight1 =parseInt((image1.height/image1.width)*320);
			if (newheight1 > domHtml.data('cache_height')) {
				domHtml.data('cache_height', newheight1);
			}
...
var buildContent = function() {
	...
	if (getObjLength(navList) == 0) {
		...
		if (domHtml.data('type') == '0') {
			html += '<div class="custom-image-swiper"><div class="swiper-container" style="height:' + domHtml.data('cache_height') + 'px"><div class="swiper-wrapper">';
			for (var i in navList) {
				html += '<div style="height:' + domHtml.data('cache_height') + 'px" class="swiper-slide"><a href="javascript:void(0);" style="height:' + domHtml.data('cache_height') + 'px;">' + (navList[i].title != '' ? '<h3 class="title">' + navList[i].title + '</h3>' : '') + '<img src="' + navList[i].image + '"></a></div>';
			}
			html += '</div></div></div>';
...
clickArr['article'] = function() {
	...
	var defaultHtml = '<div class="custom-image-swiper"><div class="swiper-container" style="height:80px"><div class="swiper-wrapper"><img style="max-height:80px;display:block;" src="/upload/images/image_hd_demo.jpg"></div></div></div>';
			...
			if (dom.find('.control-group .activity').size() == 0) {
				...
				var obj = domHtml.data('article_data');
				var defaultHtml = '';
				var activity_list_html = '';
				var j = 0;
				var htmls = "";
				defaultHtml += "<ul style='height:132px;overflow:hidden'>";
				for (var i in obj.activity_arr) {
					j++;
					//if(j <= 3){
						if (obj.activity_arr[i]['atype'] == 8) {
						//预售
						typename = "店铺动态";
					}
					...
					activity_list_html += '<li class="sort" data-activity-id="' + i + '"><a href="javascript:"><div class="coupon-money">' + obj.activity_arr[i]['title'].toString().substr(0, 4) + '</div></a><a class="close-modal js-delete-coupon small hide" data-id="0" title="删除">×</a></li>';
				}
				defaultHtml += "</ul>";
			...
			//domHtml = dom.find('.activity');
			var activity_data = domHtml.data("article_data"); //读取保存的数据
			...
			if (activity_data.activity_arr != undefined) {
				...
				for (var i in activity_data.activity_arr) {
					rightHtmls += '<li class="sort" data-activity-id="' + i + '"><a href="javascript:"><div class="coupon-money">' + activity_data.activity_arr[i]['title'].toString().substr(0, 4) + '</div></a><a class="close-modal js-delete-article small hide" data-id="0" title="删除">×</a></li>';
				}
			}
			rightHtmls += '							<li><a href="javascript:void(0);"class="js-add-articles add-goods"><i class="icon-add"></i></a></li>';
			rightHtmls += '						</ul>';
			rightHtmls += '						<a href="javascript:;"class="control-action js-add-coupon js-add-goods"style="display:none;">添加动态</a>';
			...
			rightHtml.find(".js-delete-article").click(function(event) {
				...
				var activity_id = $(this).closest("li").data("activity-id");
				var index = $(this).closest("ul").find("li").index($(this).closest("li"));

				//移除
				domHtml.find("li").eq(index).remove();
				//左侧剩余数量
				var li_length = domHtml.find("li").length;
				if (li_length == 0 || ("undefined" === li_length)) {
					var defaultHtml = '<div class=" "><div class="custom-image-swiper"><div class="swiper-container" style="height:80px"><div class="swiper-wrapper"><img style="max-height:80px;display:block;" src="/upload/images/image_hd_demo.jpg"></div></div></div></div>';
					dom.find('.control-group .activity').html(defaultHtml);
				}
				...
				try {
					...
					delete activity_data.activity_arr[index];
					delete activity_data[index];
			...
			widget_link_hd(rightHtml.find('.js-add-articles'), 'article_module', function(result) {
				...
				if (article_data.activity_arr != undefined) {
					//第二次
					var j = 0;
					html += "<ul style='height:132px;overflow:hidden'>";
					for (var i in article_data.activity_arr) {
						j++;
						if (article_data.activity_arr[i]['atype'] == 8) {
							//店铺活动
							typename = "店铺活动";
						}
					...
					for (var i in article_data) {
						j++;
						if (article_data[i]['atype'] == 8) {
							//预售;
							typename = "店铺活动";
						}
						article_data[i]['title'] = article_data[i]['title'].toString();
						htmls = '<img style="max-height:80px;" src="/upload/images/default_shop_2.jpg">';
						html += '<li style="display: block; width: 92px; height: 130px; border: 1px solid white;border-radius: 4px; background: white; float: left; margin: 5px 4px 0 0;"><a href="javascript:;">' + htmls + '</a><h3 style="font-size:.7em;text-align:center">' + typename + '标题xx</h3></li>';
						activity_list_html += '<li class="sort" data-activity-id="' + i + '"><a href="javascript:"><div class="coupon-money">' + article_data[i]['title'].substr(0, 4) + '</div></a><a class="close-modal js-delete-article small hide" data-id="0" title="删除">×</a></li>';
					}
					html += "</ul>";
					//console.log(html)
				}
				...
				rightHtml.find(".js-delete-article").click(function(event) {
					...
					//移除
					//domHtml.find("li").eq(index).remove();
					dom.find('.activity').find("li").eq(index).remove();
					//最近的<li>移除
					$(this).closest("li").remove();

					//左侧剩余数量
					//var li_length = domHtml.find("li").length;
					var li_length = dom.find('.activity').find("li").length;
					...
					dom.find('.activity').data('article_data', activity_data);
				});
});
};
	...
	data_typename = data_typename + after_pix;
		 ...
		 */
		 clickArr['subject_display'] = function() {
		 	...
		 	var days = "星期";
		 	days_code = myDate.getDay();
		 	days_code = parseInt(days_code);
		 	if (days_code == '0') days += "日";
		 	if (days_code == '1') days += "一";
		 	if (days_code == '2') days += "二";
		 	if (days_code == '3') days += "三";
		 	if (days_code == '4') days += "四";
		 	if (days_code == '5') days += "五";
		 	if (days_code == '6') days += "六";
		 	...
		 	if (domHtml.data('update_hour') == 1) {
		 		rightHtmls += '<option selected="selected" value="1">1</option>';
		 	} else {
		 		rightHtmls += '<option value="1">1</option>';
		 	}
		 	if (domHtml.data('update_hour') == 2) {
		 		rightHtmls += '<option selected="selected" value="2">2</option>';
		 	} else {
		 		rightHtmls += '<option value="2">2</option>';
		 	}
		 	if (domHtml.data('update_hour') == 3) {
		 		rightHtmls += '<option selected="selected" value="3">3</option>';
		 	} else {
		 		rightHtmls += '<option value="3">3</option>';
		 	}
		 	if (domHtml.data('update_hour') == 4) {
		 		rightHtmls += '<option selected="selected" value="4">4</option>';
		 	} else {
		 		rightHtmls += '<option value="4">4</option>';
		 	}
		 	if (domHtml.data('update_hour') == 5) {
		 		rightHtmls += '<option selected="selected" value="5">5</option>';
		 	} else {
		 		rightHtmls += '<option value="5">5</option>';
		 	}
		 	if (domHtml.data('update_hour') == 6) {
		 		rightHtmls += '<option selected="selected" value="6">6</option>';
		 	} else {
		 		rightHtmls += '<option value="6">6</option>';
		 	}
		 	...
		 	if (domHtml.data('day') == 3) {
		 		rightHtmls += '<option selected="selected" value="3">3</option>';
		 	} else {
		 		rightHtmls += '<option value="3">3</option>';
		 	}
		 	if (domHtml.data('day') == 5) {
		 		rightHtmls += '<option selected="selected" value="5">5</option>';
		 	} else {
		 		rightHtmls += '<option value="5">5</option>';
		 	}
		 	if (domHtml.data('day') == 7) {
		 		rightHtmls += '<option selected="selected" value="7">7</option>';
		 	} else {
		 		rightHtmls += '<option value="7">7</option>';
		 	}
		 	if (domHtml.data('day') == 15) {
		 		rightHtmls += '<option selected="selected" value="15">15</option>';
		 	} else {
		 		rightHtmls += '<option value="15">15</option>';
		 	}
			...
			if (domHtml.data('day_type') != 2) {
				rightHtmls += '		<input type="radio" name="day_type" value="1" checked="checked" style="margin:0px 0px 0px 4px;"> 自然天数据';
			} else {
				rightHtmls += '		<input type="radio" name="day_type" value="1" style="margin:0px 0px 0px 4px;"> 自然天数据';
			}
			if (domHtml.data('day_type') == 2) {
				rightHtmls += '		<input type="radio" name="day_type" value="2" checked="checked" style="margin:0px 0px 0px 4px;"> 数据天数据';
			} else {
				rightHtmls += '		<input type="radio" name="day_type" value="2" style="margin:0px 0px 0px 4px;"> 数据天数据';
			}
			rightHtmls += '</span>';
			rightHtmls += '</div>';
			...
			if (domHtml.data('number') == 1) {
				rightHtmls += '<option selected="selected" value="1">1</option>';
			} else {
				rightHtmls += '<option value="1">1</option>';
			}
			if (domHtml.data('number') == 2) {
				rightHtmls += '<option selected="selected" value="2">2</option>';
			} else {
				rightHtmls += '<option value="2">2</option>';
			}
			if (domHtml.data('number') == 3) {
				rightHtmls += '<option selected="selected" value="3">3</option>';
			} else {
				rightHtmls += '<option value="3">3</option>';
			}
			if (domHtml.data('number') == 5) {
				rightHtmls += '<option selected="selected" value="5">5</option>';
			} else {
				rightHtmls += '<option value="5">5</option>';
			}
			if (domHtml.data('number') == 10) {
				rightHtmls += '<option selected="selected" value="10">10</option>';
			} else {
				rightHtmls += '<option value="10">10</option>';
			}
			...
			rightHtml.find('.js-trigger-image').click(function() {
				...
				upload_pic_box(1, true, function(pic_list) {
					if (pic_list.length > 0) {
						for (var i in pic_list) {
							//替换左侧 背景图和 背景色
							domHtml.find(".tpl-shop-header").css({
								"background-color": "#ff0011",
								"background-image": "url(" + pic_list[i] + ")"
							});
							domHtml.data("shop_head_bg_img", pic_list[i]);
							//上传 返回右侧图片
							rightHtml.find(".tpl-shop-header").css({
								"height": "90px",
								"width": "320px",
								"background-color": "#ff0011",
		...
		if (pic_list.length > 0) {
			...
			if (shop_head_bg_img_data) {
				...
				rightHtml.find('.tpl-shop-header').css({
					...
					"background-repeat": "no-repeat",
					"background-size": "320px 90px",
					"background-image": "url(" + shop_head_bg_img_data + ")"
				});
				...
				upload_pic_box(1, true, function(pic_list) {
					if (pic_list.length > 0) {
						for (var i in pic_list) {
							...
							//imageDom.removeClass('add-image').addClass('modify-image').html('重新上传').before('<img src="'+pic_list[i]+'" width="118" height="118" class="thumb-image"/>');
							//替换左侧 背景图和 背景色
							domHtml.find(".tpl-wxd-header").css({
								"background-color": "#ff0011",
								"background-image": "url(" + pic_list[i] + ")"
							});
							domHtml.data("shop_head_bg_img", pic_list[i]);
...
domHtml.html(defaultHtml);
		 ...
		 */
		 clickArr['goods_group2'] = function() {
		 	if (dom.find('.control-group .sc-goods-list').size() == 0) {
		 		domHtml = dom.find('.control-group');
		 		domHtml.html('<ul class="sc-goods-list clearfix size-2 card pic"></ul>').data({
		 			'goods': [],
		 			'size': '2',
		 			'size_type': '0',
		 			'buy_btn': '1',
		 ...
		 * 茶会
		 */
		 clickArr['events'] = function() {
		 	if (dom.find('.control-group .sc-events-list').size() == 0) {
		 		domHtml = dom.find('.control-group');
		 		domHtml.html('<ul class="sc-events-list clearfix style-base card pic"></ul>').data({
		 			'events': [],
		 			'size': '0',
		 			'size_type': '0',
		 			'show_desc': '1',
		 	...
		 	rightHtml = $('<div><div class="form-horizontal"><div class="js-meta-region" style="margin-bottom:20px;"><div><div class="control-group"><label class="control-label">选择茶会：</label><div class="controls"><ul id="SortContaint" class="module-events-list clearfix ui-sortable" name="events"></ul><ul class="module-events-lists clearfix ui-sortable" ><li class="no-selected"><a href="javascript:void(0);" class="js-add-events add-events"><i class="icon-add"></i></a></li></ul><span id="msg" style="font-size:12px;opacity: 0.6;">温馨提示：拖动茶会图片，可以更换位置</span></div></div><div class="control-group"><label class="control-label">列表样式：</label><div class="controls"><label class="radio inline"><input type="radio" name="size" value="0" />样式1</label></div></div><div class="control-group"></div></div></div></div></div>');

		 	var event_data = domHtml.data('events');
		 	var html = '';
		 	for (var i in event_data) {
		 		var item = event_data[i];
		 		html += '<li class="sort SortItem"><a href="' + item.url + '" target="_blank"><img price="' + item.price + '" src="' + item.image + '" alt="' + item.title + '" title="' + item.title + '" width="84" height="48"></a><a class="close-modal js-delete-events small hide" data-id="' + i + '" data-product_id="' + item.id + '" title="删除">×</a></li>';

		 	}
		 	rightHtml.find('.module-events-list').prepend(html);
		 	rightHtml.find('.module-events-list .sort .js-delete-events').click(function() {
		 		$(this).closest('.sort').remove();
		 		var event_data = domHtml.data('events');
		 		delete event_data[$(this).data('id')];
		 		domHtml.data('events', event_data);
		 	});
			///////////////////////////////////////////////////////

			if (rightHtml.find("#SortContaint li").length > 0) {
				rightHtml.find('#SortContaint').sortable().bind('sortupdate', function(j) {
					rightHtml.find("#msg").html('温馨提示：<font style="color:#f00">茶会位置更新成功,保存后生效</font>').delay(1000).fadeIn(0, function() {
						rightHtml.find("#msg").html('温馨提示：拖动茶会图片，可以更换位置');
					});
					var event_data = domHtml.data('events');
					...
					rightHtml.find("#SortContaint li").each(function(i, item) {
						//if(i == rightHtml.find("#SortContaint li").length - 1) return false;
						var ids = $(item).find(".js-delete-events").data("id");
						var tmp = {};
						tmp.id = $(item).find('.close-modal').data('product_id');
						tmp.image = $(item).find("img").attr("src");
						tmp.price = $(item).find("img").attr("price");
					...
					domHtml.data('events', event_data2);
				});
			}
			...
			widget_link_box(rightHtml.find('.js-add-events'), 'chahui', function(result) {
				var event_data = domHtml.data('events');
				if (event_data) {
					$.merge(event_data, result);
				} else {
					event_data = result;
				}
				domHtml.data('events', event_data);
				rightHtml.find('.module-events-list .sort').remove();
				var html = '';
				for (var i in event_data) {
					var item = event_data[i];
					html += '<li class="sort"><a href="' + item.url + '" target="_blank"><img price="' + item.price + '" src="' + item.image + '" alt="' + item.title + '" title="' + item.title + '" width="84" height="48"></a><a class="close-modal js-delete-events small hide" data-id="' + i + '"  data-product_id="' + item.id + '"  title="删除">×</a></li>';
				}
				rightHtml.find('.module-events-list').prepend(html);
				rightHtml.find('.module-events-list .sort .js-delete-events').click(function() {
					$(this).closest('.sort').remove();
				...
				if (rightHtml.find("#SortContaint li").length > 0) {
					rightHtml.find('#SortContaint').sortable().bind('sortupdate', function(j) {
						rightHtml.find("#msg").html('<font style="font-size:11px;font-weight:700;color:#f00">茶会位置更新成功,保存后生效！</font>').fadeIn(200).delay(1000).fadeOut(200);
						var event_data = domHtml.data('events');
			...
			rightHtml.find('input[name="size"]').change(function() {
				...
				switch ($(this).val()) {
					case '0':
					$(this).closest('.control-group').next().replaceWith('<div class="control-group"><div class="controls"><div class="controls-card"><div class="controls-card-tab"><label class="radio inline"><input type="radio" name="size_type" value="0"/>卡片样式</label><label class="radio inline"><input type="radio" name="size_type" value="2"/>极简样式</label></div><div class="controls-card-item"><label class="checkbox inline"><input type="checkbox" name="show_desc" value="1"/>显示茶会描述</label></div><div class="controls-card-item"><label class="checkbox inline"><input type="checkbox" name="price" value="1"/>显示日期和价格</label></div></div></div></div>');
					if (domHtml.data('size_type') != '0' && domHtml.data('size_type') != '2') {
						domHtml.data('size_type', '0');
					}
					break;
					...
					break;
					case '2':
					$(this).closest('.control-group').next().replaceWith('<div class="control-group"><div class="controls"><div class="controls-card"><div class="controls-card-tab"><label class="radio inline"><input type="radio" name="size_type" value="0"/>卡片样式</label><label class="radio inline"><input type="radio" name="size_type" value="2"/>极简样式</label></div><div class="controls-card-item"><label class="checkbox inline"><input type="checkbox" name="show_desc" value="1"/>显示茶会名 (小图不显示名称)</label></div><div class="controls-card-item"><label class="checkbox inline"><input type="checkbox" name="price" value="1"/>显示日期和价格</label></div></div></div></div>');
					if (domHtml.data('size_type') != '0' && domHtml.data('size_type') != '2') {
						domHtml.data('size_type', '0');
					}
					break;
					case '3':
					$(this).closest('.control-group').next().replaceWith('<div class="control-group"><div class="controls"><div class="controls-card"><div class="controls-card-tab"><label class="radio inline"><input type="radio" name="size_type" value="0"/>卡片样式</label><label class="radio inline"><input type="radio" name="size_type" value="2"/>极简样式</label></div></div></div></div>');
					if (domHtml.data('size_type') != '0' && domHtml.data('size_type') != '2') {
						domHtml.data('size_type', '0');
					}
					break;
				...
				rightHtml.find('input[name="size_type"][value="' + domHtml.data('size_type') + '"]').prop('checked', true);
				rightHtml.find('input[name="show_desc"]').prop('checked', (domHtml.data('show_desc') == '1' ? true : false));
				rightHtml.find('input[name="price"]').prop('checked', (domHtml.data('price') == '1' ? true : false));
				...
				rightHtml.find('input[name="show_desc"]').change(function() {
					domHtml.data('show_desc', $(this).prop('checked') ? '1' : '0');
					changeEventsStyle();
				});
				rightHtml.find('input[name="price"]').change(function() {
					domHtml.data('price', $(this).prop('checked') ? '1' : '0');
			 ...
			 *
			 */
			 function changeEventsStyle() {
			 	var html = '';
			 	switch (domHtml.data('size')) {
			 		case '0':
			 		switch (domHtml.data('size_type')) {
							case '0': //卡片样式
							html = '<ul class="sc-events-list clearfix style-base card pic"><li class="events-card normal-style card"><a href="javascript:void(0);" class="link js-events clearfix"><div class="photo-block"><img class="events-photo js-events-lazy" src="/upload/images/first_demo_events.jpg"/></div>';
							html += '<div class="info clearfix ' + (domHtml.data('show_desc') == '0' ? 'info-no-desc' : '') + ' ' + (domHtml.data('price') == '1' ? 'info-price' : 'info-no-price') + '"><p class="events-title">此处显示茶会名称</p><p class="events-desc">此处显示茶会描述</p><p class="events-main"><em class="events-time events-time-icon">11/26&nbsp;&nbsp;9:00</em><em class="events-price">免费</em></p></div>';
							html += '</a></li><li class="events-card normal-style card"><a href="javascript:void(0);" class="link js-events clearfix"><div class="photo-block"><img class="events-photo js-events-lazy" src="/upload/images/two_demo_events.jpg"/></div>';
							html += '<div class="info clearfix ' + (domHtml.data('show_desc') == '0' ? 'info-no-desc' : '') + ' ' + (domHtml.data('price') == '1' ? 'info-price' : 'info-no-price') + '"><p class="events-title">此处显示茶会名称</p><p class="events-desc">此处显示茶会描述</p><p class="events-main"><em class="events-time events-time-icon">11/26&nbsp;&nbsp;9:00</em><em class="events-price">100元/人</em></p></div>';
							html += '</a></li><li class="events-card normal-style card"><a href="javascript:void(0);" class="link js-events clearfix"><div class="photo-block"><img class="events-photo js-events-lazy" src="/upload/images/n_demo_events.jpg"/></div>';
							html += '<div class="info clearfix ' + (domHtml.data('show_desc') == '0' ? 'info-no-desc' : '') + ' ' + (domHtml.data('price') == '1' ? 'info-price' : 'info-no-price') + '"><p class="events-title">此处显示茶会名称</p><p class="events-desc">此处显示茶会描述</p><p class="events-main"><em class="events-time events-time-icon">11/26&nbsp;&nbsp;9:00</em><em class="events-price">免费</em></p></div>';
							html += '</a></li></ul>';
							break;
							case '2':
							html = '<ul class="sc-events-list clearfix style-base normal pic"><li class="events-card normal-style normal"><a href="javascript:void(0);" class="link js-events clearfix"><div class="photo-block"><img class="events-photo js-events-lazy" src="/upload/images/first_demo_events.jpg"/></div>';
							html += '<div class="info clearfix ' + (domHtml.data('show_desc') == '0' ? 'info-no-desc' : '') + ' ' + (domHtml.data('price') == '1' ? 'info-price' : 'info-no-price') + '"><p class="events-title">此处显示茶会名称</p><p class="events-desc">此处显示茶会描述</p><p class="events-main"><em class="events-time events-time-icon">11/26&nbsp;&nbsp;9:00</em><em class="events-price">免费</em></p></div>';
							html += '</a></li><li class="events-card normal-style normal"><a href="javascript:void(0);" class="link js-events clearfix"><div class="photo-block"><img class="events-photo js-events-lazy" src="/upload/images/two_demo_events.jpg"/></div>';
							html += '<div class="info clearfix ' + (domHtml.data('show_desc') == '0' ? 'info-no-desc' : '') + ' ' + (domHtml.data('price') == '1' ? 'info-price' : 'info-no-price') + '"><p class="events-title">此处显示茶会名称</p><p class="events-desc">此处显示茶会描述</p><p class="events-main"><em class="events-time events-time-icon">11/26&nbsp;&nbsp;9:00</em><em class="events-price">100元/人</em></p></div>';
							html += '</a></li><li class="events-card normal-style normal"><a href="javascript:void(0);" class="link js-events clearfix"><div class="photo-block"><img class="events-photo js-events-lazy" src="/upload/images/n_demo_events.jpg"/></div>';
							html += '<div class="info clearfix ' + (domHtml.data('show_desc') == '0' ? 'info-no-desc' : '') + ' ' + (domHtml.data('price') == '1' ? 'info-price' : 'info-no-price') + '"><p class="events-title">此处显示茶会名称</p><p class="events-desc">此处显示茶会描述</p><p class="events-main"><em class="events-time events-time-icon">11/26&nbsp;&nbsp;9:00</em><em class="events-price">免费</em></p></div>';
							html += '</a></li></ul>';
							break;
						}
						...
						switch (domHtml.data('size_type')) {
							case '0':
							html = '<ul class="sc-events-list clearfix style-float card pic"><li class="events-card small-pic card"><a href="javascript:void(0);" class="link js-events clearfix"><div class="photo-block"><img class="events-photo js-events-lazy" src="/upload/images/first_demo_events.jpg"/></div>';
							if (domHtml.data('show_desc') == '1' || domHtml.data('price') == '1') {
								html += '<div class="info clearfix ' + (domHtml.data('show_desc') == '0' ? 'info-no-desc' : '') + ' ' + (domHtml.data('price') == '1' ? 'info-price' : 'info-no-price') + '"><p class="events-title">此处显示茶会名称</p><p class="events-desc">此处显示茶会描述</p><p class="events-main"><em class="events-time events-time-icon">11/26&nbsp;&nbsp;9:00</em><em class="events-price">免费</em></p></div>';
							}
							html += '</a></li><li class="events-card small-pic card"><a href="javascript:void(0);" class="link js-events clearfix"><div class="photo-block"><img class="events-photo js-events-lazy" src="/upload/images/two_demo_events.jpg"/></div>';
							if (domHtml.data('show_desc') == '1' || domHtml.data('price') == '1') {
								html += '<div class="info clearfix ' + (domHtml.data('show_desc') == '0' ? 'info-no-desc' : '') + '"><p class="events-title">此处显示茶会名称</p><p class="events-desc">此处显示茶会描述</p><p class="events-main"><em class="events-time events-time-icon">11/26&nbsp;&nbsp;9:00</em><em class="events-price">100元/人</em></p></div>';
							}
							html += '</a></li><li class="events-card small-pic card"><a href="javascript:void(0);" class="link js-events clearfix"><div class="photo-block"><img class="events-photo js-events-lazy" src="/upload/images/third_demo_events.jpg"/></div>';
							if (domHtml.data('show_desc') == '1' || domHtml.data('price') == '1') {
								html += '<div class="info clearfix ' + (domHtml.data('show_desc') == '0' ? 'info-no-desc' : '') + '"><p class="events-title">此处显示茶会名称</p><p class="events-desc">此处显示茶会描述</p><p class="events-main"><em class="events-time events-time-icon">11/26&nbsp;&nbsp;9:00</em><em class="events-price">免费</em></p></div>';
							}
							html += '</a></li><li class="events-card small-pic card"><a href="javascript:void(0);" class="link js-events clearfix"><div class="photo-block"><img class="events-photo js-events-lazy" src="/upload/images/n_demo_events.jpg"/></div>';
							if (domHtml.data('show_desc') == '1' || domHtml.data('price') == '1') {
								html += '<div class="info clearfix ' + (domHtml.data('show_desc') == '0' ? 'info-no-desc' : '') + '"><p class="events-title">此处显示茶会名称</p><p class="events-desc">此处显示茶会描述</p><p class="events-main"><em class="events-time events-time-icon">11/26&nbsp;&nbsp;9:00</em><em class="events-price">免费</em></p></div>';
							}
							html += '</a></li></ul>';
							break;
							case '1':
							html = '<ul class="sc-events-list clearfix style-float waterfall pic">';
							html += '<li class="sc-waterfall-half clearfix">';
							html += '<ul class="clearfix">';
							html += '<li class="events-card events-list small-pic waterfall"><a href="javascript:void(0);" class="link js-events clearfix"><div class="photo-block"><img class="events-photo js-events-lazy" src="/upload/images/first_demo_events.jpg" style="height:145px;"/></div>';
							if (domHtml.data('show_desc') == '1' || domHtml.data('price') == '1') {
								html += '<div class="info clearfix ' + (domHtml.data('show_desc') == '0' ? 'info-no-desc' : '') + ' ' + (domHtml.data('price') == '1' ? 'info-price' : 'info-no-price') + '"><p class="events-title">此处显示茶会名称</p><p class="events-desc">此处显示茶会描述</p><p class="events-main"><em class="events-time events-time-icon">11/26&nbsp;&nbsp;9:00</em><em class="events-price">免费</em></p></div>';
							}
							html += '</a></li>';
							html += '<li class="events-card events-list small-pic waterfall"><a href="javascript:void(0);" class="link js-events clearfix"><div class="photo-block"><img class="events-photo js-events-lazy" src="/upload/images/third_demo_events.jpg" style="height:205px;"/></div>';
							if (domHtml.data('show_desc') == '1' || domHtml.data('price') == '1') {
								html += '<div class="info clearfix ' + (domHtml.data('show_desc') == '0' ? 'info-no-desc' : '') + ' ' + (domHtml.data('price') == '1' ? 'info-price' : 'info-no-price') + '"><p class="events-title">此处显示茶会名称</p><p class="events-desc">此处显示茶会描述</p><p class="events-main"><em class="events-time events-time-icon">11/26&nbsp;&nbsp;9:00</em><em class="events-price">免费</em></p></div>';
							}
							html += '</a></li>';
							...
							html += '<li class="sc-waterfall-half clearfix">';
							html += '<ul class="clearfix">';
							html += '<li class="events-card events-list small-pic waterfall"><a href="javascript:void(0);" class="link js-events clearfix"><div class="photo-block"><img class="events-photo js-events-lazy" src="/upload/images/two_demo_events.jpg" style="height:155px;"/></div>';
							if (domHtml.data('show_desc') == '1' || domHtml.data('price') == '1') {
								html += '<div class="info clearfix ' + (domHtml.data('show_desc') == '0' ? 'info-no-desc' : '') + ' ' + (domHtml.data('price') == '1' ? 'info-price' : 'info-no-price') + '"><p class="events-title">此处显示茶会名称</p><p class="events-desc">此处显示茶会描述</p><p class="events-main"><em class="events-time events-time-icon">11/26&nbsp;&nbsp;9:00</em><em class="events-price">100元/人</em></p></div>';
							}
							html += '</a></li>';
							html += '<li class="events-card events-list small-pic waterfall"><a href="javascript:void(0);" class="link js-events clearfix"><div class="photo-block"><img class="events-photo js-events-lazy" src="/upload/images/n_demo_events.jpg" style="height:175px;"/></div>';
							if (domHtml.data('show_desc') == '1' || domHtml.data('price') == '1') {
								html += '<div class="info clearfix ' + (domHtml.data('show_desc') == '0' ? 'info-no-desc' : '') + ' ' + (domHtml.data('price') == '1' ? 'info-price' : 'info-no-price') + '"><p class="events-title">此处显示茶会名称</p><p class="events-desc">此处显示茶会描述</p><p class="events-main"><em class="events-time events-time-icon">11/26&nbsp;&nbsp;9:00</em><em class="events-price">免费</em></p></div>';
							}
							html += '</a></li>';
							...
							break;
							case '2':
							html = '<ul class="sc-events-list clearfix style-float normal pic"><li class="events-card small-pic normal"><a href="javascript:void(0);" class="link js-events clearfix"><div class="photo-block"><img class="events-photo js-events-lazy" src="/upload/images/first_demo_events.jpg"/></div>';
							if (domHtml.data('price') == '1') {
								html += '<div class="info clearfix ' + (domHtml.data('show_desc') == '0' ? 'info-no-desc' : '') + ' ' + (domHtml.data('price') == '1' ? 'info-price' : 'info-no-price') + '"><p class="events-title">此处显示茶会名称</p><p class="events-desc">此处显示茶会描述</p><p class="events-main"><em class="events-time events-time-icon">11/26&nbsp;&nbsp;9:00</em><em class="events-price">免费</em></p></div>';
							}
							html += '</a></li><li class="events-card small-pic normal"><a href="javascript:void(0);" class="link js-events clearfix"><div class="photo-block"><img class="events-photo js-events-lazy" src="/upload/images/two_demo_events.jpg"/></div>';
							if (domHtml.data('price') == '1') {
								html += '<div class="info clearfix ' + (domHtml.data('show_desc') == '0' ? 'info-no-desc' : '') + '"><p class="events-title">此处显示茶会名称</p><p class="events-desc">此处显示茶会描述</p><p class="events-main"><em class="events-time events-time-icon">11/26&nbsp;&nbsp;9:00</em><em class="events-price">100元/人</em></p></div>';
							}
							html += '</a></li><li class="events-card small-pic normal"><a href="javascript:void(0);" class="link js-events clearfix"><div class="photo-block"><img class="events-photo js-events-lazy" src="/upload/images/third_demo_events.jpg"/></div>';
							if (domHtml.data('price') == '1') {
								html += '<div class="info clearfix ' + (domHtml.data('show_desc') == '0' ? 'info-no-desc' : '') + '"><p class="events-title">此处显示茶会名称</p><p class="events-desc">此处显示茶会描述</p><p class="events-main"><em class="events-time events-time-icon">11/26&nbsp;&nbsp;9:00</em><em class="events-price">免费</em></p></div>';
							}
							html += '</a></li><li class="events-card small-pic normal"><a href="javascript:void(0);" class="link js-events clearfix"><div class="photo-block"><img class="events-photo js-events-lazy" src="/upload/images/n_demo_events.jpg"/></div>';
							if (domHtml.data('price') == '1') {
								html += '<div class="info clearfix ' + (domHtml.data('show_desc') == '0' ? 'info-no-desc' : '') + '"><p class="events-title">此处显示茶会名称</p><p class="events-desc">此处显示茶会描述</p><p class="events-main"><em class="events-time events-time-icon">11/26&nbsp;&nbsp;9:00</em><em class="events-price">免费</em></p></div>';
							}
							html += '</a></li></ul>';
						...
						switch (domHtml.data('size_type')) {
							case '0':
							html = '<ul class="sc-events-list clearfix style-round card list"><li class="events-card card"><a href="javascript:void(0);" class="link js-events clearfix"><div class="photo-block"><img class="events-photo js-events-lazy" src="/upload/images/first_demo_events.jpg"/></div><div class="info"><p class="events-title">此处显示茶会名称</p><p class="events-main"><em class="events-time events-time-icon">11/26&nbsp;&nbsp;9:00</em><em class="events-price">免费</em></p>';
							html += '</div>';
							html += '</a></li><li class="events-card card"><a href="javascript:void(0);" class="link js-events clearfix"><div class="photo-block"><img class="events-photo js-events-lazy" src="/upload/images/two_demo_events.jpg"/></div><div class="info"><p class="events-title">此处显示茶会名称</p><p class="events-desc">此处显示茶会描述</p><p class="events-main"><em class="events-time events-time-icon">11/26&nbsp;&nbsp;9:00</em><em class="events-price">100元/人</em></p>';
							html += '</div>';
							html += '</a></li><li class="events-card card"><a href="javascript:void(0);" class="link js-events clearfix"><div class="photo-block"><img class="events-photo js-events-lazy" src="/upload/images/n_demo_events.jpg"/></div><div class="info"><p class="events-title">此处显示茶会名称</p><p class="events-desc">此处显示茶会描述</p><p class="events-main"><em class="events-time events-time-icon">11/26&nbsp;&nbsp;9:00</em><em class="events-price">免费</em></p>';
							html += '</div>';
							html += '</a></li></ul>';
							break;
							case '2':
							html = '<ul class="sc-events-list clearfix style-round normal list"><li class="events-card normal"><a href="javascript:void(0);" class="link js-events clearfix"><div class="photo-block"><img class="events-photo js-events-lazy" src="/upload/images/first_demo_events.jpg"/></div><div class="info"><p class="events-title">此处显示茶会名称</p><p class="events-desc">此处显示茶会描述</p><p class="events-main"><em class="events-time events-time-icon">11/26&nbsp;&nbsp;9:00</em><em class="events-price">免费</em></p>';
							html += '</div>';
							html += '</a></li><li class="events-card normal"><a href="javascript:void(0);" class="link js-events clearfix"><div class="photo-block"><img class="events-photo js-events-lazy" src="/upload/images/two_demo_events.jpg"/></div><div class="info"><p class="events-title">此处显示茶会名称</p><p class="events-desc">此处显示茶会描述</p><p class="events-main"><em class="events-time events-time-icon">11/26&nbsp;&nbsp;9:00</em><em class="events-price">100元/人</em></p>';
							html += '</div>';
							html += '</a></li><li class="events-card normal"><a href="javascript:void(0);" class="link js-events clearfix"><div class="photo-block"><img class="events-photo js-events-lazy" src="/upload/images/n_demo_events.jpg"/></div><div class="info"><p class="events-title">此处显示茶会名称</p><p class="events-desc">此处显示茶会描述</p><p class="events-main"><em class="events-time events-time-icon">11/26&nbsp;&nbsp;9:00</em><em class="events-price">免费</em></p>';
							html += '</div>';
							html += '</a></li></ul>';
					...
					domHtml.find('.sc-events-list').replaceWith(html);
				}
...
/**
		 * 包厢
		 */
		 clickArr['tables'] = function() {
		 	if (dom.find('.control-group .sc-tables-list').size() == 0) {
		 		domHtml = dom.find('.control-group');
		 		domHtml.html('<ul class="sc-tables-list clearfix style-base card pic"></ul>').data({
		 			'tables': [],
		 			'size': '0',
		 			'size_type': '0'
		 		});
		 	...
		 	rightHtml = $('<div><div class="form-horizontal"><div class="js-meta-region" style="margin-bottom:20px;"><div><div class="control-group"><label class="control-label">选择包厢：</label><div class="controls"><ul id="SortContaint" class="module-tables-list clearfix ui-sortable" name="tables"></ul><ul class="module-tables-lists clearfix ui-sortable" ><li class="no-selected"><a href="javascript:void(0);" class="js-add-tables add-tables"><i class="icon-add"></i></a></li></ul><span id="msg" style="font-size:12px;opacity: 0.6;">温馨提示：拖动包厢图片，可以更换位置</span></div></div><div class="control-group"><label class="control-label">列表样式：</label><div class="controls"><label class="radio inline"><input type="radio" name="size" value="0"/>样式1</label></div></div><div class="control-group"></div></div></div></div></div>');
		 	var event_data = domHtml.data('tables');
		 	var html = '';
		 	for (var i in event_data) {
		 		var item = event_data[i];
		 		html += '<li class="sort SortItem"><a href="' + item.url + '" target="_blank"><img price="' + item.price + '" src="' + item.image + '" alt="' + item.title + '" title="' + item.title + '" width="84" height="48"></a><a class="close-modal js-delete-tables small hide" data-id="' + i + '" data-product_id="' + item.id + '" title="删除">×</a></li>';
		 	}
		 	rightHtml.find('.module-tables-list').prepend(html);
		 	rightHtml.find('.module-tables-list .sort .js-delete-tables').click(function() {
		 		$(this).closest('.sort').remove();
		 		var event_data = domHtml.data('tables');
		 		delete event_data[$(this).data('id')];
		 		domHtml.data('tables', event_data);
		 	});
			///////////////////////////////////////////////////////

			if (rightHtml.find("#SortContaint li").length > 0) {
				rightHtml.find('#SortContaint').sortable().bind('sortupdate', function(j) {
					rightHtml.find("#msg").html('温馨提示：<font style="color:#f00">包厢位置更新成功,保存后生效</font>').delay(1000).fadeIn(0, function() {
						rightHtml.find("#msg").html('温馨提示：拖动包厢图片，可以更换位置');
					});
					var event_data = domHtml.data('tables');
					...
					rightHtml.find("#SortContaint li").each(function(i, item) {
						//if(i == rightHtml.find("#SortContaint li").length - 1) return false;
						var ids = $(item).find(".js-delete-tables").data("id");
						var tmp = {};
						tmp.id = $(item).find('.close-modal').data('product_id');
						tmp.image = $(item).find("img").attr("src");
						tmp.price = $(item).find("img").attr("price");
					...
					domHtml.data('tables', event_data2);
				});
			}
			...
			widget_link_box(rightHtml.find('.js-add-tables'), 'baoxiang', function(result) {
				var event_data = domHtml.data('tables');
				if (event_data) {
					$.merge(event_data, result);
				} else {
				...
				domHtml.data('tables', event_data);
				rightHtml.find('.module-tables-list .sort').remove();
				var html = '';
				for (var i in event_data) {
					var item = event_data[i];
					html += '<li class="sort"><a href="' + item.url + '" target="_blank"><img price="' + item.price + '" src="' + item.image + '" alt="' + item.title + '" title="' + item.title + '" width="84" height="48"></a><a class="close-modal js-delete-tables small hide" data-id="' + i + '"  data-product_id="' + item.id + '"  title="删除">×</a></li>';
				}
				rightHtml.find('.module-tables-list').prepend(html);
				rightHtml.find('.module-tables-list .sort .js-delete-tables').click(function() {
					$(this).closest('.sort').remove();
				...
				if (rightHtml.find("#SortContaint li").length > 0) {
					rightHtml.find('#SortContaint').sortable().bind('sortupdate', function(j) {
						rightHtml.find("#msg").html('<font style="font-size:11px;font-weight:700;color:#f00">包厢位置更新成功,保存后生效！</font>').fadeIn(200).delay(1000).fadeOut(200);
						var event_data = domHtml.data('tables');
			...
			rightHtml.find('input[name="size"]').change(function() {
				...
				switch ($(this).val()) {
					case '0':
					$(this).closest('.control-group').next().replaceWith('<div class="control-group tables-control-group"><div class="controls"><div class="controls-card"><div class="controls-card-tab"><label class="radio inline"><input type="radio" name="size_type" value="0"/>显示描述</label><label class="radio inline"><input type="radio" name="size_type" value="1"/>显示价格</label><label class="radio inline"><input type="radio" name="size_type" value="2"/>显示优惠政策</label></div></div></div></div>');
					break;
				}
				rightHtml.find('input[name="size_type"][value="' + domHtml.data('size_type') + '"]').prop('checked', true);
				rightHtml.find('input[name="show_desc"]').prop('checked', (domHtml.data('show_desc') == '1' ? true : false));
			 ...
			 *
			 */
			 function changetablesStyle() {
			 	var html = '';
			 	switch (domHtml.data('size')) {
			 		case '0':
			 		switch (domHtml.data('size_type')) {
							case '0': //显示描述
							html = '<ul class="sc-tables-list clearfix style-base normal pic"><li class="tables-card normal-style normal"><a href="javascript:void(0);" class="link js-tables clearfix"><div class="photo-block"><img class="tables-photo js-tables-lazy" src="/upload/images/first_demo_tables.jpg"/></div>';
							html += '<div class="info clearfix info-desc"><p class="tables-contain"><i class="tables-contain-before"></i><span class="tables-contain-main">6人间</span><i class="tables-contain-after"></i></p><p class="tables-title">此处显示包厢名称</p><p class="tables-desc">此处显示包厢描述</p></div><div class="tables_btn"><span class="tables_order_btn">我要预订</span></div>';
							html += '</a></li><li class="tables-card normal-style normal"><a href="javascript:void(0);" class="link js-tables clearfix"><div class="photo-block"><img class="tables-photo js-tables-lazy" src="/upload/images/two_demo_tables.jpg"/></div>';
							html += '<div class="info clearfix info-desc"><p class="tables-contain"><i class="tables-contain-before"></i><span class="tables-contain-main">6人间</span><i class="tables-contain-after"></i></p><p class="tables-title">此处显示包厢名称</p><p class="tables-desc">此处显示包厢描述</p></div><div class="tables_btn"><span class="tables_order_btn">我要预订</span></div>';
							html += '</a></li><li class="tables-card normal-style normal"><a href="javascript:void(0);" class="link js-tables clearfix"><div class="photo-block"><img class="tables-photo js-tables-lazy" src="/upload/images/n_demo_tables.jpg"/></div>';
							html += '<div class="info clearfix info-desc"><p class="tables-contain"><i class="tables-contain-before"></i><span class="tables-contain-main">6人间</span><i class="tables-contain-after"></i></p><p class="tables-title">此处显示包厢名称</p><p class="tables-desc">此处显示包厢描述</p></div><div class="tables_btn"><span class="tables_order_btn">我要预订</span></div>';
							html += '</a></li></ul>';
							break;
							case '1': //显示价格
							html = '<ul class="sc-tables-list clearfix style-base normal pic"><li class="tables-card normal-style normal"><a href="javascript:void(0);" class="link js-tables clearfix"><div class="photo-block"><img class="tables-photo js-tables-lazy" src="/upload/images/first_demo_tables.jpg"/></div>';
							html += '<div class="info clearfix info-price"><p class="tables-contain"><i class="tables-contain-before"></i><span class="tables-contain-main">6人间</span><i class="tables-contain-after"></i></p><p class="tables-title">此处显示包厢名称</p><p class="tables-price">88元/小时</p></div><div class="tables_btn"><span class="tables_order_btn">我要预订</span></div>';
							html += '</a></li><li class="tables-card normal-style normal"><a href="javascript:void(0);" class="link js-tables clearfix"><div class="photo-block"><img class="tables-photo js-tables-lazy" src="/upload/images/two_demo_tables.jpg"/></div>';
							html += '<div class="info clearfix info-price"><p class="tables-contain"><i class="tables-contain-before"></i><span class="tables-contain-main">6人间</span><i class="tables-contain-after"></i></p><p class="tables-title">此处显示包厢名称</p><p class="tables-price">88元/小时</p></div><div class="tables_btn"><span class="tables_order_btn">我要预订</span></div>';
							html += '</a></li><li class="tables-card normal-style normal"><a href="javascript:void(0);" class="link js-tables clearfix"><div class="photo-block"><img class="tables-photo js-tables-lazy" src="/upload/images/n_demo_tables.jpg"/></div>';
							html += '<div class="info clearfix info-price"><p class="tables-contain"><i class="tables-contain-before"></i><span class="tables-contain-main">6人间</span><i class="tables-contain-after"></i></p><p class="tables-title">此处显示包厢名称</p><p class="tables-price">88元/小时</p></div><div class="tables_btn"><span class="tables_order_btn">我要预订</span></div>';
							...
							case '2': //显示促销
							html = '<ul class="sc-tables-list clearfix style-base normal pic"><li class="tables-card normal-style normal"><a href="javascript:void(0);" class="link js-tables clearfix"><div class="photo-block"><img class="tables-photo js-tables-lazy" src="/upload/images/first_demo_tables.jpg"/></div>';
							html += '<div class="info clearfix info-sale"><p class="tables-contain"><i class="tables-contain-before"></i><span class="tables-contain-main">6人间</span><i class="tables-contain-after"></i></p><p class="tables-title">此处显示包厢名称</p><p class="tables-sale">此处显示包厢优惠政策</p></div><div class="tables_btn"><span class="tables_order_btn">我要预订</span></div>';
							html += '</a></li><li class="tables-card normal-style normal"><a href="javascript:void(0);" class="link js-tables clearfix"><div class="photo-block"><img class="tables-photo js-tables-lazy" src="/upload/images/two_demo_tables.jpg"/></div>';
							html += '<div class="info clearfix info-sale"><p class="tables-contain"><i class="tables-contain-before"></i><span class="tables-contain-main">6人间</span><i class="tables-contain-after"></i></p><p class="tables-title">此处显示包厢名称</p><p class="tables-sale">此处显示包厢优惠政策</p></div><div class="tables_btn"><span class="tables_order_btn">我要预订</span></div>';
							html += '</a></li><li class="tables-card normal-style normal"><a href="javascript:void(0);" class="link js-tables clearfix"><div class="photo-block"><img class="tables-photo js-tables-lazy" src="/upload/images/n_demo_tables.jpg"/></div>';
							html += '<div class="info clearfix info-sale"><p class="tables-contain"><i class="tables-contain-before"></i><span class="tables-contain-main">6人间</span><i class="tables-contain-after"></i></p><p class="tables-title">此处显示包厢名称</p><p class="tables-sale">此处显示包厢优惠政策</p></div><div class="tables_btn"><span class="tables_order_btn">我要预订</span></div>';
					...
					domHtml.find('.sc-tables-list').replaceWith(html);
				}
				$('.js-sidebar-region').empty().html(rightHtml);
			};
		 ...
		 */
		 clickArr['new_activity_module'] = function() {
			...
			if (dom.find('.control-group .activity').size() == 0) {
				...
				for (var i in obj.activity_arr) {
					...
					//if(j <= 3){
						...
						//秒杀
						typename = "秒杀";
					} else if (obj.activity_arr[i]['atype'] == 7) {
						//秒杀
						typename = "降价";
					}
					htmls = '<img style="max-height:80px;" src="/upload/images/default_shop_2.jpg">';
			...
			if (activity_data.activity_arr != undefined) {
				...
				var j = 0;
				for (var i in activity_data.activity_arr) {
					rightHtmls += '<li class="sort" data-activity-id="' + i + '"><a href="javascript:"><div class="coupon-money">' + activity_data.activity_arr[i]['title'].toString().substr(0, 4) + '</div></a><a class="close-modal js-delete-activity small hide" data-id="0" title="删除">×</a></li>';
				}
			}
			...
			widget_link_hd(rightHtml.find('.js-add-goods'), 'activity_module', function(result) {
				...
				if (activity_data.activity_arr != undefined) {
					...
					for (var i in activity_data.activity_arr) {
						...
						activity_list_html += '<li class="sort" data-activity-id="' + i + '"><a href="javascript:"><div class="coupon-money">' + activity_data.activity_arr[i]['title'].toString().substr(0, 4) + '</div></a><a class="close-modal js-delete-activity small hide" data-id="0" title="删除">×</a></li>';
					}
					html += "</ul>";
					...
					for (var i in activity_data) {
						...
						htmls = '<img style="max-height:80px;" src="/upload/images/default_shop_2.jpg">';
						html += '<li style="display: block; width: 92px; height: 130px; border: 1px solid white;border-radius: 4px; background: white; float: left; margin: 5px 4px 0 0;"><a href="javascript:;">' + htmls + '</a><h3 style="font-size:.7em;text-align:center">' + typename + '标题xx</h3><p style="text-align:center"> <span style="font-size: 1.1em;color: #57ac50;margin-right: 5px;">xx 元</span></p></li>';
						activity_list_html += '<li class="sort" data-activity-id="' + i + '"><a href="javascript:"><div class="coupon-money">' + activity_data[i]['title'].substr(0, 4) + '</div></a><a class="close-modal js-delete-activity small hide" data-id="0" title="删除">×</a></li>';
					}
					html += "</ul>";
					//console.log(html)
				}
				...
				rightHtml.find(".js-delete-activity").click(function(event) {
					...
					//取出来
					//var activity_data = domHtml.data('activity_data');
					var activity_data = dom.find('.activity').data('activity_data');
					try {
						//删除coupon_data.coupon_arr数组的coupon_id
						delete activity_data.activity_arr[activity_id];
						delete activity_data[activity_id];
					...
					//重新存进去
					//domHtml.data('activity_data', activity_data);
					dom.find('.activity').data('activity_data', activity_data);
				});
});
};
...
clickArr[fieldType]();
	 ...
	 */
	 setEvent: function(obj) {
		...
		clickArr['title'] = function() {
			...
			app_field.find('.custom-title').data({
				'title': (obj.content.title ? obj.content.title : ''),
				'sub_title': (obj.content.sub_title ? obj.content.sub_title : ''),
				'show_method': (obj.content.show_method ? obj.content.show_method : ''),
				'bgcolor': (obj.content.bgcolor ? obj.content.bgcolor : '')
			});
		...
		clickArr['subject_display'] = function() {
			...
			app_field.find('.subject_display').data({
				'day_type': (obj.content.day_type ? obj.content.day_type : '1'),
				'px_style': (obj.content.px_style ? obj.content.px_style : 'asc'),
				'hour': (obj.content.hour ? obj.content.hour : '0'),
				'update_hour': (obj.content.update_hour ? obj.content.update_hour : '1'),
				'day': (obj.content.day ? obj.content.day : '3'),
				'number': (obj.content.number ? obj.content.number : '1')
			});
			app_field.find('.subject_display').attr({
				'day_type': (obj.content.day_type ? obj.content.day_type : '1'),
				'hour': (obj.content.hour ? obj.content.hour : '0'),
				'update_hour': (obj.content.update_hour ? obj.content.update_hour : '1'),
				'day': (obj.content.day ? obj.content.day : '3'),
		...
		clickArr['tpl_shop'] = function() {
			...
			app_field.find('.tpl-shop').data({
				'bgcolor': (obj.content.bgcolor ? obj.content.bgcolor : ''),
				'title': (obj.content.title ? obj.content.title : ''),
				'shop_head_bg_img': (obj.content.shop_head_bg_img ? obj.content.shop_head_bg_img : ''),
				'shop_head_logo_img': (imgs ? imgs : '')
			});
		...
		clickArr['white'] = function() {
			...
			app_field.find('.custom-white').data({
				'left': obj.content.left,
		...
		clickArr['image_ad'] = function() {
			...
			if (getObjLength(obj.content.nav_list) == 0) {
				...
				if (obj.content.image_type == '0') {
					html += '<div class="custom-image-swiper"><div class="swiper-container" style="height:' + obj.content.cache_height + 'px"><div class="swiper-wrapper">';
					for (var i in obj.content.nav_list) {
						obj.content.nav_list[i].image = obj.content.nav_list[i].image != '' ? obj.content.nav_list[i].image : '';
						html += '<div style="height:' + obj.content.cache_height + 'px" class="swiper-slide"><a href="javascript:void(0);" style="height:' + obj.content.cache_height + 'px;">' + (obj.content.nav_list[i].title != '' ? '<h3 class="title">' + obj.content.nav_list[i].title + '</h3>' : '') + '<img src="' + obj.content.nav_list[i].image + '" style="max-height:' + obj.content.cache_height + 'px;"/></a></div>';
					}
					html += '</div></div></div>';
			...
			app_field.find('.control-group').prepend(html).data({
				'navList': obj.content.nav_list,
				'type': (obj.content.image_type ? obj.content.image_type : 0),
				'size': (obj.content.image_size ? obj.content.image_size : 0),
				'cache_height': (obj.content.cache_height ? obj.content.cache_height : 0),
				'max_height': (obj.content.max_height ? obj.content.max_height : 0),
				'max_width': (obj.content.max_width ? obj.content.max_width : 0)
			});
		};
		...
		clickArr['goods_group2'] = function() {
			...
			//此处给已经添加的商品赋值给data
			app_field.find('.control-group').html('<ul class="sc-goods-list clearfix size-2 card pic"></ul>').data({
				'goods': obj.content.goods,
				'size': (obj.content.size ? obj.content.size : '0'),
				'size_type': (obj.content.size_type ? obj.content.size_type : '0'),
				'buy_btn': (obj.content.buy_btn ? obj.content.buy_btn : '0'),
				'buy_btn_type': (obj.content.buy_btn_type ? obj.content.buy_btn_type : '0'),
				'show_title': (obj.content.show_title ? obj.content.show_title : '0'),
				'price': (obj.content.price ? obj.content.price : '0')
			});
		...
		clickArr['coupons'] = function() {
			...
			app_field.find('.control-group > .custom-coupon').data('coupon_data', {
				'coupon_arr': data
			});
		...
		clickArr['events'] = function() {
			if (obj.content.events) {
				for (var i in obj.content.events) {
					obj.content.events[i].image = obj.content.events[i].image;
				}
			}
			//此处给已经添加的茶会赋值给data
			app_field.find('.control-group').html('<ul class="sc-events-list clearfix style-base card pic"></ul>').data({
				'events': obj.content.events,
				'size': (obj.content.size ? obj.content.size : '0'),
				'size_type': (obj.content.size_type ? obj.content.size_type : '0'),
				'show_desc': (obj.content.show_desc ? obj.content.show_desc : '0'),
				'price': (obj.content.price ? obj.content.price : '0')
			});
			customField.clickEvent(app_field);
			app_field.removeClass('editing');
		...
		clickArr['tables'] = function() {
			if (obj.content.tables) {
				for (var i in obj.content.tables) {
					obj.content.tables[i].image = obj.content.tables[i].image;
				}
			}
			//此处给已经添加的茶会赋值给data
			app_field.find('.control-group').html('<ul class="sc-tables-list clearfix style-base card pic"></ul>').data({
				'tables': obj.content.tables,
				'size': (obj.content.size ? obj.content.size : '0'),
				'size_type': (obj.content.size_type ? obj.content.size_type : '0'),
				'show_desc': (obj.content.show_desc ? obj.content.show_desc : '0'),
				'price': (obj.content.price ? obj.content.price : '0')
		...
		clickArr['article'] = function() {
			...
			//存值
			app_field.find('.control-group > .activity').attr('article_data', {
				'name': obj.content.name,
				'display': obj.content.display,
				'activity_arr': obj.content.activity_arr
			}).data('article_data', {
				'name': obj.content.name,
				'display': obj.content.display,
				'activity_arr': obj.content.activity_arr
		...
		clickArr['new_activity_module'] = function() {
			...
			app_field.find('.control-group').html('<ul class="activity clearfix"></ul>');
			//存值
			app_field.find('.control-group > .activity').attr('activity_data', {
				'name': obj.content.name,
				'display': obj.content.display,
				'activity_arr': obj.content.activity_arr
			}).data('activity_data', {
	 ...
	 getContent: function(dom) {
		...
		returnArr['image_nav'] = function() {
			...
			for (var i in navList) {
				returnObj[num] = {
					...
					prefix: navList[i].prefix,
					url: navList[i].url,
					image: navList[i].image.replace('./upload/', '')
				};
				num++;
			}
		};
		...
		/*		returnArr['goods_group2'] = function(){
					...
					for (var i in navList) {
						if (navList[i].type == 'link') {
							...
							returnObj[num] = {
								id: navList[i].id,
								name: navList[i].name,
								number: navList[i].number,
								url: navList[i].url,
								prefix: navList[i].prefix,
								type: navList[i].type,
								'widget': navList[i].widget
							};
						}
						num++;
					}
				...
				returnArr['image_ad'] = function() {
					...
					returnObj.cache_height = domHtml.data('cache_height');
			...
			returnArr['goods_group1'] = function() {
				...
				for (var i in goods_group_arr) {
					returnObj.goods_group1[num] = {
						id: goods_group_arr[i].id,
						title: goods_group_arr[i].title,
						show_num: goods_group_arr[i].show_num
					};
					num++;
				}
			};
		...
		returnArr['coupons'] = function() {
			...
			if (coupon_list.coupon_arr != undefined) {
				...
				for (var i in coupon_list.coupon_arr) {
					returnObj.coupon_arr[num] = {
						id: coupon_list.coupon_arr[i].id,
						title: coupon_list.coupon_arr[i].title,
						face_money: coupon_list.coupon_arr[i]['face_money'],
						condition: coupon_list.coupon_arr[i]['condition']
					};
					num++;
				}
			} else {
				...
				for (var i in coupon_list) {
					returnObj.coupon_arr[num] = {
						...
						face_money: coupon_list[i]['face_money'],
						condition: coupon_list[i]['condition']
					};
					num++;
				}
			}
		...
		returnArr['events'] = function() {
			returnObj.type = 'events';
			var domHtml = dom.find('.control-group');

			returnObj.size = domHtml.data('size');
			returnObj.size_type = domHtml.data('size_type');
			returnObj.show_desc = domHtml.data('show_desc');
			returnObj.price = domHtml.data('price');
			returnObj.events = {};
			var events = domHtml.data('events');
			var num = 0;
			for (var i in events) {
				returnObj.events[num] = {
					id: events[i].id,
					title: events[i].title,
					price: events[i].price,
					url: events[i].url,
					image: events[i].image.replace('./upload/', '')
				};
				num++;
			}
		};

		returnArr['tables'] = function() {
			returnObj.type = 'tables';
			var domHtml = dom.find('.control-group');
			...
			returnObj.price = domHtml.data('price');
			returnObj.tables = {};
			var tables = domHtml.data('tables');
			var num = 0;
			for (var i in tables) {
				returnObj.tables[num] = {
					id: tables[i].id,
					title: tables[i].title,
					price: tables[i].price,
					url: tables[i].url,
					image: tables[i].image.replace('./upload/', '')
				};
				num++;
			}
		};
		returnArr['goods'] = function() {
			...
			for (var i in goods) {
				returnObj.goods[num] = {
					...
					price: goods[i].price,
					url: goods[i].url,
					image: goods[i].image.replace('./upload/', '')
				};
				num++;
			}
			//alert(obj2String(returnObj));
		...
		returnArr['article'] = function() {
			...
			var title = domHtml.data('name');
			if (title == undefined || title == '') {
				title = $('.control-group .controls').children('input').val();
			}
			returnObj.name = title;
			returnObj.display = domHtml.data('display');
			var activity_list = domHtml.data('article_data');
			var num = 0;
			if (!activity_list) {
				activity_list = {};
			}
			...
			if (activity_list.activity_arr != undefined) {
				...
				for (var i in activity_list) {
					returnObj.activity_arr[num] = {
						...
						title: activity_list[i].title,
						atype: activity_list[i].atype
					};
					num++;
				}
				//}
	...
	/**
	 * 删除指定模块
	 * @param del_modules 要删除的模板名称 (data-field-type)
	 * @returns {boolean}
	 */
	 remove: function(del_modules) {
	 	if (del_modules == '') {
	 		return false;
	 	}
	 	if (!$.isArray(del_modules)) {
	 		del_modules = del_modules.split(',');
	 	}
	 	$('.js-add-region > div > .app-add-field > ul > li > a').each(function(i) {
	 		var field_type = $(this).data('field-type');
	 		if ($.inArray(field_type, del_modules) >= 0) {
	 			$(this).closest('li').remove();
	 		}
	 	})
	 	return true;
