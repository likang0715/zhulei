$(function(){
	...
	load_page('.app__content',load_url,{page: page_content, keyword : return_keyword, group_id : return_group_id, p : return_p},'');
    ...
    $(".js-dump-btn").live("click",function(){
    	var checkout_type = $(this).val();
    	var loadi =layer.load('正在查询', 10000000000000);	
    	var checkout_type = $("#select_level").val();
		var group_id = $('.chosen-single').attr('group-id');
		var keyword = $('.ui-search-box .txt').val();
    	$.post(
    			checkout_url,
    			{page: page_content,"checkout_type":checkout_type,"type":checkout_type,"check_count":1,"group_id":group_id,"keyword":keyword},
    			function(obj) {
    				layer.close(loadi);
    				if(obj.err_msg == '您没有操作权限'){
    					layer.alert('您没有操作权限！', 8); 
    					return false;
    				}
    				if(obj.err_msg>0) {
    					layer.confirm('该指定条件下有 商品  '+obj.err_msg+' 个，确认导出？',function(indexs){
    						layer.close(indexs);
    					 	location.href=checkout_url+"&page="+page_content+"checkout_type="+checkout_type+"&type="+checkout_type+"&group_id="+group_id+"&keyword="+keyword;
    					});
    				} else {
    					layer.alert('该搜索条件下没有商品数据，无需导出！', 8); 
    			}	
    		},
			'json'
    	)	  
        ...
        button_box($(this), e, 'left', 'confirm', '确认删除？', function(){
            ...
            $.post(del_product_url, {'product_id': product_id}, function(data) {
                ...
                if (data.err_code == 0) {
                    ...
                    var p = 0;
                    if ($(".js-page-list a.active").length > 0) {
                        p = $(".js-page-list a.active").attr('data-page-num');
                    }
                    var orderbyfield = '';
                    var orderbymethod = '';
                    var index = 0;
                    ...
                    load_page('.app__content',load_url,{page: page_content, 'p': p, 'keyword': keyword, 'group_id': group_id, 'orderbyfield': orderbyfield, 'orderbymethod': orderbymethod},'', function(){
    ...
    $('.js-promotion-btn').live('click', function(){
        ...
        qrcode_url = './source/qrcode.php?type=good&id=' + product_id + '&store_id=' + store_id;
        ...
        $.post(get_qrcode_activity_url, {'product_id': product_id}, function(data){
            ...
            if (data) {
                ...
                for (i in data) {
                    ...
                    qrcode_arr[data[i]['pigcms_id']] = '<img class="loading" src="./source/qrcode.php?type=good&id=' + product_id + '&store_id=' + store_id + '&activity=' + data[i]['pigcms_id'] + '" /><p>' + desc + '</p><div class="clearfix qrcode-links"><a class="pull-left" href="./source/qrcode.php?type=good&id=' + product_id + '&activity=' + data[i]['pigcms_id'] + '" download="true" target="_blank">下载二维码</a><a class="pull-right js-edit" data-id="' + data[i]['pigcms_id'] + '" href="javascript:;">修改优惠</a></div>';
                }
            }
        ...
        $.post(save_qrcode_activity_url, {'product_id': product_id, 'buy_type': buy_type, 'type': type, 'discount': discount, 'price': amount, 'activity_id': activity_id}, function(data){
            ...
            if (data.code) {
                ...
                if (data.type == 'add') {
                    ...
                    qrcode_arr[data.data.pigcms_id] = $('.text-center').html();
                    
                    for (var i in qrcode_data) {
                    	if (qrcode_data[i].pigcms_id == data.data.pigcms_id) {
                    		qrcode_data[i] = data.data;
                    	}
                    }
                }
