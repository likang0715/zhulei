/**
 * Created by ediancha on 2016/08/02.
 */
 var t;
 function location_page(mark, dom) {
 	...
 	switch (mark_arr[0]) {
 		...
 		$(".third-header-inner .contact").addClass("active").siblings().removeClass("active");
 		...
 		break;
 		case '#list':
 		$(".third-header-inner .list:eq(0)").addClass("active").siblings().removeClass("active");
 		...
 		break;
 		case '#admin_list':
 		$(".third-header-inner .list").removeClass("active");
 		$('.js-app-nav.admin').addClass('active').siblings('.js-app-nav').removeClass('active');
 		load_page('.app__content', load_url, {
 			page: 'admin_list'
 		}, '');
 		break;
...
$('.js-admin-submit').live('click', function(event) {
	var nowDom = $(this);
	layer.closeAll();
	var formObj = {};
	var form = $('.form-horizontal').serializeArray();
	...
	$.post('user.php?c=setting&a=edit_admin', formObj, function(result) {
		if (typeof(result) == 'object') {
			if (result.err_code) {
