		...
		case "#edit":
			if(mark_arr[1]){
				load_page('.app__content', load_url,{page:'wei_page_category_edit',cat_id:mark_arr[1]},'',function(){
					...
					defaultFieldObj.data({'cat_id':editDataDom.attr('cat-id'),'moban':editDataDom.attr('moban'),'cat_name':editDataDom.attr('cat-name'),'first_sort':editDataDom.attr('first-sort'),'second_sort':editDataDom.attr('second-sort'),'cat-src':editDataDom.attr('cat-src')});
...
function page_create(emptyData){
	...
	defaultFieldObj = $('.js-config-region .app-field');
	if(emptyData == 1) defaultFieldObj.data({'cat_id':'0','cat_name':'','first_sort':'0','second_sort':'0','moban':'0'});
	defaultFieldObj.click(function(){
		...
		//作为模板
		$('.js-sidebar-region input.input-xxlarge[value="'+defaultFieldObj.data('moban')+'"]').attr('checked', 'checked')
		$('.js-sidebar-region input.input-xxlarge').change(function(){
			defaultFieldObj.data('moban',$(this).val());
		});
		//默认图片
		$('.js-sidebar-region .controls img').attr('src',defaultFieldObj.data('cat-src'));
	...
	$('.form-actions .btn-save').live('click',function(){
		...
		post_data.moban = defaultFieldObj.data('moban');
		...
		//封面路径
		post_data.cover_img=$('.controls img').attr('src');
...
var defaultHtmlObj = function(){
	if(is_adminuser==1) {
	return '<div><div class="form-horizontal"><div class="js-meta-region" style="margin-bottom: 20px;"><div><div class="control-group"><label class="control-label"><em class="required">*</em>分类名：</label><div class="controls"><input type="text" name="title" value=""></div></div><div class="control-group"><label class="control-label">第一优先级：</label><div class="controls"><select name="first_sort"><option value="0">序号越大越靠前</option><option value="1">最热的排在前面</option></select></div></div><div class="control-group"><label class="control-label">第二优先级：</label><div class="controls"><select name="second_sort"><option value="0">创建时间越晚越靠前</option><option value="1">创建时间越早越靠前</option><option value="2">最热的排在前面</option></select></div></div><div class="control-group"><label class="control-label">用户模板：</label><div class="controls"><input class="input-xxlarge" type="radio" name="moban" checked="checked" value="1" id="moban_yes"><label for="moban_yes" style="display: inline-block;">作为模板</label>&nbsp;&nbsp;&nbsp;&nbsp;<input class="input-xxlarge" type="radio" name="moban" id="moban_no" value="0"><label for="moban_no" style="display: inline-block;">不作为模板</label></div></div><!--div class="control-group"><label class="control-label">显示方式：</label><div class="controls"><label class="radio inline"><input type="radio" name="show_method" value="0" checked=""/>仅显示杂志列表</label><label class="radio inline"><input type="radio" name="show_method" value="1"/>用期刊方式展示</label/></div--></div></div></div><div class="control-group"><hr/><div class="separate-line"><p class="text-center">微页面分类简介</p><p class="text-center">v</p></div></div><div class="control-group"><div name="content"><script class="js-editor" type="text/plain"></script></div></div></div></div>';
	}else{
	return '<div><div class="form-horizontal"><div class="js-meta-region" style="margin-bottom: 20px;"><div><div class="control-group"><label class="control-label"><em class="required">*</em>分类名：</label><div class="controls"><input type="text" name="title" value=""></div></div><div class="control-group"><label class="control-label">第一优先级：</label><div class="controls"><select name="first_sort"><option value="0">序号越大越靠前</option><option value="1">最热的排在前面</option></select></div></div><div class="control-group"><label class="control-label">第二优先级：</label><div class="controls"><select name="second_sort"><option value="0">创建时间越晚越靠前</option><option value="1">创建时间越早越靠前</option><option value="2">最热的排在前面</option></select></div></div><div class="control-group"><label class="control-label">封面图：</label><div class="controls"><img src="/upload/images/default_ucenter.jpg" width="100"/>&nbsp;&nbsp;&nbsp;<a class="js-choose-bg control-action" href="javascript: void(0);">修改封面图</a><p class="help-block">建议尺寸：230 x 150 像素</p></div></div><!--div class="control-group"><label class="control-label">显示方式：</label><div class="controls"><label class="radio inline"><input type="radio" name="show_method" value="0" checked=""/>仅显示杂志列表</label><label class="radio inline"><input type="radio" name="show_method" value="1"/>用期刊方式展示</label/></div--></div></div></div><div class="control-group"><hr/><div class="separate-line"><p class="text-center">微页面分类简介</p><p class="text-center">v</p></div></div><div class="control-group"><div name="content"><script class="js-editor" type="text/plain"></script></div></div></div></div>';
	}
};
var keyword = '';
$(function(){
    ...
    $(window).keydown(function(event){
        if (event.keyCode == 13 && $('.js-wei_page_txt').is(':focus')) {
            keyword = $('.js-wei_page_txt').val().trim();
            load_page('.app__content', load_url,{page:'wei_page_category_content', 'keyword': keyword}, '', function() {
                $('.js-wei_page_txt').val(keyword);
            });
        }
    })
