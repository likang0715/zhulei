...
<div class="widget-list">
	<div class="js-list-filter-region clearfix ui-box" style="position:relative;">
		<div>
			<a href="#create" class="ui-btn ui-btn-primary">添加赠品</a>
	...
	?>
		<table class="ui-table ui-table-list" style="padding:0px;">
			...
			<tbody class="js-list-body-region">
				...
				?>
					<tr class="js-present-detail" service-id="<?php echo $present['id']?>">
						<td><?php echo htmlspecialchars($present['name']) ?></td>
						<td><?php echo date('Y-m-d H:i:s', $present['start_time']) ?>至<?php echo date('Y-m-d H:i:s', $present['end_time']) ?></td>
						<td><?php echo $present['status'] ? getTimeType($present['start_time'], $present['end_time']) : '已结束' ?></td>
						<!-- <td><?php echo $present['number'];?></td> -->
						<td class="text-right js-operate" data-present_id="<?php echo $present['id'] ?>">
							<?php 
							if ($present['status']) {
							?>
								<a href="#edit/<?php echo $present['id']?>" class="js-edit">编辑资料</a>
								<span>-</span>
