...
$(function(){
	...
	load_page('.app__content',load_url,{page:'create_content'},'',function(){
		...
		var defaultDesc = '<div class="goods-details-block" style="background:#fff;"><h4>礼物详情区</h4><p>点击进行编辑</p></div>';
	...
	$('.js-price').live('blur',function(){
		if($(this).val().length > 0){
			var float_val = parseInt($(this).val());
			if(float_val > 9999999.99){
				layer_tips(1,'积分最大为 9999999');
				$(this).val('9999999.99');
			}else if(!/^\d+(\.\d+)?$/.test($(this).val())){
				layer_tips(1,'只输入合法的积分');
				return false;
			}else if(float_val < 0){
				layer_tips(1,'积分最小为 0');
				return false;
			}else{
	...
	$('.js-batch-price').live('click',function(){
		...
		$('.js-batch-form').show();
		$('.js-batch-type').hide();
		$('.js-batch-txt').attr('placeholder','请输入积分');
		$('.js-batch-txt').focus();
	});
	...
	$('.js-btn-save').live('click', function(){
		...
		$.post(save_url, {'category_id': category_id, 'buy_way': buy_way, 'buy_url': buy_url, 'quantity': quantity, 'show_stock': show_stock, 'code': code, 'name': name, 'price': price, 'original_price': original_price,'after_subscribe_price': after_subscribe_price,'images': images, 'postage_type': postage_type, 'postage': postage, 'postage_tpl_id': postage_tpl_id, 'buyer_quota': buyer_quota, 'sold_time': sold_time, 'discount': discount, 'invoice': invoice, 'warranty': warranty, 'skus': skus, 'status': status, 'intro': intro, 'info': info, 'preview': preview,'sys_fields': sys_fields, 'fields': fields,'group_ids':group_ids, 'weight' : weight, 'is_wholesale': is_wholesale, 'is_fx': is_fx, 'is_recommend' : is_recommend, 'recommend_title' : recommend_title, 'custom':customField.checkEvent(), 'send_other' : send_other, 'send_other_postage' : send_other_postage,'give_points':give_points,'product_degree_discount_id':product_degree_discount_id,'product_degree_discount_name':product_degree_discount_name,'product_degree_discount':product_degree_discount,'check_give_points':check_give_points,'check_degree_discount':check_degree_discount}, function(data) {
			if (data.err_code == 0) {
			  $('.notifications').html('<div class="alert in fade alert-success">礼物保存成功</div>');
			...
			if(cat_id == '' || cat_name == ''){
				...
				if (next_id == 2) {
					...
					if ($("input[name='title']").val() == '') {
						...
						$('.js-step-2').addClass('active');
						$('.fm-goods-info button.js-switch-step').attr('data-next-step',3);
						layer_tips(1,'礼物名长度不能少于一个字或者多于100个字');
			...
			if(next_id == 1){
				...
				$('.table-sku-stock > tbody > tr').each(function(i){
					...
					if(sku_price.length > 0){
						var float_val = parseInt(sku_price);
						if(float_val > 9999999){
							layer_tips(1,'积分最大为 9999999');
							$(this).find(".js-price").val('9999999');
...
function create_sku(){
	...
	switch(sku_dom_count){
		case 0:
			...
			return false;
		case 1:
			html += '<thead><tr><th class="text-center">'+(sku_arr[0].siblings('h3').find('span.select2-chosen').html())+'</th><th class="th-price">所需积分</th><th class="th-stock">库存</th><th>重量(克)</th><th class="th-code">礼物编码</th><th class="text-right">销量</th></tr></thead>';
			html += '<tbody>';
		...
		case 2:
			...
			break;
		case 3:
			html += '<thead><tr><th class="text-center">'+(sku_arr[0].siblings('h3').find('span.select2-chosen').html())+'</th><th class="text-center">'+(sku_arr[1].siblings('h3').find('span.select2-chosen').html())+'</th><th class="text-center">'+(sku_arr[2].siblings('h3').find('span.select2-chosen').html())+'</th><th class="th-price">所需积分</th><th class="th-stock">库存</th><th>重量(克)</th><th class="th-code">礼物编码</th><th class="text-right">销量</th></tr></thead>';
			html += '<tbody>';
