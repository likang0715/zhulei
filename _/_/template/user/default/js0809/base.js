 ...
 function teaAlert(txt,act, fun) {
 	...
 	if (act=="loading") {
 		if (teaAlertLoading) {
 			...
 			$('.teaAlert').remove();
 			var txt = txt?txt:"保存中";
 			var div = $('<div class="teaAlert teaAlertLoading" style="background: rgba(0,0,0,0.8);padding:0px 20px;min-height: 70px;margin:-35px 0 0 -120px;min-width: 240px;position: fixed;text-align: center;border-radius:10px;"><span style="color: #ffffff;line-height: 70px;font-size:20px;">' + txt + '</span></div>');
 			$('body').append(div);
 		...
 		if ($('.teaAlert.teaAlertLoading').size()>0) {
 			...
 			$('.teaAlert').remove();
 			var div = $('<div class="teaAlert teaAlertComplete" style="background: rgba(0,0,0,0.8);padding:0px 20px;min-height: 70px;margin:-35px 0 0 -120px;min-width: 240px;position: fixed;text-align: center;border-radius:10px;"><span style="color: #ffffff;line-height: 70px;font-size:20px;">' + txt + '</span></div>');
 			$('body').append(div);
 			div.css('zIndex', 9999999);
 			div.css('left', parseInt(($(window).width() - div.width()) / 2));
 			var top = parseInt($(window).scrollTop() + ($(window).height() - div.height()) / 2);
 ...
 */
 ...
 function format(times)
 {
 	...
 	var returntime = y+'-'+add0(m)+'-'+add0(d)+' '+add0(h)+':'+add0(mm)+':'+add0(s);
 ...
 */
 function teaLayer (type,dom,title,fun,closefun) {
 	var type = Number(type);
 	var html = '<div class="modal-backdrop fade in"></div>';
 	if (type==1) {
 		var widgetDom = $('<div class="widget-image modal fade in" style="top:-460px;"><div class="modal-header"><a class="close" data-dismiss="modal">×</a><ul class="module-nav modal-tab js-modal-tab"><li class="js-modal-tab-item"><a href="javascript:;">'+title+'</a></li></ul></div><div class="modal-body">'+dom+'</div></div>');	
 	} else if(type==2) {
 		var widgetDom = $('<div class="widget-image modal fade in" style="top:-460px;"><div class="modal-header"><a class="close" data-dismiss="modal">×</a><ul class="module-nav modal-tab js-modal-tab"><li class="js-modal-tab-item"><a href="javascript:;">'+title+'</a></li></ul></div><div class="modal-body">'+dom+'</div></div>');
 	} else if(type==3) {
 		if((typeof (dom)=="object") && (dom[0].length == dom[1].length)){
 			...
 			var widgetDom = $('<div class="widget-image modal fade in"><div class="modal-header"><a class="close" data-dismiss="modal">×</a><ul class="module-nav modal-tab js-modal-tab">'+liDom+'</ul></div><div class="modal-body">'+paneDom+'</div></div>');
 			widgetDom.find('.js-modal-tab a').click(function(){
 				if(!$(this).closest('li').hasClass('active')){
 					$(this).closest('li').addClass('active').siblings('li').removeClass('active');
 					$('.js-tab-pane'+$(this).data('pane')).removeClass('hide').siblings('.js-tab-pane').addClass('hide');
