...
$(function(){
	...
	function location_page(mark, page){
		...
		switch(mark_arr[0]){
			case '#create':
				load_page('.app__content', load_url, {page : page_cutprice_create}, '',function(){
					add_product();
				});
				break;
			case "#edit":
...
$(".js-copy-link").live("click", function (e) {
	var cutprice_id = $(this).closest("td").data("present_id");
	button_box($(this),e,'left','copy', cutprice_wap_url + cutprice_id, function(){
		layer_tips(0,'复制成功');
	});
});
...
$(".js_show_ewm").live("click",function(e) {
	...
	var id = dom.data("id");
	var qrcode_url = cutprice_qrcode_url + '&store_id='+ store_id + "&id=" + id;
	var htmls = "";
