...
$(function() {
    ...
    //选项卡切换
    $('.income-nav > ul > li > a').live('click', function(){
        var page_content = $(this).attr('href').replace('#', '') + '_content';
        var index = $(this).index('.income-nav > ul > li > a');
        load_page('.app__content',load_url,{page: page_content, 'status': status},'', function(){
            $('.income-nav > ul > li').eq(index).siblings('li').removeClass('active');
            $('.income-nav > ul > li').eq(index).addClass('active');
        });
    })
