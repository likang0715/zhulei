...
$(function(){
	...
	load_page('.app__content',load_url,{page:'create_content'},'',function(){
		...
		$("input[name='open_return_point']").live('click', function(e) {
			if ($(this).is(':checked')) {
				$(this).closest('label').next('.controls').children("input[name='return_point']").attr('readOnly', false).focus();
			} else {
	...
	/*$('.js-batch-point').live('click',function(){
		js_batch_type = 'point';
		$('.js-batch-form').show();
		$('.js-batch-type').hide();
		$('.js-batch-txt').attr('placeholder','请输入价格');
	...
	$('.js-batch-save').live('click',function(){
		...
		if(js_batch_type == 'price'){
			...
			var is_point = true;
			$('.js-sku_point').each(function () {
				var price = $(this).closest("tr").find(".js-price").val();
				
				if (price.length > 0 && parseFloat(price) < parseFloat(batch_txt.val())) {
					is_point = false;
					layer_tips(1, '赠送的消费积分不能大于价格');
					$(this).focus();
					return false;
				}
				$(this).val(batch_txt.val());
			});
			...
			$('.js-goods-stock .js-sku_point').val(batch_txt.val());
			$("input[name='open_return_point']").prop("checked", true);
			
			$('input[name="return_point"]').val(batch_txt.val());
			batch_txt.val('');
    ...
    $('.js-btn-save').live('click', function(){
        ...
        if ($('.js-message-container > .message-item').length > 0) {
            $('.js-message-container > .message-item').each(function(i){
                ...
                if ($("input[name='multi_rows']:checked").length) {
                    var field_multi_rows = $(this).children("input[name='multi_rows']:checked").val();
                }
                if ($("input[name='required']:checked").length) {
                    var field_required = $(this).children("input[name='required']:checked").val();
                }
		...
		$('.table-sku-stock > tbody > tr').each(function(i){
            ...
            //var sku_after_subscribe_discount = $(this).find('.js-after-subscribe-discount').val(); //关注后折扣
            ...
            if (is_batch_point && sku_point.length > 0 && parseFloat(sku_point) > parseFloat(sku_price)) {
            	...
            	is_sku = false;
            	$('.js-step').hide();
				$('.js-switch-step').removeClass('active');
				$('#step-2').show();
				$('#step-2').addClass('active');
				$('.fm-goods-info button.js-switch-step').attr('data-next-step',3);
            	return false;
            }
            skus[i] = {"props_str":props_str.substring(0, props_str.length-1),"price":sku_price, "point": sku_point, "after_subscribe_price":sku_after_subscribe_price,"quantity":sku_quantity,"weight" : sku_weight, "code":sku_code};
        });
        ...
        //var after_subscribe_price = $("input[name='after_subscribe']").val();
        var images = [];
        ...
        if ($('.js-message-container > .message-item').length > 0) {
            $('.js-message-container > .message-item').each(function(i){
                ...
                if ($("input[name='multi_rows']:checked").length) {
                    var field_multi_rows = $(this).find("input[name='multi_rows']:checked").val();
                }
                if ($("input[name='required']:checked").length) {
                    var field_required = $(this).find("input[name='required']:checked").val();
                }
		...
		var send_other = $("input[name='send_other']:checked").val();
		var send_other_postage = $("input[name='send_other_postage']").val();
        ...
        $.post(save_url, {
			...
			'after_subscribe_price': after_subscribe_price,
			'images': images,
	...
	switch(now_step){
			...
			if(cat_id == '' || cat_name == ''){
                ...
                if (next_id == 2) {
					...
					if ($("input[name='after_subscribe_discount']").val() != '' && (isNaN($("input[name='after_subscribe_discount']").val()) || $("input[name='after_subscribe_discount']").val() < 1 || $("input[name='after_subscribe_discount']").val() > 10)) {
						...
						$('.js-step-2').addClass('active');
						$('.fm-goods-info button.js-switch-step').attr('data-next-step', 3);
						layer_tips(1, '关注后折扣只能填写1到10之间的数字');
						$("input[name='after_subscribe_discount']").val('');
						$("input[name='after_subscribe_discount']").focus();
						return false;
					}

					//if ($("input[name='after_subscribe']").val() != '' && (isNaN($("input[name='after_subscribe']").val()) || $("input[name='after_subscribe']").val() < 0)) {
					//	$('.js-step').hide();$('#step-2').show();
					//	$('.js-step-1').removeClass('active');
					//	$('.js-step-2').addClass('active');
					//	$('.fm-goods-info button.js-switch-step').attr('data-next-step',3);
					//	layer_tips(1,'关注后价格只能填写大于0的数字');
					//	$("input[name='after_subscribe']").val('');
					//	$("input[name='after_subscribe']").focus();
					//	return false;
					//}
		...
		case '2':
			if(next_id == 1){
				...
				if ($("input[name='after_subscribe_discount']").val() != '' && (isNaN($("input[name='after_subscribe_discount']").val()) || $("input[name='after_subscribe_discount']").val() < 1 || $("input[name='after_subscribe_discount']").val() >10)) {
					layer_tips(1,'关注后折扣只能填写1到10之间的数字，可包含小数');
					$("input[name='after_subscribe_discount']").val('');
					$("input[name='after_subscribe_discount']").focus();
					return false;
				}
				...
				if ($("input[name='send_other']:checked").val() == "1" && ($("input[name='send_other_postage']").val() == '' || isNaN($("input[name='send_other_postage']").val()) || $("input[name='send_other_postage']").val() < 0)) {
...
function create_sku(){
	...
	switch(sku_dom_count){
		case 0:
			...
			return false;
		case 1:
			html += '<thead><tr><th class="text-center">'+(sku_arr[0].siblings('h3').find('span.select2-chosen').html())+'</th><th class="th-price">价格（元）</th><th class="th-price"><input type="checkbox" class="js-open_point" /> 消费积分</th><th class="th-stock">库存</th><th>重量(克)</th><th class="th-code">商品编码</th><th class="text-right">销量</th></tr></thead>';
			html += '<tbody>';
			$.each(sku_arr[0].find('.sku-atom'),function(i,item){
				html += '<tr sku-ids="' + ($(item).attr('atom-id')) + '"><td rowspan="1">'+($(item).find('span').html())+'</td><td><input type="text" name="sku_price" class="js-price input-mini" value="" maxlength="10"/></td><td><input type="text" name="sku_point" class="js-sku_point input-mini" value="" readonly="readonly" maxlength="10"/></td><td><input type="text" name="stock_num" class="js-stock-num input-mini" value="" maxlength="9"/></td><td><input type="text" name="sku_weight" class="js-sku_weight input-mini" value="0" maxlength="9"/></td><td><input type="text" name="code" class="js-code input-small" value=""></td><td class="text-right">0</td></tr>';
			});
			html += '</tbody>';
			html += '<tfoot><tr><td colspan="7"><div class="batch-opts">批量设置： <span class="js-batch-type"><a class="js-batch-price" href="javascript:;">价格</a>&nbsp;&nbsp;<a class="js-batch-stock" href="javascript:;">库存</a>&nbsp;&nbsp;<a class="js-batch-weight" href="javascript:;">重量</a></span><span class="js-batch-form" style="display:none;"><input type="text" class="js-batch-txt input-mini" placeholder=""/>&nbsp;&nbsp;<a class="js-batch-save" href="javascript:;">保存</a>&nbsp;&nbsp;<a class="js-batch-cancel" href="javascript:;">取消</a><p class="help-desc"></p></span></div></td></tr></tfoot>';
			break;
		case 2:
			html += '<thead><tr><th class="text-center">'+(sku_arr[0].siblings('h3').find('span.select2-chosen').html())+'</th><th class="text-center">'+(sku_arr[1].siblings('h3').find('span.select2-chosen').html())+'</th><th class="th-price">价格（元）</th><th class="th-price"><input type="checkbox" class="js-open_point" /> 消费积分</th><th class="th-stock">库存</th><th>重量(克)</th><th class="th-code">商品编码</th><th class="text-right">销量</th></tr></thead>';
			html += '<tbody>';
			$.each(sku_arr[0].find('.sku-atom'),function(i,item){
			...
			html += '</tbody>';
			html += '<tfoot><tr><td colspan="8"><div class="batch-opts">批量设置： <span class="js-batch-type"><a class="js-batch-price" href="javascript:;">价格</a>&nbsp;&nbsp;<a class="js-batch-stock" href="javascript:;">库存</a>&nbsp;&nbsp;<a class="js-batch-weight" href="javascript:;">重量</a></span><span class="js-batch-form" style="display:none;"><input type="text" class="js-batch-txt input-mini" placeholder=""> <a class="js-batch-save" href="javascript:;">保存</a> <a class="js-batch-cancel" href="javascript:;">取消</a><p class="help-desc"></p></span></div></td></tr></tfoot>';
			break;
		case 3:
			html += '<thead><tr><th class="text-center">'+(sku_arr[0].siblings('h3').find('span.select2-chosen').html())+'</th><th class="text-center">'+(sku_arr[1].siblings('h3').find('span.select2-chosen').html())+'</th><th class="text-center">'+(sku_arr[2].siblings('h3').find('span.select2-chosen').html())+'</th><th class="th-price">价格（元）</th><th class="th-price"><input type="checkbox" class="js-open_point" checked="checked" /> 消费积分</th><th class="th-stock">库存</th><th>重量(克)</th><th class="th-code">商品编码</th><th class="text-right">销量</th></tr></thead>';
			html += '<tbody>';
			...
			html += '</tbody>';
			console.log(html);
			html += '<tfoot><tr><td colspan="9"><div class="batch-opts">批量设置： <span class="js-batch-type"><a class="js-batch-price" href="javascript:;">价格</a>&nbsp;&nbsp;<a class="js-batch-stock" href="javascript:;">库存</a>&nbsp;&nbsp;<a class="js-batch-weight" href="javascript:;">重量</a></span><span class="js-batch-form" style="display:none;"><input type="text" class="js-batch-txt input-mini" placeholder=""> <a class="js-batch-save" href="javascript:;">保存</a> <a class="js-batch-cancel" href="javascript:;">取消</a><p class="help-desc"></p></span></div></td></tr></tfoot>';
			break;
	}
