 ...
 */$(function(){
    ...
    $('.js-page-list > a').live('click', function(e){
        ...
        $('.app__content').load(load_url, {page: page_content,'is':is, 'p': p, 'keyword': keyword, 'group_id': group_id, 'orderbyfield': orderbyfield, 'orderbymethod': orderbymethod}, function(){
            if (group != '') {
    ...
    $('.js-search-btn').live('click',function(){
        var name = $("input[name='good_name']").val();
        var is = $('.data-is').data('is');
        var group_id = $('.js-search-drp-group').val();
        var group_name = $('.js-search-drp-group').find("option:selected").text()

        load_page('.app__content', load_url, {page:'my_wholesale_content','name':name, 'is': is, 'group_id':group_id}, '', function(){

            if(name != ''){
                $("input[name='good_name']").val(name);
            }
            if(group_name != ''){
                $('.js-search-drp-group').find("option[value='" + group_id + "']").attr("selected",true);
            }
        });
    ...
    $('.ui-nav > ul > li').live('click', function() {
        ...
        load_page('.app__content', load_url, {page:'my_wholesale_content','is':is}, '', function(){
        });
    })
    ...
    $('.js-batch-unload').live('click', function(e){
        ...
        button_box($(this), e, 'right', 'confirm', '确认下架？', function(){
            ...
            $.post(soldout_url, {'product_id': product_id}, function(data){
                ...
                $('.app__content').load(load_url, {page: page_content,'is':is, 'p': p, 'orderbyfield': orderbyfield, 'orderbymethod': orderbymethod}, function(){
                    $('.orderby').children('span').remove();
                    if (orderbymethod == 'asc') {
                        $('.orderby').eq(index).append('<span class="orderby-arrow asc"></span>');
                    } else {
    ...
    //下架单个商品
    $('.js-unload').live('click', function(e){
        var is = $(this).data('is');
        var product_id = $(this).attr('data');
        button_box($(this), e, 'left', 'confirm', '确认下架？', function(){
            $.post(soldout_url, {'product_id': product_id}, function(data){
                if (!data.err_code) {
                    $('.notifications').html('<div class="alert in fade alert-success">' + data.err_msg + '</div>');
                } else {
    ...
    $('.js-batch-delete').live('click', function(e){
        ...
        button_box($(this), e, 'right', 'confirm', '确认删除？', function(){
            $.post(del_product_url, {'product_id': product_id}, function(data) {
                ...
                if (data.err_code == 0) {
                    $('.notifications').html('<div class="alert in fade alert-success">' + data.err_msg + '</div>');
                    load_page('.app__content',load_url,{page: page_content,'is':is,'p':p},'');
                } else {
                    $('.notifications').html('<div class="alert in fade alert-error">' + data.err_msg + '</div>');
                }
            })
