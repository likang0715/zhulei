...
$(function(){
	...
	$('.btn-save-rule').live('click', function(){
		...
		$.post('/user.php?c=weixin&a=save_qrcode', {'name':name, 'pigcms_id':pigcms_id}, function(response){
			...
			if (response.err_code) {
				...
				if (pigcms_id == 0) {
					...
					var rule_html = '<div class="rule-group" data-id="' + response.ruleid + '"><div class="rule-meta"><h3><em class="rule-id">' + response.ruleid + ')</em> <span class="rule-name" id="rule-name-' + response.ruleid + '">'+name+'</span><div class="rule-opts"><a href="javascript:;" class="js-edit-rule" data-id="' + response.ruleid + '">编辑</a><span>-</span><a href="javascript:;" class="js-delete-rule" data-id="' + response.ruleid + '">删除</a></div></h3></div><div class="rule-body"><div class="long-dashed"></div><div class="rule-keywords"><div class="rule-inner"><h4>关键词：</h4><div class="keyword-container"><div class="info"></div><div class="keyword-list" id="keyword-list-' + response.ruleid + '><div class="keyword input-append"><a href="javascript:;" class="close--circle">×</a><span class="value">'+name+'</span><span class="add-on">全匹配</span></div></div></div><hr class="dashed"/><div class="opt"><a href="javascript:;" class="js-add-keyword">+ 添加关键词</a></div></div><div class="rule-replies"><div class="rule-inner"><h4>自动回复：<span class="send-method">随机发送</span></h4><div class="reply-container"><div class="info">还没有任何回复！</div><ol class="reply-list"></ol></div><hr class="dashed"/><div class="opt"><a class="js-add-reply add-reply-menu" href="javascript:;">+ 添加一条回复</a><span class="disable-opt hide">最多十条回复</span></div></div></div></div></div>';
	...
	$('.js-delete-rule').live('click', function(){
		...
		$.get('/user.php?c=weixin&a=delete_qrcode', {'rid':rid}, function(response){
			golbal_tips(response.err_msg, response.err_code);
			if (response.err_code) {
			} else {
...
//		keyhtml += '</div>';
	...
	$('.btn-save-keyword').live('click', function(){
		...
		$.post('/user.php?c=weixin&a=save_keyword', {'content':content, 'qid':rid, 'kid':kid}, function(response){
			obj.parents('.popover').remove();
			golbal_tips(response.err_msg, response.err_code);
...
//			location.href = '/user.php?c=weixin&a=auto&keyword=' + $(this).val();
	...
	$('.js-delete-it').live('click', function(){
		...
		$.get('/user.php?c=weixin&a=delete_reply', {'qid':rid, 'replyid':replyid}, function(response){
			golbal_tips(response.err_msg, response.err_code);
			if (response.err_code == 0) {
...
//			html += '<input type="hidden" id="content" value="" />';
	...
	$('.js-save-reply').live('click', function(){
		...
		$.post('/user.php?c=weixin&a=save_reply', {'content':content, 'qid':rid, 'replyid':replyid, 'type':type, 'sid':sid}, function(response){
			obj.parents('.popover').remove();
			golbal_tips(response.err_msg, response.err_code);
			if (response.err_code == 0) {
				if (replyid == 0) {
