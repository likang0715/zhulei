		...
		?>
			<div class="content_list">
				<ul class="content_list_ul">
					...
					?>
						<li>
							<a href="<?php echo url_rewrite('goods:index', array('id' => $product['product_id'])) ?>" title="<?php echo htmlspecialchars($product['name']) ?>">
							<div class="content_list_img"><img src="<?php echo $product['image'] ?>">
								<div class="shop_list">
									<form onsubmit="return false">
										<div class="shop_guanzhu">
											<button onclick="userAttention('<?php echo $product['product_id'] ?>', '1',<?php echo $product['store_id']?>)">关注产品</button>
										</div>
										<div class="shop_shoucang">
											<button onclick="userCollect('<?php echo $product['product_id'] ?>', '1',<?php echo $product['store_id']?>)">收藏产品</button>
										</div>
									</form>
								</div>
							</div>
							...
							<div class="content_list_txt js-store-position" data-long="<?php echo $long ?>" data-lat="<?php echo $lat ?>">
								<div class="content_list_day"><?php echo $long ? '请设置您的位置' : '商家未设置位置'; ?> </div>
								<div class="content_list_add"></div>
								<!-- 积分显示 -->
									<?php 
										$points_name = '赠送'.$platform_credit_name.'： 　　　　';
										$points_price = $product['give_points'];
										if($product['open_return_point']){
											$points_price = $product['return_point'];
										}else{
											$points_price = $product['price']*option('credit.platform_credit_rule');
										}
									?>
									<?php if($open_margin_recharge){?>
									<div class="content_list_pice"><?php echo $points_name;?></div>
									<div class="content_list_pice" style="float:right"><?php echo $points_price;?></div>
									<?php }?>
							</div>
							<div class="shop_info">
