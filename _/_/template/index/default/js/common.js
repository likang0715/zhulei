...
//添加收藏
function userCollect(id, type,storeid) {
	var url = "/index.php?c=collect&a=add&id=" + id + "&type=" + type + "store_id=" + storeid;
	$.getJSON(url, function (data) {
		...
		if (type == 1 && data.status == true) {
			try {
				...
				$(".js_goods_collect").attr("onclick", "cancelCollect('" + id + "', '" + type + "', '" + storeid + "')");
			...
			try {
				...
				$(".js_store_collect_click").attr("onclick", "cancelCollect('" + id + "', '" + type + "', '" + storeid + "')");
...
//关注商品
function userAttention(id, type,storeid) {
	var url = "/index.php?c=collect&a=attention&id=" + id + "&type=" + type + "&store_id=" + storeid;
	$.getJSON(url, function (data) {
		if (data.status == false) {
			try {
				if (data.data.error == 'login') {
...
// 非个人中心中的取消收藏
function cancelCollect(id, type,storeid) {
	...
	var url = "/index.php?c=collect&a=cancel&id=" + id + "&type=" + type + "&store_id=" + storeid;
	$.getJSON(url, function (data) {
		if (data.status == false) {
			try {
				if (data.data.error == 'login') {
		...
		if (type == 1 && data.status == true) {
			try {
				...
				$(".js_goods_collect").attr("onclick", "userCollect('" + id + "', '" + type + "', '" + storeid + "')");
			...
			try {
				...
				$(".js_store_collect_icon").html('+')
				$(".js_store_collect_click").attr("onclick", "userCollect('" + id + "', '" + type + "', '" + storeid + "')");
...
function attention_cancel(id, type,storeid) {
var url = "/index.php?c=collect&a=attention_cancel&id=" + id + "&type=" + type + "&store_id=" + storeid;
$.getJSON(url, function (data) {
	showResponse(data);
})
}
...
// 取消收藏
function userCancelCollect(id, type,storeid) {
	var url = "/index.php?c=collect&a=cancel&id=" + id + "&type=" + type + "&store_id=" + storeid;
	$.getJSON(url, function (data) {
		showResponse(data);
	})
...
function tusi(txt, fun) {
	...
	var div = $('<div class="tusi" style="background:rgba(90, 91, 92, 0.8);padding:0px 20px;min-height: 77px;min-width: 270px;position: absolute;left: -1000px;top: -1000px;text-align: center;border-radius:10px; rgba(255, 255, 255, 80)"><span style="color: #ffffff;line-height: 77px;font-size:20px;">' + txt + '</span></div>');
	$('body').append(div);
...
$(function(){
	...
	$(".tuan_sub_search").click(function () {
		var keyword = $('.combobox-input').val();
		if(keyword == ''){
			alert('请输入搜索关键词');
		}else{
			var url = "/index.php?c=tuan&a=tuan_list&keyword=" + keyword;
			location.href = url;
		}
	});
...
function show_activity_win(obj,type){
	...
	switch(type){
		...
		case 'tuan':
			...
			var title = sobj.name;
			var cyrs = sobj.cyrs;
			var p1 = "开团数：  <span> " + cyrs + "</span>";
			
			if (sobj.type == "tuan_team") {
				p1 = "购买数：  <span> " + cyrs + "</span>";
			}
...
$(function() {
	...
	//全站活动的二维码检测7天
	var check_qcode_activity_url = "/index.php?c=activity&a=check_qcode";
	$(".check_activity").hover(function(){
		var check_index = $(".check_activity").index($(this));
		if(!$(this).data("sign")) {
			$(this).data("sign",1);		
			var activity_qcode_starttime = $(this).attr("data-q-start-time");
			var aid = $(this).attr("data-id");
			$.post(
				check_qcode_activity_url,
				{activity_id:aid},
				function(obj) {
					if(obj.code == '0') {
						$(".check_activity .code_img").eq(check_index).attr("src", obj.wx_img) ;	
					}
				},
				'json'
			)
	...
	//全站商铺的二维码检测7天
	var check_qcode_shoper_url = "/index.php?c=store&a=check_qcode";
	$(".check_store_qcode").hover(function(){
		var check_index = $(".check_store_qcode").index($(this));
		if(!$(this).data("sign")) {
			$(this).data("sign",1);		
			var activity_qcode_starttime = $(this).attr("data-q-start-time");
			var sid = $(this).attr("data-id");
			$.post(
				check_qcode_shoper_url,
				{store_id:sid},
				function(obj) {
					if(obj.code == '0') {
						$(".check_store_qcode .code_img").eq(check_index).attr("src", obj.wx_img) ;	
					}
				},
				'json'
			)
