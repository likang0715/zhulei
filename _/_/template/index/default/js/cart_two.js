$(function () {
	...
	$("#cart_add_submit").click(function () {
		...
		if ($(".js-payment_list").size() > 0) {
			...
			if (payment_obj.data("payment") == "peerpay") {
				if ($("#js-peerpay_content").val().length == 0) {
					tusi("请填写代付求助语");
					$("#cart_add_submit").html("确认收货信息");
					is_click = false;
					return false;
				...
				$("#peerpay_type").val($(".js-peerpay_type").find(".order_curn").data("type"));
				$("#peerpay_content").val($("#js-peerpay_content").val());
			}
		} else {
		...
		$("input[name='msg']").val($("input[name='buyer_comment']").val());
	...
	$(".js-peerpay_type li").click(function () {
		if ($(this).hasClass("order_curn")) {
			return;
		}
