$(function(){
	...
	$.getScript("http://api.map.baidu.com/getscript?v=2.0&ak=4c1bb2055e24296bbaef36574877b4e2",function(){
		var map = null;
		var oPoint = new BMap.Point(116.402544,39.915378);
		var setPoint = function(mk,b){
			var pt = mk.getPosition();
			(new BMap.Geocoder()).getLocation(pt,function(rs){
				addComp = rs.addressComponents;
				
				infoWindow = new BMap.InfoWindow('<div class="info-window"><p class="address">'+ addComp.city + addComp.district + addComp.street +'</p><div style="text-align:center;"><a class="ui-btn ui-btn-confirm js-confirm" onclick="setSelect(\''+pt.lat+'\',\''+pt.lng+'\')">确认位置</a></div></div>',{width:300,height:70,title:"",enableMessage:false,message:""});
		...
		$('.shop-map-container .left .place-list li').live('mouseover mouseout click',function(e){			
			...
			if(e.type == 'mouseover'){
				...
				(new BMap.Geocoder()).getLocation(pt,function(rs){
					addComp = rs.addressComponents;
					infoWindow = new BMap.InfoWindow('<div class="info-window"><h3 style="font-size:14px;font-weight:bold;">'+nowDom.data('title')+'</h3><p class="address">'+ nowDom.data('content')+'</p><div style="text-align:center;"><a class="ui-btn ui-btn-confirm js-confirm" onclick="setSelect(\''+nowDom.data('lat')+'\',\''+nowDom.data('lng')+'\')">确认位置</a></div></div>',{width:250,height:(nowDom.data('title').length>15 ? 110 : 90),title : "",enableMessage:false,message:""});
		...
		$('.js-search').click(function(){
			...
			if($('.js-address-input').val() != ''){
				search_val = $('.js-address-input').val();
        ...
        $('#lbs1').change(function(){
            map.centerAndZoom($('#lbs1 :selected').html(),15);
            $('#lbs2').html('<option>选择城市</option>');
            if($(this).val() != ''){
                getCitys('lbs2','lbs1','');
            }
            $('#lbs3').html('<option>选择地区</option>');
        });
        $('#lbs2').change(function () {
            map.centerAndZoom($('#lbs2 :selected').html(),15);
            getAreas('lbs3', 'lbs2', '');
        });
        $("#lbs3").click(function(){
            map.centerAndZoom($('#lbs3 :selected').html(),15);
        });
	});
});
...
function setSelect(lat,lng){
	//layer_tips(-1,'地理位置已确认，正在跳转');
	 layer.msg('地理位置已确认，正在跳转……', 200, -1);
	 $.post(site_url+"/index.php?c=changecity&a=set_location",{"lng":lng,"lat":lat},function(obj){
