$(function () {
	...
	$(".store_check").click(function () {
		...
		if ($(this).attr("checked") == "checked") {
			...
			$(this).closest("tbody").siblings().find("input[type='checkbox']").attr("checked", false);
		} else {
			$(".cart_list_" + store_id + " input[type='checkbox']").attr("checked", false);
		}
		show_cartResponse_by_check();
	});
	
	$(".order-table .j_cartItem").click(function() {
	...
	//勾选操作更新订单信息 js
	function show_cartResponse_by_check() {
		do_change();
...
function do_change() {
	...
	var cart_total = 0;
	var cart_money = 0;
	var cart_money_checked = 0;
	var cart_total_checked = 0;
	$("input[type='text']").each(function () {
		var num = parseInt($(this).val());
		//var price = parseFloat($(this).attr("data-price"));
		var  price= parseInt($(this).attr("data-price")*100)*0.01;
		...
		if($(this).closest("tr").find("input[type='checkbox']").is(':checked')){
			cart_money_checked += parseInt(num * price*100)*0.01;
			//alert(cart_money_checked)
			cart_total_checked += num;
		}

		$("#product_price_total_" + product_id).html(parseInt(num * price*100)*0.01);
	});
	
	
	
	//$(".cart-allPrice").html(" &yen; " + cart_money);
	$(".cart-allPrice").html(" &yen; " + cart_money_checked);
	
	
	$(".cart-nums").html(cart_total_checked);
