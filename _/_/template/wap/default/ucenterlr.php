...
<html class="no-js admin <?php if($_GET['ps']<=320){ ?>responsive-320<?php }elseif($_GET['ps']>=540){ ?>responsive-540<?php }?> <?php if($_GET['ps']>540){ ?> responsive-800<?php } ?>" lang="zh-CN">
	<head>
		...
		<style>
			.order-related .block.block-list.list-vertical>a.link::before{background-size:19px 286px}
			.order-related .block.block-list.list-vertical>a.icon-vip::before{background-size:19px 286px}
		</style>
	</head>
			...
			<div class="content">
				<div class="content-body">
					...
					<div class="custom-level">
						<img class="custom-level-img js-lazy" style="width:100%" src="<?php echo $now_ucenter['bg_pic'];?>"/>
					...
					<div class="order-related">
						...
						<div class="block block-list list-vertical">
							...
							<a class="block-item link clearfix icon-vip" href="./degree.php?id=<?php echo $now_store['store_id'];?>" target="_blank">
								<p class="title-info c-black font-size-14">我的等级</p>
							</a>
							<?php 
							if (!$_SESSION['sync_user']) {
							?>
							...
							<a class="block-item link clearfix icon-guanzhu" href="./my_guanzhu.php?action=goods&store_id=<?php echo $now_store['store_id'];?>" target="_blank">
								<p class="title-info c-black font-size-14">我的关注</p>
							</a>
							<a class="block-item link clearfix icon-collect" href="./my_shoucang.php?action=goods&store_id=<?php echo $now_store['store_id'];?>" target="_blank">
								<p class="title-info c-black font-size-14">我的收藏</p>
							</a>
							<a class="block-item link clearfix icon-return" href="./return.php?id=<?php echo $now_store['store_id'];?>" target="_blank">
								<p class="title-info c-black font-size-14">我的退货</p>
							</a>
							<a class="block-item link clearfix icon-rights" href="./rights.php?id=<?php echo $now_store['store_id'];?>" target="_blank">
								<p class="title-info c-black font-size-14">我的维权</p>
							</a>
						</div>
