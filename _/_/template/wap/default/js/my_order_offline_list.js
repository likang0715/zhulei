$(function() {
	...
	var url = "my_order_offline_list.php";
	getOrder();
	
	$(".js-store_order").click(function () {
		location.href = "store_offline_list.php";
	});
				...
				for (var i in order_list) {
					...
					html += '<li>\
								<div class="shoplisttit manage-shoplisttit">\
									<span class="fl shoplisttit-left">订单号：' + order_list[i].order_no_txt + '</span>' + store_str + '\
					...
					if (order_list[i].status == "7") {
						html += '		<span class="fr shoplist-del manage-shoplist-order logistics-btn js-order_complate" data-url="order.php?action=complate&order_no=' + order_list[i].order_no + '" style="background: #ff7216; color: #fff; border: 0px;">交易完成</span>';
					} else if (order_list[i].status != "4") {
						html += '		<span class="fr shoplist-del manage-shoplist-order logistics-btn" style="background: #CCC; color: #fff; border: 0px;">等待确认</span>';
					}
