...
function getCoupons(type,page2){
    ...
    $.ajax({
        type:"POST",
        url:'store_coupon.php?action=get_usercoupon_list&id='+store_id+"&couponid="+couponid,
        data:'type='+type+'&page='+couponPage+'&sort='+couponSort+"&couponid="+couponid,
        dataType:'json',
            ...
            if(result.err_code){
                ...
                if(result.err_msg.list.length > 0){
                    ...
                    for(var i in result.err_msg.list){
                        ...
                        productHtml +=          '<div>   </div>';
                        productHtml += '<div class="promote-use-state font-size-16" style="width:12px;padding:14px 16px;line-height:20px;"> '+typename+'</div> </div> ';
