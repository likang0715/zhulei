$(function(){
	...
	var checkCart = function(){
		...
		$('.js-total-price').html(parseInt(cartMoney.toFixed(2)));
		if(cartCount){
	...
	$('.js-edit-list').click(function(){
		if(isEdit == false){
			...
			$.each($('.block-item-cart'),function(i,item){
                ...
                html += '<button class="minus response-area-minus" type="button"></button>';
                if (buyer_quota > 0 && buyer_quota >= $(item).data('num')) {
                    ...
                    html += '<button class="plus response-area-plus disabled" type="button" disabled="true"></button>';
					//html += '<button class="plus disabled" type="button" disabled="true"></button><div class="response-area response-area-plus"></div>';
                } else {
                    html += '<input type="text" class="txt" value="' + ($(item).data('skunum') == '0' ? '0' : $(item).data('num')) + '"/>';
                    html += '<button class="plus response-area-plus" type="button"></button>';
					//html += '<button class="plus" type="button"></button><div class="response-area response-area-plus"></div>';
                }
