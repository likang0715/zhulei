...
function skuBuy(product_id,buyType,showCallback){
	...
	$.post('./goodinfo.php',{action: 'getSku',store_id: storeId, product_id: product_id},function(result){
		...
		if(productInfo.minPrice,productInfo.maxPrice) {
			show_maxprice = getFinalPrice(parseInt(productInfo.minPrice),parseInt(productInfo.maxPrice)); 
		} else {
			show_maxprice = 0;
		}
		...
		popCon.find('.js-sku-views .sku-sel-list li').click(function(){
			...
			if(popCon.find('.js-sku-views .sku-sel-list').size() == popCon.find('.js-sku-views .sku-sel-list li.active').size()){
					...
					case 1:
						...
						for(var i in result.err_msg.skuList) {
							if(nowSkuId == result.err_msg.skuList[i].sku_id){
								...
								if(result.err_msg.skuList[i].price) {
									err_msg_price = result.err_msg.skuList[i].price;
								}
								err_msg_price = err_msg_price ? parseInt(err_msg_price) : 0;
								//$('.js-goods-price').html(getFinalPrice(err_msg_price,0));
								
								$('.js-goods-price-jifen').html(getFinalPrice(err_msg_price,0));
								break;
							}
						}
						break;
				...
				if(productInfo.minPrice,productInfo.maxPrice) {
					err_msg_price = productInfo.minPrice,productInfo.maxPrice;
				}
				err_msg_price = err_msg_price ? parseInt(err_msg_price) : 0;
				$('.js-goods-price-jifen').html(getFinalPrice(err_msg_price,0));
			}
