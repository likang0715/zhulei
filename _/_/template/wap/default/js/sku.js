...
if(typeof(after_subscribe_price) == "undefined"){
	var after_subscribe_price = 0,follow = 0;
}
...
function skuBuy(product_id,buyType,showCallback){
	...
	$.post('./goodinfo.php',{action: 'getSku',store_id: storeId, product_id: product_id},function(result){
		if(showCallback) showCallback();
		...
		if(buyType == 5) {
			//预售
			var ysj = "";
			if(typeof(presale_price) != "undefined") {
				
				ysj = '<div style="clear:both" class="current-price pull-left c-black"><span class="price-name pull-left font-size-14 c-orange">预付：￥</span><i class="jprice font-size-14 vertical-middle c-orange">'+presale_price+'</i></div>';				
			}
			
			
			var skuHtml = '<div id="n65dA7sX3X"  class="aaaaaaaa sku-layout sku-box-shadow" style="overflow:hidden;bottom:0px;left:0px;right:0px;visibility:visible;position:absolute;z-index:1100;opacity:1;"><div class="layout-title sku-box-shadow name-card sku-name-card"><div class="thumb"><img src="'+productInfo.image+'" alt="'+productInfo.name+'"></div><div class="detail goods-base-info clearfix"><p class="title c-black ellipsis">'+productInfo.name+'</p><div class="goods-price clearfix"><div class="current-price pull-left c-black"><span class="price-name pull-left font-size-14 c-orange">总价：￥</span><i class="js-goods-price price font-size-14 vertical-middle c-orange">'+getFinalPrice(productInfo.minPrice,productInfo.maxPrice)+'</i></div>'+ysj+'</div></div><div class="js-cancel sku-cancel"><div class="cancel-img"></div></div></div><div class="adv-opts layout-content"><div class="goods-models js-sku-views block block-list block-border-top-none">';
					...
					case 1:
						...
						for(var i in result.err_msg.skuList){
							if(nowSkuId == result.err_msg.skuList[i].sku_id){
								...
								if(result.err_msg.skuList[i].after_subscribe_price >= 1 && follow == 1){
									$('.js-goods-price').html(getFinalPrice(result.err_msg.skuList[i].after_subscribe_price,0));
								}else{
									$('.js-goods-price').html(getFinalPrice(result.err_msg.skuList[i].price,0));
								//	console.log(result.err_msg.skuList[i]);
								}
								break;
							}
						}
		...
		popCon.find('.js-mutiBtn-confirm').click(function(){
			...
			if(result.err_msg.customFieldList.length > 0){
				...
				for(var i in result.err_msg.customFieldList){
					...
					if(nowField.required == '1' && $.trim(fieldDom.val()).length==0){
						...
						if (nowField.field_type == 'id_no') {
							var reg = /(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/;  
							if(reg.test(fieldDom.val()) === false) {
								motify.log('请正确填写身份证号码格式');
								fieldDom.focus();
								return  false;  
							}
						}
			...
			if (buyType == 3) {
				post_data.send_other = 1;
			}
			if(buyType == 5) {
				post_url = "./presale_saveorder.php";
				post_data.presale = 1 ;//标注预售presale
				post_data.presale_id = presale_id;
			}
			...
			$.post(post_url,post_data,function(result){
				...
				if(post_data.isAddCart){
					...
					if($('#global-cart').size()==0){
						var cartObj = $('<div id="right-icon" class="js-right-icon icon-hide no-border" data-count="1"><div class="right-icon-container clearfix"><a id="global-cart" href="./cart.php?id='+storeId+'" class="no-text new showcart"><p class="right-icon-img"></p><p class="right-icon-txt">购物车</p></a></div></div>');
						$('body').append(cartObj);
						dragTool('right-icon');
					}
					isPostNow = false;
                    ...
                    var buy_quantity = parseInt($('.buyer-quota').data('buy-quantity'));
                    var buyer_quota = parseInt(productInfo.buyer_quota);
                    if ((buy_quantity + post_data.quantity) >= buyer_quota) {
						//限购
                        $('.js-bottom-opts').html('<li><a href="javascript:" class="noBuy"><i></i>限购，您已购买' + buyer_quota + '件</a></li>');
                    }
                    $('.buyer-quota').attr('data-buy-quantity', (buy_quantity + post_data.quantity));
				} else if (buyType == 3) {
					window.location.href = './order_friend_confirm.php?order_id=' + result.err_msg;
				} else{
					var param = "";
					if (buyType == 4) {
						param = "&pay_type=peerpay";
					}
