						...
						if(result.err_msg.list.length > 0){
							...
							for(var i in result.err_msg.list){
								...
								if (order.type == "6") {
									...
									str += '<span class="c-orange">预售订单</span>';
								}
								if(order.status<2) {
								...
								for(var ii in order.product_list) {
									var order_product = order.product_list[ii];
									var product_url = "good.php?id=" + order_product.product_id;
									if (order.type == "6") {
										product_url = "/webapp/#/details/" + order.data_id;
									}
									if(order.type == '7') {
								...
								if(order.status<2) {
									...
									if(order.status == 4)  str+='<a class="btn btn-in-order-list" href="'+order.comment_url+"&oid="+order.order_id+"&order_no="+order.order_no+'">评论</a>';
									...
									if(order.status<2) {
										...
										if(order.status == 4 || order.status == 7)  str+='<a class="btn btn-in-order-list" href="'+order.comment_url+"&oid="+order.order_id+"&order_no="+order.order_no+'">评论</a>';
										str+='<a class="btn btn-in-order-list" href="'+order.url+'">详情</a>';
										if (order.status == 7) {
											str+='<a href="javascript:void(0);" class="btn btn-in-order-list btn-orange btn-complate" data-url="'+order.complate_url+'&order_no=' + order.order_no + '">交易完成</a>';
										}
									}								
								}
