...
$(function(){
		...
		$.post('address.php?action=postage',{orderNo:orderNo,address_id:address_id,province_id:province_id},function(result){
            if(typeof(result) == 'object'){
				...
				if (result.err_code == 1001) {
					...
					//$('.js-order-total').html('<p>￥'+sms_total.toFixed(2)+' + ￥'+postage.toFixed(2)+'运费</p><strong class="js-real-pay c-orange js-real-pay-temp">需付：￥'+(sms_total+postage).toFixed(2)+'</strong>');
					$('.js-step-topay').removeClass('hide');
				...
				if($('#selffetch_id').val() != '0' && selffetchEdit == false){
					for(var i in selffetchListJson){
						...
						if(selffetch.pigcms_id == $('#selffetch_id').val()){
							...
							//$('.js-order-total').html('<p>￥'+sms_total.toFixed(2)+' + ￥0.00运费</p><strong class="js-real-pay c-orange js-real-pay-temp">需付：￥'+sms_total.toFixed(2)+'</strong>');
							break;
						}
					}
				}else{
	...
	$('.js-order-address .opt-wrapper .js-edit-address').live('click',function(){
		...
		editAdress(function(){
			...
			if (nowAdress.zipcode == '0') {
				$('#addAdress input[name="zipcode"]').val('');
			} else {
	...
	$('#confirm-pay-way-opts .btn-pay').click(function(){
		...
		//$('.js-msg-container').val()
		var post_data = {payType:payType, orderid:orderid, address_id:address_id, msg:'留言啊发发发'};
		if($('#address_id').size() > 0){
...
/*
        if ($(this).hasClass('go-pay')) {
            $.post('saveorder_by_sms.php?action=go_pay',post_data,function(result){
                if (!result.err_code) {
...
*/
		...
		$.post('unitary_pay_order.php?action=pay',post_data,function(result){
					...
					if ($("#js-total").html() == "0.00") {
						...
						window.WeixinJSBridge.invoke("getBrandWCPayRequest",result.err_msg,function(res){
							...
							if(res.err_msg=="get_brand_wcpay_request:ok"){
								// alert('订单支付成功');
								window.location.href = pay_redirect;
							}else{
...
function resetPrice() {
	...
	var money = unitary_total;
	if (money < 0) {
		money = 0;
	}
		//money.toFixed(2)
	$("#js-total").html(unitary_total);
}
