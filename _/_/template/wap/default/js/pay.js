...
$(function(){
		...
		addAdressDom.find('.js-address-save').click(function(){
			...
			$.post(post_url,post_data,function(result){
				if(result.err_code){
                    ...
                    $('.js-logistics-content').html('<div><div class="block block-form block-border-top-none block-border-bottom-none"><div class="js-order-address express-panel" style="padding-left:0;"><div class="opt-wrapper"><a href="javascript:;" class="btn btn-xxsmall btn-green butn-edit-address" id="addList" style="margin-bottom: 5px;">切换</a><br /><a href="javascript:;" class="btn btn-xxsmall butn-edit-address js-edit-address" style="border-color: #f60;background-color:#f60;color: #fff;">修改</a></div><ul><li><span>'+data.name+'</span>, '+data.tel+'</li><li>'+__alldiv[province][0]+' '+__alldiv[city][0]+' '+area_text+' </li><li>'+data.address+'</li></ul></div></div>');
	...
	var getPostage = function(type){
		if (presale_type == 'presale_first_pay') {
		...
		$.post('address.php?action=postage',{orderNo:orderNo,address_id:address_id,province_id:province_id},function(result){
			if(typeof(result) == 'object'){
				...
				if (result.err_code == 1001) {
					...
					if(presale_type == 'presale_first_pay') {
						$("#js-postage").html('0.00');
						$("input[name='postage_list']").val("0.00");
						supplier_postage_money = 0.00;
					} else {
						$("#js-postage").html(postage.toFixed(2));
						$("input[name='postage_list']").val(result.err_dom.postaage_list);
						
						if (parseFloat(result.err_dom.supllier_postage) < parseFloat(postage)) {
							supplier_postage_money = result.err_dom.supllier_postage;
						} else {
							supplier_postage_money = postage;
						}
					}
	...
	$('#confirm-pay-way-opts .btn-pay').click(function(){
		...
		if ($("#point_2").prop("checked") && $("input[name='platform_point']").val().length > 0) {
			post_data.platform_point = $("input[name='platform_point']").val();
			post_data.platform_point_money = $(".js-platform_point_money span").html();
		}
	...
	// 更改平台积分
	$(".js-platform_point_input").click(function () {
		if ($(this).prop("checked")) {
			$(".js-platform_point_money span").html($(".js-platform_point_use_money").html());
			if ($(".js-platform_point_use_money").html() > 0) {
	...
	// 更改使用平台积分
	$("input[name='platform_point']").blur(function () {
		return;
		if ($(this).val().length == 0) {
			...
			var point = $(this).val();
			if (!/^\d+(\.\d+)?$/.test(point)) {
				motify.log(credit_setting.platform_credit_name + ",请输入合法的数字");
				$(this).val($(this).attr("old-value"));
				$(this).focus();
				return;
			} else if(parseFloat(point) < 0.00) {
				motify.log(credit_setting.platform_credit_name + ',最小为 0.00');
				$(this).val($(this).attr("old-value"));
				$(this).focus();
				return;
			}
			...
			if (parseFloat(point) > parseFloat($(".js-platform_point_max").html())) {
				motify.log('最多只能使用' + $(".js-platform_point_max").html() + "个" + credit_setting.platform_credit_name);
				$(this).val($(this).attr("old-value"));
				$(this).focus();
				return;
			}
			...
			$(".js-platform_point_use_money").html((parseFloat(point) / credit_setting.platform_credit_use_value).toFixed(2));
			$(".js-platform_point_money span").html((parseFloat(point) / credit_setting.platform_credit_use_value).toFixed(2));
...
function setPoint() {
	...
	if (is_platform_point && point_init && $(".js-platform_point_input").prop("checked")) {
		platformPointInit();
		point_init = false;
	}
}
...
function platformPointInit() {
	...
	// 现金比例
	var online_trade_money = parseFloat(credit_setting.online_trade_money);
	if (online_trade_money > 0) {
		//money = money - money * online_trade_money / 100;
	}
	...
	if (max_platform_point < platform_point - postage1 * parseFloat(credit_setting.platform_credit_use_value)) {
		//platform_point = max_platform_point;
	}
...
function resetPrice() {
	...
	if($("#js-presale_discount_money").length > 0) {
		presale_discount_money = parseFloat($("#js-presale_discount_money").html())
	} 
	if(presale_type == 'presale_first_pay') {
		presale_discount_money = 0;
	}
	
	if((presale_type == 'presale_second_pay') || (presale_type == 'presale_first_pay')) {
		var money = sub_total + postage1 - presale_discount_money;
	} else {
		var money = sub_total + postage1 - reward - user_coupon - float_amount - discount_money - point_money_t - platform_money;
	}
	...
	if (money == 0 && platform_money > 0) {
		$('#confirm-pay-way-opts > div').hide();
		$('#confirm-pay-way-opts > .point-pay').show();
	} else {
		$('#confirm-pay-way-opts > div').show();
		$('#confirm-pay-way-opts > .point-pay').hide();
		$('#confirm-pay-way-opts > .disabled-pay').remove();
	}
}
...
function appFromAndroid() {
	...
	// 隐藏支付宝
	$('#confirm-pay-way-opts .btn-pay').each(function () {
