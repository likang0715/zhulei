	...
	$.getScript("http://api.map.baidu.com/getscript?v=2.0&ak=4c1bb2055e24296bbaef36574877b4e2",function(){
		...
		var setPoint = function(mk,b){
			...
			(new BMap.Geocoder()).getLocation(pt,function(rs){
				...
				infoWindow = new BMap.InfoWindow('<div class="info-window"><p class="address">'+ addComp.city + addComp.district + addComp.street +'</p><div style="text-align:center;margin-top:5px;"><a class="ui-btn ui-btn-confirm js-confirm" onclick="setSelect(\''+pt.lat+'\',\''+pt.lng+'\')">确认位置</a></div></div>',{width:270,height:60,title:"",enableMessage:false,message:""});
		...
		if($('#map_long').val() != '' && $('#map_lat').val() != ''){
			...
			function myFun(result){
                var location = JSON.parse(getCookie("Web_user"));
                if(typeof(location.area_name) != "undefined" && location.area_name!=null){
                    map.centerAndZoom(location.area_name);
                }else if(typeof(location.city_name) != "undefined" && location.city_name!=null){
                    map.centerAndZoom(location.city_name);
                }else{
		...
		$('.shop-map-container .left .place-list li').live('mouseover mouseout click',function(e){			
			...
			if(e.type == 'mouseover'){
				...
				(new BMap.Geocoder()).getLocation(pt,function(rs){
					addComp = rs.addressComponents;
					infoWindow = new BMap.InfoWindow('<div class="info-window"><h3 style="font-size:14px;font-weight:bold;">'+nowDom.data('title')+'</h3><p class="address">'+ nowDom.data('content')+'</p><div style="text-align:center;"><a class="ui-btn ui-btn-confirm js-confirm" onclick="setSelect(\''+nowDom.data('lat')+'\',\''+nowDom.data('lng')+'\')">确认位置</a></div></div>',{width:250,height:(nowDom.data('title').length>15 ? 110 : 60),title : "",enableMessage:false,message:""});
...
function setSelect(lat,lng){
	//layer_tips(-1,'地理位置已确认，正在跳转');
	 ...
	 var location_url = "./changecity.php?action=set_location";
	 $.post(location_url,{"lng":lng,"lat":lat},function(obj){
