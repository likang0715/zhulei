...
<html class="no-js admin <?php if($_GET['ps']<=320){echo ' responsive-320';}elseif($_GET['ps']>=540){echo ' responsive-540';} if($_GET['ps']>540){echo ' responsive-800';} ?>" lang="zh-CN">
	<head>
		...
		<title>   <?php echo ($cat_info['cat_name']) ? $cat_info['cat_name'] :  '商品搜索';?></title>
	...
	<body style="<?php if(!empty($storeNav)){ ?>padding-bottom:45px;<?php } ?> -webkit-tap-highlight-color:rgba(0,0,0,0);" >
		<div class="container">
			<div class="header">
						...
						<ul class="js-goods-list sc-goods-list clearfix list size-3">
							...
							?>
										<li class="goods-card goods-list <?php if($i%3 == 1){echo 'big-pic';}else{echo 'small-pic';}?> <?php if($nowGroup['list_style_type'] == 2){echo 'normal';}else{echo 'card';}?>">
											<a href="<?php echo $config['wap_site_url'];?>/good.php?id=<?php echo $value['product_id'];?>&store_id=<?php echo $now_store['store_id'];?>" class="js-goods link clearfix" target="_blank" title="<?php echo $value['name'];?>">
												...
												<div class="info">
													...
													<p class="goods-price">
														<?php if($value['is_present'] == 1) {?>
														<em><span class="point_ico"></span><?php echo $value['price'];?></em>
														<?php } else {?>
