...
<head>
    ...
    <title>切换店铺 - <?php echo $store_detail['name']; ?></title>
...
<div id="myStore" class="reveal-modal alert-header radius" data-reveal="">
    <ul class="side-nav">
        ...
        <li class="second"><a href="./home.php?id=<?php echo $store_detail['store_id']; ?>"><i class="icon-eye-w"></i>进入我的微店</a>
        </li>
    </ul>
</div>
<div class="fixed">
...
<div class="panel memberhead">
    ...
    <div class="header-r">
        <a href="./drp_ucenter.php?a=profile">
            <span class="name"><?php echo $store_detail['name']; ?></span>
            <?php if ($drp_approve) { ?>
            ...
            <span class="header-tip-text">
                <?php if (!empty($store_detail['supplier'])) { ?>
                    供货商：<?php echo $store_detail['supplier']; ?>
                <?php } else { ?>
                    您已成为正式分销商...
                <?php } ?>
            </span>
...
<div class="row count">
    ...
    <div class="small-4 large-3 columns mid">
        <a href="./drp_store.php?a=sales" class="count-a">
            <p class="count-dis-mony"><?php echo $store_detail['sales']; ?></p>
            <span class="count-title">本店销售总额(元)</span>
        </a>
    </div>
    <div class="small-4 large-3 columns mid">
        <a href="./drp_commission.php?a=statistics" class="count-a">
            <p class="count-dis-mony"><?php echo $store_detail['balance']; ?></p>
            <span class="count-title">佣金余额(元)</span>
        </a>
    </div>
</div>
<?php if (!empty($stores)) { ?>
    <div class="panel member-nav">
        <ul class="side-nav">
            <?php foreach ($stores as $key => $store_info) { ?>
                <li <?php if ($key == (count($store_info) - 1)) { ?>class="last"<?php } ?>>
                    <a href="./drp_store.php?a=select&id=<?php echo $store_info['store_id']; ?>">
                        <i class="icon-shop"
                           style="background:url('<?php echo $store['logo']; ?>');background-size: 41px 41px;"></i>
                        <span class="text"><?php echo $store_info['name']; ?></span>
                        <i class="arrow"></i>
                    </a>
                </li>
            <?php } ?>
