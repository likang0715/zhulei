...
<head>
    ...
    <title>提取分红 - <?php echo $store['name']; ?></title>
...
<div class="fixed">
    <nav class="tab-bar">
        <section class="left-small">
            <a href="./dividends_record.php?a=index" class="menu-icon"><span></span></a>
        </section>
        <section class="middle tab-bar-section">
            <h1 class="title">提取分红</h1>
        </section>
    </nav>
</div>
<div class="panel extract">
    <div class="commision-total"><span class="span-title">可提现金额</span><span
            class="number">￥<?php echo $dividends; ?></span><i class="icon-horn"></i></div>
    <div class="horn-text" style="display:;">可提现金额为发放成功且未提现的金额</div>
    ...
    <div class="panel extract-account">
        <ul class="side-nav">
            <li>
                ...
                <span class="bank-info">开户行：</span><?php echo $store['opening_bank']; ?><a href="./drp_commission.php?a=withdraw_account&type=dividends" style="display:inline"><i class="arrow"></i></a><br/>
...
<script type="text/javascript">
    function btnSave() {
        var MaxCashAmount = parseFloat("<?php echo $dividends; ?>");
            ...
            if (CashAmount > 0) {
               if (CashAmount <= MaxCashAmount) {
                    if (CashAmount >= MinaAmountCash) {
                        $.post('./drp_commission.php', {'type': 'dividends_withdrawal', 'bank_id': bank_id, 'bank_card': bank_card, 'bank_card_user': bank_card_user, 'opening_bank': opening_bank, 'amount': CashAmount}, function(data){
                            if (data.err_code == 0) {
                                window.location.href = "./drp_commission.php?a=dividends_withdrawal_result&status=1&amount=" + data.err_msg;
                            } else if (data.err_code == 1002) {
                                ShowMsg(data.err_msg);
                                return false;
                            } else {
                                window.location.href = "./drp_commission.php?a=dividends_withdrawal_result&status=0&amount=" + data.err_msg;
                            }
                        })
                    } else {
