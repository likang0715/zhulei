...
<head>
    ...
    <title><?php if (empty($seller)) { ?>本店<?php } else { ?>分销商<?php } ?>销售额 - <?php echo $store['name'];?></title>
    ...
    <script>
        var store_id    = parseInt("<?php echo !empty($_GET['store_id']) ? intval(trim($_GET['store_id'])) : 0; ?>");
...
<div class="fixed">
    <nav class="tab-bar">
        ...
        <section class="middle tab-bar-section">
            <h1 class="title"><?php if (empty($seller)) { ?>本店<?php } else { ?>分销商<?php } ?>销售额(元)</h1>
        </section>
    </nav>
</div>

<div class="panel bro-yesterday">
    <p><i class="icon-monyg"></i><?php if (empty($seller)) { ?>本店<?php } else { ?>“<?php echo $seller; ?>”<?php } ?>销售额(元)</p>
