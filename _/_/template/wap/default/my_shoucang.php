...
<body>
<?php if(!$_GET['store_id']){?>
<div class="tabber tabber-n2 tabber-double-11 clearfix"> <a <?php if($_GET['action']=='goods'||empty($_GET['action'])){?>class="active"<?php } ?> href="./my_shoucang.php?action=goods">商品</a> <a <?php if($_GET['action']=='store'){?>class="active"<?php } ?> href="./my_shoucang.php?action=store">店铺</a> <a <?php if($_GET['action']=='chahui'){?>class="active"<?php } ?> href="./my_shoucang.php?action=chahui">茶会</a></div>
<?php }else{?>
<div class="tabber tabber-n2 tabber-double-11 clearfix"> <a class="active" href="./my_shoucang.php?action=goods&store_id=<?php echo $_GET['store_id']?>" style="width:100%">商品</a> <a <?php if($_GET['action']=='chahui'){?>class="active"<?php } ?> href="./my_shoucang.php?action=chahui">茶会</a> </div>
<?php } ?>

<div class="wx_wrap"><?php if(is_array($new_product_list)){ ?>
	<ul class="mod_list recent" id="recentList">
		<?php foreach($attention_list as $v){?>
		<li class="hproduct"> <a href="/wap/good.php?id=<?php echo $v['dataid']; ?>&store_id=<?php echo $v['store_id']?>">
			...
			<p><span style="cursor:pointer" class="cancel_guanzhu" onClick="cancel_goods($(this),'cancel',<?php echo $v['type']; ?>,<?php echo $v['store_id']?>)" product-id="<?php echo $v['dataid']?>">取消收藏</span></p>
			</li>
		<?php } ?>
	</ul>
<?php } ?><?php if(is_array($store_list)){ ?>
	<ul class="mod_list recent" id="recentList">
		<?php foreach($store_list as $v){ ?>
		<li class="hproduct"> <a href="<?php echo $v['url'];?>">
			<div class="list_inner">
				<div class="photo"><img src="<?php echo $v['logo'];?>"/></div>
				<div class="info">
					<p class="title"><?php echo $v['name'];?></p>
				</div>
				<div class="time" style="white-space:nowrap;"><?php echo time_tran(date('Y-m-d H:i:s',$new_attention_list[$v['store_id']]['add_time']));?></div>
			</div>
			</a> 
			<p><span style="cursor:pointer" class="cancel_guanzhu" onClick="cancel_goods($(this),'cancel',<?php echo $new_attention_list[$v['store_id']]['type']; ?>,<?php echo $v['store_id']?>)" product-id="<?php echo $v['store_id']?>">取消收藏</span></p>
			</li>
		<?php } ?>
	</ul>
<?php } ?>
<?php if(is_array($chahui_list)){ ?>
	<ul class="mod_list recent" id="recentList">
		<?php foreach($chahui_list as $v){ ?>
		<li class="hproduct"> <a href="<?php echo $v['url'];?>">
			<div class="list_inner">
				<div class="photo"><img src="<?php echo $v['logo'];?>"/></div>
				<div class="info">
				...
				<div class="time" style="white-space:nowrap;"><?php echo time_tran(date('Y-m-d H:i:s',$new_attention_list[$v['pigcms_id']]['add_time']));?></div>
			</div>
			</a> 
			<p><span style="cursor:pointer" class="cancel_guanzhu" onClick="cancel_goods($(this),'cancel',<?php echo $new_attention_list[$v['pigcms_id']]['type']; ?>,<?php echo $v['store_id']?>)" product-id="<?php echo $v['pigcms_id']?>">取消收藏</span></p>
			</li>
		<?php } ?>
	</ul>
<?php } ?>
...
function cancel_goods(obj,sMethod,type,store_id){
	var product_id=obj.attr('product-id');
	//var url = "/index.php?c=collect&a="+sMethod+"&id=" + product_id + "&type="+type;
	var url = "/wap/my_shoucang.php?action=collect_cancel&id=" + product_id + "&type="+type+'&store_id='+store_id;
