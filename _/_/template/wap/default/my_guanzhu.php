...
<div class="tabber tabber-n2 tabber-double-11 clearfix"> <a <?php if($_GET['action']=='goods'||empty($_GET['action'])){?>class="active"<?php } ?> href="./my_guanzhu.php?action=goods">商品</a> <a <?php if($_GET['action']=='store'){?>class="active"<?php } ?> href="./my_guanzhu.php?action=store">店铺</a></div>
<?php }else{?>
<div class="tabber tabber-n2 tabber-double-11 clearfix"> <a class="active" href="./my_guanzhu.php?action=goods&store_id=<?php echo $_GET['store_id']?>" style="width:100%">商品</a> </div>
<?php } ?>
<div class="wx_wrap"><?php if(is_array($new_product_list)){ ?>
	<ul class="mod_list recent" id="recentList">
		<?php foreach($attention_list as $v){?>
		<li class="hproduct"> <a href="/wap/good.php?id=<?php echo $v['data_id']; ?>&store_id=<?php echo $v['store_id']?>">
			...
			<p><span style="cursor:pointer" class="cancel_guanzhu" onClick="cancel_goods($(this),'attention_cancel',<?php echo $v['data_type']; ?>,<?php echo $v['store_id']?>)" product-id="<?php echo $v['data_id']?>">取消关注</span></p>
			</li>
		<?php } ?>
	</ul>
<?php } ?>
...
<?php if(is_array($store_list)){ ?>
	<ul class="mod_list recent" id="recentList">
		...
		<li class="hproduct"> <a href="<?php echo $v['url'];?>">
			...
			<p><span style="cursor:pointer" class="cancel_guanzhu" onClick="cancel_goods($(this),'attention_cancel',<?php echo $new_attention_list[$v['store_id']]['data_type']; ?>,<?php echo $v['store_id']?>)" product-id="<?php echo $v['store_id']?>">取消关注</span></p>
			</li>
		<?php } ?>
	</ul>
<?php } ?>
...
function cancel_goods(obj,sMethod,type,store_id){
	...
	var url = "/wap/my_guanzhu.php?action=attention_cancel&id=" + product_id + "&type="+type+'&store_id='+store_id;
