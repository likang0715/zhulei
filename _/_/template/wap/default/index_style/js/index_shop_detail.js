...
function getShops(){
	...
	$.ajax({
		type:"POST",
		url:'index_ajax.php?action=get_shop_list',
		data:'keyword='+keyword+'&key_id='+key_id+'&tag_id='+tag_id+'&page='+productPage+'&sort='+productSort + "&prop=" + prop,
		dataType:'json',
			...
			if(result.err_code){
				...
				if(result.err_msg.list.length > 0){
					...
					for(var i in result.err_msg.list){
						var store = result.err_msg.list[i];
						productHtml+= '<div class="hproduct '+(productShowRows ? '' : 'item_long_cover')+'"> <a href="'+store.url+'"><p class="cover"><img src="'+store.logo+'"></p><p class="fn">'+store.name+'</p><p class="prices" style="display:"><strong><em> </em></strong></p><p class="sku"><span class="comment_num '+(store.sales=='0' ? 'hide' : '')+'">收藏数 <span>'+store.collect+'</span></span>&nbsp;/&nbsp;<span class="comment_num" style="float:none">关注数 <span>'+store.attention_num+'</span></span></p></a></div>';
					}
		...
		error:function(){
			$('.wx_loading2').hide();
			motify.log('店铺读取失败，<br/>请刷新页面重试',0);
		}
	});
}
