...
$(function(){
	...
	$('#topSearchTxt').focus(function(){
		...
		if(e.keyCode == 13){
			if(val.length > 0){
				window.location.href = './nearweidian.php?domain='+encodeURIComponent(domain)+'&keyword='+encodeURIComponent($(this).val());
			}else{	
				motify.log('请输入搜索关键词');
			}
		}
	...
	$('#topSearchCbtn').click(function(){
		$('#topSearchTxt').val('');
		$('#topSearchClear').hide();
		window.location.href = './nearweidian.php?domain='+encodeURIComponent(domain);
	});
	
	
	$('.wx_wrap').css({'min-height':$(window).height(),'border-bottom':'none'});
...
// $(function(){
	// user_lat = '31.808544250149254';
	// user_long = '117.22682535010125';
	// get_near_shops();
// });
/** 
* 获取地址位置失败[暂不处理] 
...
function get_near_shops(){
	$.post('./index_ajax.php?action=butt_nearshops_search',{'domain':domain,'keyword':keyword,'long':user_long,'lat':user_lat},function(result){
		if(result.err_code){
			...
			if(result.err_msg){
				...
				arroundErrorTips('没有找到与 <b>'+keyword+'</b> 相关的微店！');
			}
		}
	});
}
