			...
			if(result.err_code){
				...
				if(result.err_msg.list){
					...
					for(var i in result.err_msg.list){
						...
						var points_price = product.give_points;
						if(product.open_return_point > 0){
							points_price = product.return_point;
						}else{
							points_price = product.price * credit_platform_credit_rule;
						}
						var points_html = '<p class="fn" style="color:#E4393C;font-size:12px;">'+points_name+' '+points_price+'</p>';
						if(open_margin_recharge <= 0){
							points_html = '';
						}
						
						productHtml+= '<div class="hproduct '+(productShowRows ? '' : 'item_long_cover')+'"> <a href="./good.php?id='+product.product_id+'&platform=1"><p class="cover"><img src="'+product.image+'"></p><p class="fn">'+product.name+'</p><p class="prices"><strong><em>¥'+product.price+'</em></strong></p><p class="sku"><span class="comment_num '+(product.sales=='0' ? 'hide' : '')+'">销量 <span>'+product.sales+'</span></span>&nbsp;<span class="stock hide">无货</span></p>'+points_html+'</a></div>';
					}
