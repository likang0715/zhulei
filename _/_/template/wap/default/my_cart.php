                ...
                <div id="cart-container" class="block block-order">
					<?php if(!empty($now_store_cart)){ ?>
						<div class="header js-list-header">
							<span>店铺：<?php echo $now_store_cart['store_name']?></span>
						...
						<ul class="list block block-list block-list-cart block-border-none">
							<?php foreach($now_store_cart['cart_list'] as $value){ ?>
									<li class="block-item block-item-cart relative clearfix item-<?php echo $value['pigcms_id'];?>" data-id="<?php echo $value['pigcms_id'];?>" data-num="<?php echo $value['pro_num']?>" data-price="<?php echo floatval($value['pro_price'])?>" data-skuid="<?php echo $value['sku_id'];?>" data-proid="<?php echo $value['sku_id'];?>" data-skunum="<?php echo $value['sku_num'];?>" data-status="<?php echo $value['status'];?>">
							...
							<div class="block block-cart block-order block-order-team">
								...
								<div class="goods-imgs">
									<?php foreach($value['cart_list'] as $cart_value){ ?>
										<img width="60" height="60" class="js-view-image" src="<?php echo $cart_value['image'] ?>"/>
									<?php } ?>
								</div>
							</div>
						</a>
