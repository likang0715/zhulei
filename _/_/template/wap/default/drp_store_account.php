...
<?php if(!$phone) {?>
    <ul class="side-nav">
    <li><a href="javascript:;"><span class="text">登录地址：<?php echo $admin_url; ?></span></a></li>
        <li><a href="javascript:;"><span class="text">登录账户：<input id="reg_phone" class="input-weak input_phone" type="tel" placeholder="手机号" name="phone" value="" /></span></a></li>
        <li ><a href="javascript:;"><span class="text">初始密码：<input id="reg_txt_password" class="input-weak kv-k" type="password" name="password" placeholder="6位以上的密码" /></span></a></li>
        <li class="last"><span class="button [radius red round]" id="login_submit" style="padding: 0;margin-top:10px;margin-bottom: 10px;">绑定帐号</span></li>
    <script type="text/javascript">

	$('#login_submit').click(function(){
		var phone=$('input[name="phone"]').val();
		var password=$('input[name="password"]').val();
		var store_id="<?php echo $_SESSION['tmp_store_id']?>";
		$.post('./login.php?action=complete_info',{'phone':phone,'password':password,'store_id':store_id},function(data){
			alert(data['err_msg']);
			if(data['err_code']==0){
      ...
      <?php if (!$password) { ?> <li class="last"><a href="javascript:;"><span class="text">初始密码：<?php echo $phone; ?></span></a></li><?php } ?>
        
        <?php if ($password) { ?><li class="last"><a href="./drp_store.php?a=reset_pwd" class="button [radius red round]" style="padding: 0;margin-top:10px;margin-bottom: 10px;">重置为初始密码</a></li><?php }?>
        </ul>
