...
>         <style type="text/css">            .green {                color:green;            }            .red {                color:red;            }        </style>		<div class="mainbox">			<div id="nav" class="mainnav_title">				<ul>					<a href="<?php echo U('Express/index');?>" class="on">快递公司列表</a>|					<a href="javascript:void(0);" onclick="window.top.artiframe('<?php echo U('Express/add');?>','添加快递公司',520,250,true,false,false,addbtn,'add',true);">添加快递公司</a>				</ul>			</div>			<form name="myform" id="myform" action="" method="post">				<div class="table-list">					<table width="100%" cellspacing="0">						<thead>							<tr>								<th>编号</th>								<th>公司名称</th>								<th>公司代码</th>								<th>公司网址</th>								<th>添加时间</th>								<th>状态</th>								<th class="textcenter">操作</th>							</tr>						</thead>						<tbody>							<?php if(is_array($expresses)): if(is_array($expresses)): $i = 0; $__LIST__ = $expresses;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$express): $mod = ($i % 2 );++$i;?><tr>										<td><?php echo ($express["pigcms_id"]); ?></td>										<td><?php echo ($express["name"]); ?></td>										<td><?php echo ($express["code"]); ?></td>										<td><a href="<?php echo ($express["url"]); ?>" target="_blank"><?php echo (rtrim($express["url"],'/')); ?></a></td>										<td><?php echo (date('Y-m-d H:i:s',$express["add_time"])); ?></td>										<td><?php if($express['status']): ?><span class="green">启用</span><?php else: ?><span class="red">关闭</span><?php endif; ?></td>										<td class="textcenter"><a href="javascript:void(0);" onclick="window.top.artiframe('<?php echo U('Express/edit',array('id'=>$express['pigcms_id'],'frame_show'=>true));?>','查看详细信息',520,250,true,false,false,false,'add',true);">查看</a> | <a href="javascript:void(0);" onclick="window.top.artiframe('<?php echo U('Express/edit',array('id'=>$express['pigcms_id']));?>','编辑快递公司信息',520,250,true,false,false,editbtn,'add',true);">编辑</a> | <a href="javascript:void(0);" class="delete_row" parameter="id=<?php echo ($express["pigcms_id"]); ?>" url="<?php echo U('Express/del');?>">删除</a></td>									</tr><?php endforeach; endif; else: echo "" ;endif; ?>								<tr><td class="textcenter pagebar" colspan="8"><?php echo ($page); ?></td></tr>							<?php else: ?>								<tr><td class="textcenter red" colspan="8">列表为空！</td></tr><?php endif; ?>						</tbody>					</table>				</div>			</form>		</div>	</body>
</html>