...
<div class="mainbox">
	<div id="nav" class="mainnav_title">
		<ul>
			<a href="<?php echo U('Store/diyAttestation');?>" class="on">认证自定义表单</a>
		</ul>
	</div>
				...
				<?php foreach ($diy_attestation_list as $v) { ?>
						...
						<tr>
							...
							<td>
								...
								<a href="javascript:void(0)" onClick="diyRow_del($(this),<?php echo $v['id'];?>)" class="del_btn">
									<img src="<?php echo ($static_path); ?>images/icon_delete.png" width="18" title="删除" alt="删除" />
								</a>
							</td>
						</tr>
					...
					<?php } else { ?>
						<tr>
							...
							<td class="validate_type">
								<textarea name="field-type-str[]" rows="4" cols="30"><?php  parse_str($v['type'],$val_str); $str=''; foreach (explode('|',$val_str['value']) as $val) { $str.=end(explode(':',$val)).','; } echo rtrim($str,',') ?></textarea>
