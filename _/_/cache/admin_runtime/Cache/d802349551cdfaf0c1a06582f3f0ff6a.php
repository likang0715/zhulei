	...
	<script type="text/javascript">
		$(function() {
			...
			$('.status-enable-hot > .cb-enable').click(function(){
				if (!$(this).hasClass('selected')) {
					...
					$.post("<?php echo U('Product/comment_status'); ?>",{'status': 1, 'id': comment_id}, function(data){})
				}
			})
			$('.status-disable-hot > .cb-disable').click(function(){
				if (!$(this).hasClass('selected')) {
					var comment_id = $(this).data('id');
					if (!$(this).hasClass('selected')) {
						$.post("<?php echo U('Product/comment_status'); ?>", {'status': 0, 'id': comment_id}, function (data) {})
					}
				}
			})	
		...
		<div class="mainbox">
			<div id="nav" class="mainnav_title">
				<ul>
					<a href="<?php echo U('Product/comment');?>" class="on">商品评论列表</a>
				</ul>
			</div>
			<table class="search_table" width="100%">
				<tr>
					<td>
						<form action="<?php echo U('Product/comment');?>" method="get">
					...
					<table class="tables" width="100%" cellspacing="0">
						<thead>
							<tr>
								<th width="4%">删除</th>	
								<th width="12%">产品信息</th>
								<th width="14%" class="textcenter">评论人信息</th>
								<th width="12%">评论的标签</th>
								<th width="20%">评论内容</th>
								<th width="10%">评论图片</th>
								<!--th>分组</th-->
								<th width="8%">当前审核状态</th>
								<th width="10%">评论时间</th>
						...
						<tbody>
							<?php if(is_array($comments)): if(is_array($comments[comment_list])): $k = 0; $__LIST__ = $comments[comment_list];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$comment): $mod = ($k % 2 );++$k;?><tr>
										...
										<?php else: ?> 
										 <td class="first_td">
										 	<a url="<?php echo U('Product/comment_del', array('comment_id' => $comment['id'],'delete'=>1)); ?>"  class="delete_row"><img src="<?php echo ($static_path); ?>images/icon_delete.png" width="18" title="删除" alt="删除" /></a> 
										 </td><?php endif; ?> 
										<td>
										...
										<td class="textcenter">
										<div style="height:65px;display:line-block">
											<img src="<?php echo $comments['user_list'][$comment['uid']]['avatar'] ?>" width="60"/>
										</div>
										<div>
											昵称： <?php echo $comments['user_list'][$comment['uid']]['nickname'] ?><br/>(ID:<?php echo $comments['user_list'][$comment['uid']]['uid'] ?>)
										 </div>
										</td>
										<td>
										<ul>
										...
										<td><?php if($comment['content']){echo strip_tags($comment['content']);} ?>&nbsp;</td>
