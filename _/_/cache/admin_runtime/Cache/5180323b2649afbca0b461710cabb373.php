...
<div class="mainbox">
	<div id="nav" class="mainnav_title">
		<ul>
			<a href="<?php echo U('Admin/index');?>" class="on">后台管理员列表</a>|
			<a href="javascript:void(0);" onclick="window.top.artiframe('<?php echo U('Admin/admin_add');?>','添加管理员',700,240,true,false,false,addbtn,'add',true);">添加后台管理员</a>
		</ul>
	</div>
			...
			<td>
				<form action="<?php echo U('Admin/index');?>" method="get">
					...
					<select name="type">
						...
						<?php if(is_array($admin_type_list)): $i = 0; $__LIST__ = $admin_type_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if($_GET['type'] == $vo['id']): ?>selected="selected"<?php endif; ?>><?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
					</select>
			...
			<table width="100%" cellspacing="0">
				...
				<tbody>
					<?php if(is_array($admin_list)): if(is_array($admin_list)): $i = 0; $__LIST__ = $admin_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
								...
								<td class="textcenter">
									<?php if($vo["type"] != 0): ?><a href="javascript:void(0);" onclick="window.top.artiframe('<?php echo U('Admin/admin_edit',array('id'=>$vo['id']));?>','编辑管理员',620,260,true,false,false,editbtn,'edit',true);">编辑</a><?php endif; ?>
								</td>
