...
class comment_model extends base_model{
	/**
	 * 返回三种评论满意的总数量
	 * 返回数组，total => '总量', t1 => '不满意数', t2 => '一般数', t3 => '满意数', t4 => '有图的数量'
	 */
	public function getCountList($where) {
		$where['status'] = 1;
		$where['delete_flg'] = 0;
		$count = $this->db->field('count(*) as total, sum(if(score <= 2, 1, 0)) as t1, sum(if(score = 3, 1, 0)) as t2, sum(if(score > 3, 1, 0)) as t3, sum(if(has_image > 0, 1, 0)) as t4')->where($where)->find();
		return $count;
	}
	...
	public function getCount($where) {
		$comment_count = $this->db->field('count(1) as count')->where($where)->find();
	...
	public function getList($where, $order_by = '', $limit = 0 , $offset = 0, $is_attachment = false) {
		...
		if ($is_attachment) {
			...
			if ($comment_id_arr) {
				...
				$comment_attachment_list = M('Comment_attachment')->getList($where);
				
				foreach ($comment_attachment_list as $key => $comment_attachment) {
					$return_comment_list[$key]['attachment_list'] = $comment_attachment;
				}
				
				$comment_tag_list = M('Comment_tag')->getListName($comment_id_arr);
	...
	public function satisfaction($where) {
		$data = $this->db->field('count(1) as count, sum(if(score > 3, 1, 0)) as pass')->where($where)->find();
	...
	public function delete($where) {
		$this->db->where($where)->data(array('delete_flg' => 1))->save();
	}
}