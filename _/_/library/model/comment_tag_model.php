 ...
 * 评论TAGmodel
 */
class comment_tag_model extends base_model{
	/**
	 * 返回相应TAG数量
	...
	public function getCountList($where) {
		$where['status'] = 1;
		$where['delete_flg'] = 0;
		$count_list = $this->db->field('count(tag_id) as count, tag_id')->where($where)->group('tag_id')->select();
	...
	public function getListName($cid_arr) {
		...
		$comment_tag_list = $this->db->query($sql);
