 ...
 */
	class product_sku_model extends base_model {
		...
		//添加商品库存信息
		public function add($product_id, $skus,$tmp_sku=array()) {
			$tmp_skus = array();
			foreach ($skus as $key=>$sku) {
				$data = array(
					...
					'price' => $sku['price'],
					'return_point' => $sku['point'] + 0,
					'wholesale_sku_id' => count($tmp_sku) > 0 ? $tmp_sku[$key]['sku_id'] : '',
				...
				$tmp_skus[] = array(
					...
					'properties' => $sku['properties'],
					'price'	  => $sku['price'],
					'return_point' => $sku['return_point'] + 0,
					'after_subscribe_price'	  => $sku['after_subscribe_price'],
					'weight' => $sku['weight'],
					'quantity'   => $sku['quantity'],
					'code'	   => $sku['code']
        ...
        //复制商品库存信息
        public function supplier_to_wholesale($product_id, $skus) {
            $tmp_skus = array();
            foreach ($skus as $key=>$sku) {
                $data = array(
                    'product_id' => $product_id,
		...
		public function edit($product_id, $skus) {
			...
			foreach ($skus as $sku) {
                ...
                $this->db->where(array ('sku_id'=>$sku['sku_id'],'product_id' => $product_id))->data(array (
                    'quantity' => $sku['quantity'],
                    'price' => $sku['price'],
                	'return_point' => $sku['point'] + 0,
					'after_subscribe_price' => $sku['after_subscribe_price'],
        ...
        public function wholesaleUpdate($product_id, $skuId, $skus) {
            $tmp_skus = array();
            foreach ($skus as $key=>$sku) {
                $this->db->where(array('wholesale_sku_id'=>$sku['sku_id']))->data(array('quantity' => $sku['quantity'],'weight' => $sku['weight']))->save();
                $tmp_skus[] = array(
                    'sku_id'	 => $skuId[$key],
                    'properties' => $sku['properties'],
                    'price'	  => $sku['price'],
        ...
        //同步修改经销商品库存信息
        public function wholesaleSkus($product_id, $skus) {
            foreach ($skus as $sku) {
                $data = array(
                    'sale_min_price' => !empty($sku['sale_min_price']) ? $sku['sale_min_price'] : 0,
                    'price' => !empty($sku['sale_min_price']) ? $sku['sale_min_price'] : 0,
                ...
                $this->db->where(array('wholesale_sku_id' =>$sku['sku_id'],'product_id' => $product_id))->data($data)->save();
            }
        }


        public function wholesale_Skus($product_id, $skus) {
            foreach ($skus as $sku) {

                $this->db->where(array ('wholesale_sku_id'=>$sku['sku_id'],'product_id' => $product_id))->data(array (
                    'quantity' => $sku['quantity'],
                    'price' => $sku['price'],
                    'return_point' => $sku['point'] + 0,
                    'after_subscribe_price' => $sku['after_subscribe_price'],
