...
class product_category_model extends base_model {
	...
	public function getAllCategory($limit="",$is_show_son=false) {
		...
		$category_list = $this->db->where($where)->order('`cat_fid` asc,`cat_sort` ASC,`cat_id` ASC')->limit($limits)->select();
		if($is_show_son) {
			...
			foreach($category_list as $k=>$v) {
				...
				if ($v['cat_wap_banner']) {
					$v['cat_wap_banner'] = getAttachmentUrl($v['cat_wap_banner']);
				}
				if ($v['cat_indiana_pic']) {
					$v['cat_indiana_pic'] = getAttachmentUrl($v['cat_indiana_pic']);
				}
				...
				if (is_array($levelArray)) {
					foreach($levelArray as $key => $value) {
						...
						if ($value['cat_wap_banner']) {
							$levelArray[$key]['cat_wap_banner'] = getAttachmentUrl($value['cat_wap_banner']);
						}
					}
				}
				$v['larray'] = $levelArray;
			...
			foreach ($category_list as $value) {
				...
				if ($value['cat_wap_banner']) {
					$value['cat_wap_banner'] = getAttachmentUrl($value['cat_wap_banner']);
				}
	...
	public function getCategories($wheres =array(),$order="") {
		if(empty($order)){
			$order 	= 'cat_sort ASC,cat_id ASC,cat_path ASC';
		}
		$categories = $this->db->where($wheres)->order($order)->select();
		return $categories;
	}
