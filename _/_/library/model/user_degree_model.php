 ...
 * create by：Simon
 */
class user_degree_model extends base_model{
	
	/*更新表积分*/
	public function updating($store_id,$uid) {
	...
	public function getUserDegree($uid,$store_id) {
		...
		//获取用户在店铺积分信息
		$user_point_info = M('Store_user_data')->getpoints_by_storeid($uid,$store_id);
		if($user_point_info['point']) {
			 $return = $this->db->where("store_id='".$store_id."' and points_limit <= '".$user_point_info['point']."' ")->order("points_limit desc")->limit(1)->find();
		}
