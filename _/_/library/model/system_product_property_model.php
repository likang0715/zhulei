...
class system_product_property_model extends base_model{
	...
	public function get_list($property_type_id= array()){
		...
		if(count($property_type_id)) { //查询指定属性类别的属性
			...
			$list = $this->db->order('sort ASC,pid ASC')->where($map)->select();

		} else {
			$list = $this->db->order('sort ASC,pid ASC')->select();
		}
	...
	public function getPropertyAndValue($cat_id_arr) {
		...
		$property_list = $this->db->where(array('pid' => array('in', $cat_id_arr)))->order('sort ASC,pid ASC')->select();
