...
class weixin_bind_model extends base_model
{
	/**
	 * @param $store_id
	 * @param bool $get_component_access_token
	 * @return array|mixed
	 */
	...
	public function get_component_access_token(){
		import('source.class.Http');
		$component_access_tokenName = 'system_component_access_token';
		$result = S($component_access_tokenName);

		if ($result && $result[0] > time()) {
			return array('errcode' => 0,'component_access_token'=>$result[1]);
		} else {
			...
			if ($result['errcode']) {
				...
				return array('errcode' => 0,'component_access_token'=>$result['component_access_token']);
			}
		}
	}
	...
	public function getByStoreid($store_id) {
		return	$this->db->where(array("store_id" => $store_id))->find();
	}
}
?>