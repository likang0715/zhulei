...
class tuan_controller extends base_controller {
	...
	public function index() {
		...
		if ($store['open_nav'] && !empty($store['use_nav_pages'])) {
			...
			if (in_array('7', $use_nav_pages_arr)) {
				if ($this->is_app) {
					$store_nav = M('Store_nav')->getStoreNav($store['store_id']);
					
					if(!empty($nav) && !empty(unserialize($store_nav['data']))) {
	...
	public function order() {
		...
		if ($type == 1) {
			...
			$tuan_config = D('Tuan_config')->where(array('tuan_id' => $tuan_id, 'discount' => array('<', 10)))->order('discount DESC')->find();
		}
	...
	public function my_tuan() {
		...
		// 所需字段
		$field = "o.order_id, o.order_no, o.total, o.pro_num, o.status as o_status, o.data_money, tt.team_id, tt.tuan_id, tt.uid, tt.type, tt.item_id, tt.status, t.product_id, p.name, p.image";
