...
class account_controller extends base_controller{
	...
	public function buysms(){
		...
		if(IS_POST) {
			...
			$data['sms_amount'] = $_POST['sms_amount'];
			if(!preg_match('/^[1-9]{1}[0-9]{3,}$/',$data['sms_amount'])) {
				json_return(2,'购买条数异常！');
			}
			...
			$sms_count_price = $data['sms_amount'] * $sms_price * 0.01;
			
			$order_no = 'SMSPAY_' . date('YmdHis') . mt_rand(100000,999999);
