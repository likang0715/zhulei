...
class weixin_controller extends base_controller
{
	...
	public function save_info(){
		...
		$status = D('Weixin_bind')->data(array('hurl'=>$hurl,'qrcode_url'=>$qrcode_url))->where($where)->save();
		if($status) {
			json_return(0,'操作成功');
		}else{
			json_return(1,'操作失败');
	...
	public function load()
	{
		...
		$temp = array();
		foreach ($list as $l) {
			$lt_ids_arr = unserialize($l['it_ids']);
	...
	public function one()
	{
		...
		if ($data = D('Source_material')->where(array('pigcms_id' => $pigcms_id, 'store_id' => $this->store_session['store_id']))->find()) {
			...
			$image_text['cover_pic'] = getAttachmentUrl($image_text['cover_pic']);
	...
	public function multi()
	{
		...
		if ($data = D('Source_material')->where(array('pigcms_id' => $pigcms_id, 'store_id' => $this->store_session['store_id']))->find()) {
			...
			$contents  = array();
			foreach ($image_text as $k=>$t) {
				$pic 	= getAttachmentUrl($t['cover_pic']);
				$content= html_entity_decode($t['content']);
				$image_text[$k]['cover_pic'] = $pic;
				$image_text[$k]['content'] =  $content;
				$t['id'] = $t['pigcms_id'];
				$t['cover_pic'] = $pic;
				$t['content'] = $content;
	...
	public function fetch_wentu()
	{
		...
		if ($data = D('Source_material')->where(array('pigcms_id' => $pigcms_id, 'store_id' => $this->store_session['store_id']))->find()) {
			...
			if ($data['type'] == 0) {
				...
				$image_text = D('Image_text')->where(array('pigcms_id' => array('in', $it_ids)))->order('pigcms_id asc')->select();
				foreach ($image_text as $key=>&$val) {
					$image_text[$key]['cover_pic'] = getAttachmentUrl($val['cover_pic']);
	...
	public function save_image_text()
	{
		...
		$data['cover_pic'] = getAttachment($data['cover_pic']);
	...
	public function save_multi_image_text()
	{
		...
		foreach ($data as $d) {
			...
			$d['cover_pic'] = getAttachment($d['cover_pic']);
			...
			if ($d['id'] && $pigcms_id) {
				$d['pigcms_id'] = $d['id'];
				unset($d['id']);
				if (D('Image_text')->where(array('pigcms_id' => $d['pigcms_id'], 'store_id' => $this->store_session['store_id']))->data($d)->save()) {
					$ids[] = $d['pigcms_id'];
				} else {
					$flag = true;
					break;
	...
	public function get_wentu()
	{
		...
		foreach ($image_text as $txt) {
			...
			$txt['cover_pic'] = getAttachmentUrl($txt['cover_pic']);
			if (!isset($result[$temp[$txt['pigcms_id']]['pigcms_id']])) {
				$result[$temp[$txt['pigcms_id']]['pigcms_id']] = $temp[$txt['pigcms_id']];//$txt;
			}
			$result[$temp[$txt['pigcms_id']]['pigcms_id']]['list'][] = $txt;
...
// 				$v_list[$re['cid']] = $re['rid'];//内容id 对应规则id
	...
	public function save_keyword()
	{
		...
		if($qid && empty($rid)){
			$rule = D('Qrcode')->where(array('store_id' => $this->store_session['store_id'], 'pigcms_id' => $qid))->find();
			}else{
		...
		if ($keyword = D('Keyword')->where("`store_id`='{$this->store_session['store_id']}' AND `content`='{$content}' AND `pigcms_id`<>'{$kid}'")->find()) {
			...
			if($qid && empty($rid)){
			$count = D('Keyword')->where(array('store_id' => $this->store_session['store_id'], 'q_id' => $qid))->count('pigcms_id');
				...
				if($qid && empty($rid)){
			$kid = D('Keyword')->data(array('store_id' => $this->store_session['store_id'], 'content' => $content, 'q_id' => $qid))->add();
			}else{
				$kid = D('Keyword')->data(array('store_id' => $this->store_session['store_id'], 'content' => $content, 'from_id' => $rid))->add();
				}
	...
	public function save_reply()
	{
		...
		if($qid && empty($rid)){
			$count = D('Reply_relation')->where(array('store_id' => $this->store_session['store_id'], 'q_id' => $qid))->count('pigcms_id');
			}else{
...
// 				}
		...
		if ($cid) {
			...
			if($qid && empty($rid)){
				...
				D('Reply_relation')->where(array('store_id' => $this->store_session['store_id'], 'pigcms_id' => $replyid))->data(array('cid' => $cid, 'q_id' => $qid, 'type' => $type))->save();
		...
		D('Reply_relation')->where(array('store_id' => $this->store_session['store_id'], 'pigcms_id' => $replyid))->data(array('cid' => $cid, 'rid' => $rid, 'type' => $type))->save();
			...
			$replyid = D('Reply_relation')->data(array('store_id' => $this->store_session['store_id'], 'cid' => $cid, 'q_id' => $qid, 'type' => $type))->add();
	...
	public function delete_reply()
	{
		...
		$rule = D('Qrcode')->where(array('store_id' => $this->store_session['store_id'], 'pigcms_id' => $qid))->find();
		echo D('Qrcode')->last_sql;
			}else{
	  	$rule = D('Rule')->where(array('store_id' => $this->store_session['store_id'], 'pigcms_id' => $rid))->find();
		echo D('Rule')->last_sql;
				}
		...
		if($qid && empty($rid)){
			...
			$relation = D('Reply_relation')->where(array('store_id' => $this->store_session['store_id'], 'pigcms_id' => $replyid, 'q_id' => $qid))->find();
		...
		if($qid && empty($rid)){
		D('Reply_relation')->where(array('store_id' => $this->store_session['store_id'], 'pigcms_id' => $replyid, 'q_id' => $qid))->delete();
			}else{
		D('Reply_relation')->where(array('store_id' => $this->store_session['store_id'], 'pigcms_id' => $replyid, 'rid' => $rid))->delete();
				}
	...
	public function auth()
	{
		...
		$result = M('Weixin_bind')->get_access_token('system', TRUE);
...
// 			die;
	...
	public function get_url_list()
	{
		...
		if ($type == 'feature') {
			...
			foreach ($page_list['page_list'] as &$v) {
				...
				$v['data_id'] = $v['page_id'];
				$v['url'] = $this->config['wap_site_url'] . '/page.php?id=' . $v['page_id'] . '&store_id=' . $this->store_session['store_id'];
			}
			...
			foreach ($page_list['page_list'] as &$v) {
				...
				$v['data_id'] = $v['cat_id'];
				$v['url'] = $this->config['wap_site_url'] . '/pagecat.php?id=' . $v['cat_id'] . '&store_id=' . $this->store_session['store_id'];
			}
		} elseif ($type == 'goods') {
			$product = M('Product');
			$where = array();
			...
			foreach ($products as $p) {
				...
				$p['url'] = $this->config['wap_site_url'] . '/good.php?id=' . $p['product_id'] . '&store_id=' . $this->store_session['store_id'];
			...
			foreach ($page_list['page_list'] as &$v) {
				...
				$v['url'] = $this->config['wap_site_url'] . '/goodcat.php?id=' . $v['group_id'] . '&store_id=' . $this->store_session['store_id'];
			}
		}
	...
	public function save_menu_content()
	{
		...
		$type = isset($_POST['type']) ? intval($_POST['type']) : 0;
		$url = isset($_POST['url']) ? html_entity_decode($_POST['url']) : '';
		if (!empty($url)) {
			if (strpos($url, '?') === false) {
				$url .= '?referer_weixin=1';
			} else {
				$url .= '&referer_weixin=1';
			}
		}
...
// 					}
			...
			if ($data = D('Source_material')->where(array('pigcms_id' => $pigcms_id, 'store_id' => $this->store_session['store_id']))->find()) {
				...
				if ($data['type'] == 0) {
					...
					if ($image_text['cover_pic']) {
						if(file_exists(PIGCMS_PATH.'upload/'.$image_text['cover_pic'])){
							$url 	= PIGCMS_PATH.'upload/'.$image_text['cover_pic'];
						}else{
							$org_url= getAttachmentUrl($image_text['cover_pic']);
							$url 	= PIGCMS_PATH.'upload/'.$image_text['cover_pic'];
					...
					$content = str_replace(array('"','"/ueditor'),array('\"','"'.option('config.site_url').'/ueditor'), $content);

					$send = '{"thumb_media_id":"'.$media['media_id'].'","author":"'.$image_text['author'].'","title":"'.$image_text['title'].'","content_source_url":"'.$image_text['url'].'","content":"'.$content.'","digest":"'.$image_text['digest'].'", "show_cover_pic":"'.$image_text['is_show'].'"}';
				} else {				
					$image_text = D('Image_text')->where(array('pigcms_id' => array('in', $it_ids)))->order('pigcms_id asc')->select();
					foreach ($image_text as $image) {
						$media['media_id'] = '';
						if ($image['cover_pic']) {
							if(file_exists(PIGCMS_PATH.'upload/'.$image['cover_pic'])){
								$url 	= PIGCMS_PATH.'upload/'.$image['cover_pic'];
							}else{
								$org_url= getAttachmentUrl($image['cover_pic']);
								$url 	= PIGCMS_PATH.'upload/'.$image['cover_pic'];
								Http::curlDownload($org_url,$url);
							}
							$media  = Http::curlPost($upload_url, array('media' => '@' . $url));
						...
						$send .= $comma . '{"thumb_media_id":"'.$media['media_id'].'","author":"'.$image['author'].'","title":"'.$image['title'].'","content_source_url":"'.$image['url'].'","content":"'.$content.'","digest":"'.$image['digest'].'", "show_cover_pic":"'.$image['is_show'].'"}';
						$comma = ',';
					}
				}
	...
	public function template_msg(){
		...
		if(IS_POST){
			...
			$oauth_info 	= D('Weixin_bind')->where(array('store_id'=>$_SESSION['store']['store_id']))->field('authorizer_appid')->find();
			$data 			= D('Tempmsg')->where(array('token'=>$oauth_info['authorizer_appid']))->order('id ASC')->select();
			foreach($data as $key=>$val){
			...
			foreach ($templs as $k => $v){
				$dbtempls = D('Tempmsg')->where(array('tempkey' => $k,'token'=>$oauth_info['authorizer_appid']))->find();
				if(empty($dbtempls)){
					$list[] 	= array(
						'tempkey' 	=> $k,
						'name'		=> $v['name'],
	...
	//微信 短信开关列表
	public function notice_power_list() {
		$store_id = $this->store_session['store_id'];
		$weixin_bind_info = D('Weixin_bind')->where(array('store_id'=>$store_id))->find();
		$store_info = D("Store")->where(array("store_id"=>$store_id))->find();
		if($weixin_bind_info['authorizer_appid']) {
			
			$data = D('Tempmsg')->where("token ='".$weixin_bind_info['authorizer_appid']."'  and status=1 ")->order('id ASC')->select();
			foreach($data as $key=>$val) {
				$list[$val['tempkey']] = $val;
			}
			...
			//获取当前店铺的短信/通知 权限
			$store_notice_manager = M('Store_notice_manage')->get($store_id);
	...
	//
	public function store_notice_setting() {	
		...
		$store_notice = D('Store_notice_manage')->where(array('store_id'=>$store_id))->find();
		$array['timestamp'] = time();
		if($store_notice) {
			D('Store_notice_manage')->data($array)->where(array('store_id'=>$store_id))->save();
		} else {
			$array['store_id'] = $store_id;
			D('Store_notice_manage')->data($array)->add();
	...
	public function qrcode_load()
	{
		...
		$this->assign('keyword', $keyword);
		$count = D('Qrcode')->where($where)->count('pigcms_id');
		import('source.class.user_page');
		$p = new Page($count, 10);
		$list = D('Qrcode')->where($where)->order('pigcms_id DESC')->limit($p->firstRow.','.$p->listRows)->select();
		$rids = array();
		foreach ($list as $l) {
			$rids[] = $l['pigcms_id'];
		}
		if ($rids) {
			$keywords = D('Keyword')->where(array('store_id' => $this->store_session['store_id'], 'q_id' => array('in', $rids)))->select();
			
			
			$k_list = array();// = array(rid => array(), ...)
			foreach ($keywords as $key) {
				$key['show_content'] = str_replace($this->face_key, $this->face_image, $key['content']);
				if (isset($k_list[$key['q_id']])) {
					$k_list[$key['q_id']][] = $key;
				} else {
					$k_list[$key['q_id']] = array($key);
				}
			}
			...
			$reply_relations = D('Reply_relation')->where(array('store_id' => $this->store_session['store_id'], 'q_id' => array('in', $rids)))->select();
			foreach ($reply_relations as $re) {
				$c_ids[$re['type']][] = $re['cid'];
// 				if ($re['type'] == 0) {
				${"v{$re['type']}_list"}[$re['cid']]['rule'] = $re['q_id'];//内容id 对应规则id
				${"v{$re['type']}_list"}[$re['cid']]['reply'] = $re['pigcms_id'];//内容id 对应规则id
// 				} elseif ($re['type'] == 1) {
// 					$v1_list[$re['cid']]['rule'] = $re['rid'];//内容id 对应规则id
// 					$v1_list[$re['cid']]['reply'] = $re['pigcms_id'];//内容id 对应规则id
...
// 		print_r($list);die;
	...
	public function save_qrcode()
	{
		...
		if ($rule = D('Qrcode')->where("`store_id`='{$this->store_session['store_id']}' AND `name`='{$name}' AND `pigcms_id`<>'{$pigcms_id}'")->find()) {
			json_return(1, '规则名已存在');
		}
		$now_time = time();
		if($pigcms_id && ($rule = D('Qrcode')->where("`store_id`='{$this->store_session['store_id']}' AND `pigcms_id`='{$pigcms_id}'")->find())) {
			if ($flag = D('Qrcode')->where(array('store_id' => $this->store_session['store_id'], 'pigcms_id' => $pigcms_id))->data(array('name' => $name, 'dateline' => $now_time))->save()) {
				exit(json_encode(array('err_code' => 0, 'err_msg' => '规则名修改成功', 'ruleid' => $pigcms_id, 'iskeyword' => 0)));
				json_return(0, '规则名修改成功');
			} else {
				json_return(1, '规则名修改失败，稍后重试');
			...
			if($rid = D('Qrcode')->data(array('store_id' => $this->store_session['store_id'], 'name' => $name, 'dateline' => $now_time))->add()) {
						
			$qrcode_id = $rid + 1800000000;
            $qrcode_return = M('Recognition')->get_limit_scene_qrcode($qrcode_id,$this->store_session['store_id']);
		if ($qrcode_return['msg']) {
        	$qrcode=$qrcode_return['msg'];
        } else {
            $qrcode=$qrcode_return['ticket'];
        }			
				D('Qrcode')->where(array('pigcms_id' => $rid))->data(array('qrcode' => $qrcode, 'scene_str' => $qrcode_id))->save();	
	...
	{
		$rid = isset($_GET['rid']) ? intval($_GET['rid']) : 0;
		if ($rule = D('Qrcode')->where("`store_id`='{$this->store_session['store_id']}' AND `pigcms_id`='{$rid}'")->find()) {
		if($rule['status']>0){
		$status=0;
		}else{
		$status=1;
...
D('Qrcode')->where("`store_id`='{$this->store_session['store_id']}' AND `pigcms_id`='{$rid}'")->data(array('status' => $status))->save();
			json_return(0, '规则名已更新');
		} else {
			json_return(1, '规则名更新失败，稍后重试');
		}
	}
