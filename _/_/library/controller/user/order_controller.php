...
class order_controller extends base_controller {
    ...
    public function load() {
        ...
        $drp_team_id     = isset($_POST['drp_team_id']) ? intval(trim($_POST['drp_team_id'])) : '';
        ...
        switch ($action) {
            ...
            break;
            case 'clear_return_owe_add_content':
            $this->_clear_return_owe_add_content();
            break;
            case 'clear_return_owe_log_content':
            $this->_clear_return_owe_log_content();
            break;
            case 'dividends_mx_content':
...
public function do_printing() {
  ...
  $store_name = $this->store_session['name'];
		...
		foreach($order_arr as $k=>$order) {
			...
			if(($order['type'] == 7) && $order['data_money']) {
				$discount_money += $order['data_money'];
				$order['other_discount'] ="已去预售折扣：<i>&yen". number_format($order['data_money'], 2) ."元</i><br/>";
			}
		...
		$tpls = preg_replace('/\[%vo_all.order_typename%\]/',"<?php switch(\$vo_all['type']) {case '1': \$typename='代付';break;case '2': \$typename='送礼';break;case '3': \$typename='分销';break;case '4': \$typename='活动';break;case '5': \$typename='批发';break;case '6': \$typename='团购';break;case '7': \$typename='预售';if(\$vo_all['presale_order_id'] == \$vo_all['order_id']) {\$typename .= '尾款';} else {\$typename .= '定金';}break;default: \$typename='普通';}echo \$typename.'订单';?>",$tpls);	//订单类型
		...
		$tpls = preg_replace('/\[%vo_all.other_discount%\]/',"<?php echo \$vo_all['other_discount'];?>",$tpls);	//订单打折优惠
    ...
    if($order['type'] == 7) {
       if(($order['presale_order_id'] == 0) || $order['presale_order_id'] != $order['order_id'] ) {
        ...
        if($order['presale_order_id']) {
         redirect(url('order:detail')."&id=".$order['presale_order_id']);
         return;
     }
 } else {
				//当前为：尾款订单 读定金订单
...
private function _all_content($data)
{
    ...
    if (M('Drp_team')->checkDrpTeam($this->store_session['store_id'], true) && empty($this->store_session['drp_supplier_id'])) {
        $drp_teams = M('Drp_team')->getDrpTeams(array('dt.supplier_id' => $this->store_session['store_id']), 'dt.pigcms_id DESC', 0, 0);
 ...
 $is_packaged = false;
            ...
            if (!empty($tmp_order['useStorePay'])) {
                ...
                if ($tmp_order['storePay'] == $this->store_session['store_id']) {
        ...
        if ($tmp_order['type'] != 5 && empty($this->store_session['drp_supplier_id'])) {
                //查看是否有未支付的供货商订单
...
private function _activity_content($data)
{
    ...
    $where['type'] = array('in', array('6','7','50','53','55'));
    ...
    if ($data['status'] != '*') {
        ...
        if (is_numeric($data['activity_type'])) {
            $where['type'] = $data['activity_type'];
        }
        if (!empty($data['user'])) {
            $where['address_user'] = $data['user'];
    ...
    private function _dashboard()
    {
        ...
        //待付款订单数
        $not_paid_orders = $order->getOrderCount(array('store_id' => $this->store_session['store_id'], 'status' => 1, 'activity_data' => array('is_null', "is_null")));
        //待发货订单数
        $not_send_orders = $order->getOrderCount(array('store_id' => $this->store_session['store_id'], 'status' => 2, 'activity_data' => array('is_null', "is_null")));
        //已发货订单数
        $send_orders = $order->getOrderCount(array('store_id' => $this->store_session['store_id'], 'status' => 3, 'activity_data' => array('is_null', "is_null")));
        //7天收入
        $where = array();
        //开始时间
        $start_time = strtotime($days[0] . ' 00:00:00');
    ...
    $this->assign('status', $data['status']);
    $this->assign('payment_method', $payment_method);
    $this->assign('shipping_method', 'selffetch');
    $this->assign('orders', $orders);
    $this->assign('page', $page->show());
}
...
public function cancel_status()
{
 ...
 if ($order_detail['payment_method'] != 'codpay') {
    if ($order_detail['status'] > 1) {
        json_return(1000, '此订单状态不正确，不能取消');
    }
        //货到付款订单 收货前可以取消
} else {
...
public function _statistics_content($data)
{
   ...
   if ($this->user_session['type'] == 1) {
    $physical = D("Store_physical")->where(array('pigcms_id'=>$this->user_session['item_store_id']))->find();
}
...
if ($order['shipping_method'] == 'send_other' && $order['send_other_type'] != 2) {
   ...
   if (time() - $order['paid_time'] > $order['send_other_hour'] * 3600 && count($order_friend_address_list) < $order['send_other_number']) {
    ...
    if (empty($count)) {
     ...
     $data['uid'] = $order['uid'];
		...
		$products = array();
		foreach ($tmp_products as $key => $tmp_product) {
            ...
            $wherePhysical = array(
                'store_id' => $this->user_session['drp_store_id'],
                'physical_id' => $this->user_session['item_store_id'],
                'product_id' => $tmp_product['product_id'],
                'sku_id' => $tmp_product['sku_id'],
                );
            $physical_quantity = D("Store_physical_quantity")->where($wherePhysical)->find();
            $quantity = !empty($physical_quantity) ? $physical_quantity['quantity'] : 0;
   ...
   public function create_package()
   {
    ...
    $data['express_code'] = isset($_POST['express_id']) ? trim($_POST['express_id']) : '';
        ...
        // 门店管理员 物流发货检测门店库存
        if ($this->user_session['type'] == 1 && $store['open_local_logistics'] == 0) {
            // echo 1;exit;
            foreach ($tmp_product_list as $key => $tmp_product_id) {
                ...
                if (empty($temp_physical_quantity) || $temp_physical_quantity['quantity'] < $tmp_order_product['pro_num']) {
                    json_return(1004, '部分选中商品，本店库存不足，请补充库存后发货！');
                }
      ...
      $data['express_code'] = isset($_POST['express_id']) ? trim($_POST['express_id']) : '';
		...
		if ($package_id) {
			...
			if ($order['user_order_id']) {
				...
				$send_pro_num = D('Order_friend_address')->where("(order_id = '" . $order['order_id'] . "') AND package_id > 0")->sum('pro_num');
			}
			...
			if ($send_pro_num >= $order_product['pro_num']) {
				...
				if (!empty($order['user_order_id'])) {
					...
					D('Order_product')->where("order_id = '" . $order['order_id'] . "'")->data(array('is_packaged' => 1, 'in_package_status' => 1))->save();
					D('Order')->where("order_id = '" . $order['order_id'] . "'")->data(array('sent_time' => time(), 'status' => 3))->save();
				}
			}
    ...
    public function complate_status()
    {
        ...
        if (IS_POST) {
			/*import('source.class.Order');
			$result = Order::complate($_POST['order_id'], $this->store_session['store_id']);
			exit;*/
         ...
         if ((($order_info['status'] == 7 && ($order_info['delivery_time'] + $config_order_return_date * 24 * 3600 < time() || $order_info['sent_time'] + $config_order_complete_date * 24 * 3600 < time())) || ($order_info['status'] == 3 && $order_info['sent_time'] + $config_order_complete_date * 24 * 3600 < time()))) {
    ...
    $user_order_info = $order->get(array('order_id' => $user_order_id));
            ...
            $sql = "SELECT SUM(r.product_money) AS return_sub_total, SUM(r.postage_money) AS return_postage, SUM(r.platform_point) AS return_point FROM " . option('system.DB_PREFIX') . "return r, " . option('system.DB_PREFIX') . "return_product rp WHERE r.id = rp.return_id AND rp.supplier_id = r.store_id AND r.order_id = '" . $order_id . "' AND r.store_id = '" . $store_id . "' AND r.status = 5";
            ...
            //退货积分
            $order_returns_point = !empty($return_detail[0]['return_point']) ? $return_detail[0]['return_point'] : 0;;
            //退货总额
            $order_returns_total = $order_returns_sub_total + $order_returns_postage + ($order_returns_point / $user_order_info['point2money_rate']);
            ...
            if (empty($my_products) && empty($order_info['is_offline']) && empty($order_products)) {
            ...
            if ($order_returns_total > 0) {
                $tmp_returns = $return->getReturns($order_id, array(5));
         ...
         if ($order->editStatus($where, $data)) {
            ...
            if (!empty($return_products)) {
                ...
                if (!$has_fx_order) { //无分销和批发
                    ...
                    if (!empty($order_info['useStorePay']) || ($user_order_info['payment_method'] == 'point' && empty($this->store_session['drp_supplier_id']))) {
                        $useStorePay = 1;
                    } else {
                        $useStorePay = 0;
                    }
                    ...
                    //退货减销售额
                    if ($return_amount > 0) {
                        //D('Store')->where(array('store_id' => $user_order_store['store_id']))->setDec('sales', $return_amount);
                    }
                    ...
                    if (Margin::check()) { //有保证模式
                        Margin::init($store_id);
                        Margin::sendPoint($order_id);
                    } else if (empty($order_info['useStorePay'])) { //无保证模式 + 平台收款
                      $open_promotion_reward = option('credit.open_promotion_reward');
                      if (!empty($open_promotion_reward)) {
							//退货后的订单总额
                       $order_total = $order_info['total'] - $order_returns_sub_total - $order_returns_postage;
                       if ($order_total > 0) {
								//获取供货商
                        Margin::init($store_id);
                        $store_info = D('Store')->field('wxpay')->where(array('store_id' => $store_id))->find();
                        Margin::promotion_reward($order_total, $order_id, 0, 0, $store_info['wxpay']);
                    }
                }
            }
            ...
            foreach ($orders as $tmp_order) {
                ...
                if ($tmp_order['type'] != 5) {
                    if ($tmp_order['store_id'] == $dealer_id) {
                        if (empty($tmp_order['useStorePay'])) {
                         $supplier_cost = D('Financial_record')->field('income')->where(array('store_id' => $dealer_id, 'supplier_id' => $store_id, 'order_id' => $tmp_order['order_id'], 'income' => array('<', 0)))->find();
									//供货商成本
                         $supplier_cost = !empty($supplier_cost['income']) ? abs($supplier_cost['income']) : 0;
                         $income        = $order_info['sale_total'] - $supplier_cost;
                         $new_income    = $order_info['sale_total'] - $supplier_cost;
                     } else {
             ...
             if ($tmp_order['order_id'] == $user_order_info['order_id']) {
                            //退货减销售额
                if ($order_returns_total > 0) {
                                //D('Store')->where(array('store_id' => $user_order_store['store_id']))->setDec('sales', $order_returns_total);
                }
            }
        }
        ...
        foreach ($supplier_products as $supplier_product) {
                    ...
                    if (!empty($order_info['useStorePay']) || !empty($order_info['use_deposit_pay']) || ($user_order_info['payment_method'] == 'point' && empty($this->store_session['drp_supplier_id']))) {
                        $useStorePay = 1;
                    }
                ...
                if ($return_amount > 0) {
                    if ($dealer_relation['sales'] >= $return_amount) {
						...
						//主订单为平台收款且经销商没有使用保证金
                if (empty($user_order_info['useStorePay']) && empty($order_info['use_deposit_pay'])) {
                   if ($seller_product_profit > 0) {
                    D('Store')->where(array('store_id' => $dealer_id))->setDec('balance', $seller_product_profit);
                    D('Store')->where(array('store_id' => $store_id))->setInc('balance', ($seller_product_profit - $order_returns_total));
                }
            }
        }
        ...
        if (!empty($tmp_returns)) {
            foreach ($tmp_returns as $tmp_return) {
                if (array_key_exists($tmp_return['user_return_id'], $returns_seller_profit)) {
                    ...
                    $return_amount = ($tmp_return['product_money'] + $tmp_return['postage_money'] + $tmp_return['platform_point'] / $user_order_info['point2money_rate']) - $return_amount;
            ...
            if (!empty($user_order_store['drp_team_id']) && $return_amount > 0) {
                M('Drp_team')->setSalesDec($user_order_store['drp_team_id'], $return_amount);
                    ...
                    if (empty($order_uncomplated)) { //交易完成
                        ...
                        if (Margin::check()) { //有保证金模式
                            Margin::init($store_id);
                            Margin::sendPoint($order_id);
                        } else if (empty($user_order_info['useStorePay'])) { //无保证金模式 + 平台收款
                           $open_promotion_reward = option('credit.open_promotion_reward');
                           if (!empty($open_promotion_reward)) {
								//主订单的退货总额
                            $order_returns_total = D('Return')->where(array('order_id' => $user_order_id, 'status' => 5))->sum('product_money + postage_money');
								//退货后的订单总额
                            $order_total = $user_order_info['total'] - $order_returns_total;
                            if ($order_total > 0) {
                             Margin::init($user_order_info['store_id']);
                             $supplier = D('Store')->field('wxpay')->where(array('store_id' => Margin::$_STORE_ID))->find();
                             Margin::promotion_reward($order_total, $user_order_id, 0, 0, $supplier['wxpay']);
                         }
                     }
                 }
                 ...
                 Dividends::auto_send($store_id);
             }
         }

         if ($user_order_info['status'] != 4) {
            /*====通知 start====*/
                    //产生提醒-已生成订单
            import('source.class.Notice');
    ...
    private function _check_content($data)
    {
        ...
        $store = D('Store')->field('unbalance,balance,drp_supplier_id')->where(array('store_id' => $store_id))->find();
        //在平台可提现金额
        //分销商分润
    ...
    foreach ($orders as &$tmp_order) {
        $user_order_id = !empty($tmp_order['user_order_id']) ? $tmp_order['user_order_id'] : $tmp_order['order_id'];
			//状态
        $tmp_order['status_id'] = $tmp_order['status'];
        ...
        $tmp_order['return_amount'] = number_format($return_amount, 2, '.', '');
            ...
            //分润
            if ($tmp_order['type'] == 5) { //批发订单
                $tmp_order['seller_profit'] = '0.00';
                $tmp_order['income'] = $tmp_order['total'];
                $tmp_order['profit'] = D('Financial_record')->where(array('order_id' => $tmp_order['order_id']))->sum('income');

                if ($tmp_order['status'] == 4) {
                 $tmp_order['uncheckout'] = 0;
                 $tmp_order['checkout'] = $tmp_order['profit'] + $return_amount;
             } else {
                 $tmp_order['uncheckout'] = $tmp_order['total'];
                 $tmp_order['checkout'] = 0;
             }
         } else {
            ...
            $seller_profit = 0;
            if ($tmp_order['type'] == 3) {
             $seller_profit = D('Financial_record')->where(array('supplier_id' => $tmp_order['store_id'], 'user_order_id' => $user_order_id, 'order_id' => array('<', $tmp_order['order_id'])))->sum('income');
         }
         $tmp_order['seller_profit'] = number_format($seller_profit, 2, '.', '');

				//订单有批发商品
         if (empty($store['drp_supplier_id']) && !empty($tmp_order['suppliers']) && $tmp_order['suppliers'] != $store_id) {
             $tmp_order['is_ws'] = true;
         }

         if (!empty($tmp_order['is_ws'])) {
             $suppliers = explode(',', $tmp_order['suppliers']);
             foreach ($suppliers as $supplier) {
              if ($supplier == $store_id) {
               continue;
           }
           $ws_profit = M('Order')->getWsSellerProfit(array('user_order_id' => $user_order_id, 'order_id' => array('<=', $tmp_order['order_id'])));
           $supplier_order = D('Order')->where(array('user_order_id' => $user_order_id, 'status' => 4))->find();
           if (!empty($supplier_order)) {
               $tmp_order['checkout'] += $ws_profit[$supplier];
           } else {
               $tmp_order['uncheckout'] += $ws_profit[$supplier];
           }
           if ($tmp_order['status_id'] == 4) {
               $tmp_order['checkout'] += $tmp_order['sale_total'];
           } else {
               $tmp_order['uncheckout'] += $tmp_order['sale_total'];
           }
       }
				} else if (empty($tmp_order['is_fx']) && empty($tmp_order['user_order_id'])) { //自营
					$checkout = D('Financial_record')->where(array('order_id' => $tmp_order['order_id'], 'status' => 3))->sum('income');
					$tmp_order['uncheckout'] = $tmp_order['income'] - $checkout;
					$tmp_order['checkout'] = $checkout;
				} else if (!empty($tmp_order['is_fx']) || !empty($tmp_order['user_order_id'])) {
					if ($tmp_order['status_id'] != 4) {
						$tmp_order['uncheckout'] = $tmp_order['income'];
						$tmp_order['checkout'] = 0;
					} else {
						$tmp_order['uncheckout'] = 0;
						$tmp_order['checkout'] = $tmp_order['income'];
					}
				}
			}

			$tmp_order['uncheckout'] = number_format($tmp_order['uncheckout'], 2, '.', '');
			$tmp_order['checkout'] = number_format($tmp_order['checkout'] , 2, '.', '');
    ...
    private function _seller_check_content()
    {
        ...
        $sql = "SELECT SUM(sales) AS sales, SUM(balance) AS total FROM " . option('system.DB_PREFIX') . "store WHERE root_supplier_id = '" . $store_id . "'";
        $sellers = D('')->query($sql);
        //销售总额
        ...
        //分销商可提现利润
        if (!empty($sellers[0]['total'])) {
            $seller_profit_balance = $sellers[0]['total'];
        } else {
        ...
        //分销商数量
        $sellers_count = D('Store')->where(array('root_supplier_id' => $store_id))->count('store_id');

        $where = array();
        $where['root_supplier_id'] = $store_id;
        if (!empty($_POST['name'])) {
           $where['name'] = array('like', '%' . trim($_POST['name']) . '%');
       }
       if (!empty($_POST['level']) && $_POST['level'] >= 9) {
           $where['drp_level'] = array('>=', intval(trim($_POST['level'])));
       } else if (!empty($_POST['level'])) {
           $where['drp_level'] = intval(trim($_POST['level']));
       }
       $seller_count = D('Store')->where($where)->count('store_id');

       $page = new Page($seller_count, 15);

       $sellers = D('Store')->field('store_id AS seller_id,name,drp_level,sales,balance,unbalance')->where($where)->order('store_id ASC')->limit($page->firstRow . ',' . $page->listRows)->select();
       if (!empty($sellers)) {
    ...
    $this->assign('seller_count', $sellers_count);
...
public function supplier_check()
{
    ...
    $return_owe       = !empty($suppliers[0]['return_owe']) ? $suppliers[0]['return_owe'] : 0;
        ...
        //获得经销分红奖金
    $where['store_id'] = $store_id;
    $where['dividends_type'] = '1';
    $dividends_total = M('Dividends_send_log')->getAmountTotal($where);

    $this->assign('dividends_total', $dividends_total);
...
private function _supplier_check_content()
{
        ...
        if ($type == 1) {
            $sql .= " AND (s.margin_amount = 1 OR sdr.bond > 0)";
            $sql2 .= " AND (s.margin_amount = 1 OR sdr.bond > 0)";
        } else {
    ...
    private function _profit_check_content()
    {
        ...
        $store['dividends'] = number_format($store['dividends'], 2, '.', '');
    ...
    //销退货欠款
    public function clear_return_owe()
    {
        //经销商id
        $dealer_id = !empty($_REQUEST['dealer_id']) ? intval(trim($_REQUEST['dealer_id'])) : 0;
        $store_id = $this->store_session['store_id'];

        //经销商
        $dealer = D('Supp_dis_relation')->where(array('distributor_id' => $dealer_id, 'supplier_id' => $store_id))->find();

        if (IS_POST) {
            $amount = !empty($_POST['amount']) ? floatval(trim($_POST['amount'])) : 0;
            $bak = trim($_POST['bak']);
            $now = time();

            if (empty($dealer)) {
                json_return(1001, '经销商不存在');
            }
            if ($dealer['return_owe'] < $amount) {
                json_return(1002, '待销账金额不足');
            }

            if (D('Clear_return_owe_log')->data(array('store_id' => $dealer_id, 'supplier_id' => $store_id, 'amount' => $amount, 'bak' => $bak, 'add_time' => $now))->add()) {
                $dealer['return_owe'] -= $amount;
                $dealer['clear_return_owe'] += $amount;
                D('Supp_dis_relation')->where(array('distributor_id' => $dealer_id, 'supplier_id' => $store_id))->data(array('return_owe' => $dealer['return_owe'], 'clear_return_owe' => $dealer['clear_return_owe']))->save();
                json_return(0, '销账成功');
            } else {
                json_return(1003, '销账失败');
            }
        }


        if (empty($dealer_id)) {
            pigcms_tips('访问的页面参数无效');
        }
        if (empty($dealer)) {
            pigcms_tips('经销商不存在');
        }
        $dealer_info = D('Store')->field('name')->where(array('store_id' => $store_id))->find();
        $dealer['name'] = $dealer_info['name'];
        $dealer['return_owe'] = number_format($dealer['return_owe'], 2, '.', '');
        $dealer['clear_return_owe'] = number_format($dealer['clear_return_owe'], 2, '.', '');
    ...
    private function _clear_return_owe_add_content()
    {
        //经销商id
        $dealer_id = !empty($_POST['dealer_id']) ? intval(trim($_POST['dealer_id'])) : 0;
        $store_id = $this->store_session['store_id'];
    ...
    private function _clear_return_owe_log_content()
    {
        import('source.class.user_page');
        ...
        $log_count = D('Clear_return_owe_log')->where(array('store_id' => $dealer_id, 'supplier_id' => $store_id))->count('pigcms_id');
        $page = new Page($log_count, 20);
        $logs = D('Clear_return_owe_log')->where(array('store_id' => $dealer_id, 'supplier_id' => $store_id))->order('pigcms_id DESC')->limit($page->firstRow . ',' . $page->listRows)->select();
    ...
    {
    	$this->assign('credit_setting', option('credit'));
        $this->display();
    }
    ...
    private function _order_return_list()
    {
        ...
        if ($count > 0) {
            ...
            import('source.class.user_page');
            $pages = new Page($count, $limit, $page);
    ...
    private function _order_return_detail() {
        ...
        $where = "r.order_no = '" . $return['order_no'] . "' AND r.id != '" . $return['id'] . "'";
        $relation_return_list = M('Return')->getList($where);
        $this->assign('relation_return_list', $relation_return_list);
        ...
        if ($order['cash_point'] > 0 && $order['point2money_rate'] > 0 && $return['status'] != 6 && $return['is_fx'] == 0 && empty($return['cancel_dateline'])) {
           $order_id = $order['user_order_id'] ? $order['user_order_id'] : $order['order_id'];
           $product_rate_arr = Margin::orderRate($order_id);

           $product_id = $return['original_product_id'] ? $return['original_product_id'] : $return['product_id'];

           $platform_point_rate = 0;
           if (isset($product_rate_arr[$product_id . '_' . md5($return['sku_data'])])) {
            $platform_point_rate = $product_rate_arr[$product_id . '_' . md5($return['sku_data'])] * $return['pro_num'];
        }

        if ($platform_point_rate) {
            $platform_point = round($order['cash_point'] * $platform_point_rate, 2);
            $platform_point_money = round($platform_point / $order['point2money_rate'], 2);
        }
    }
    ...
    $where = array();
    $where['store_id'] = $store_id;
    $info = D('Store')->where($where)->field('name,tel')->find();
    $store_contact = D('Store_contact')->where($where)->field('province,city,county,address')->find();

    $this->assign('platform_point', $platform_point);
    $this->assign('platform_point_money', $platform_point_money);
    $this->assign('order', $order);
    ...
    public function return_save() {
        ...
        $dealer_order = D('')->query($sql);
        $dealer_id = !empty($dealer_order[0]['store_id']) ? $dealer_order[0]['store_id'] : 0;

        //主订单
        $user_order = D('Order')->field('point2money_rate,useStorePay,store_id')->where(array('order_id' => $user_order_id))->find();
        //下单时积分现在兑换比率
        $point2money_rate = !empty($user_order['point2money_rate']) ? $user_order['point2money_rate'] : 0;
		//退货总额
        $return_total = $product_money + $postage_money + ($platform_point / $point2money_rate);

        if (empty($order)) {
            json_return(1001, '未查到相应的订单');
        }

        $order_status = $return['order_status'] ? $return['order_status'] : $order['status'];
        if ($status == '2') {
            if (empty($store_content)) {
                json_return(1001, '不同意退货理由不能为空，请填写');
            }
        } else {
            if ($product_money < 0 && $platform_point <= 0) {
        ...
        if ($status == '2') {
            ...
            if ($result) {
                ...
                $return['store_content'] = $store_content;
                ShopNotice::ReturnApplyAuth($status,$return);
            ...
            if ($order_status == 2 || $order_status == 6) {
                $data['status'] = 5;
				if (empty($user_order['useStorePay'])) { //平台收款
					$data['refund_status'] = 0; //等待平台退款
				} else {
					$data['refund_status'] = 1; //已退款，商家自行处理
				}
            }
            ...
            if ($result) {
                ...
                if (!empty($dealer_order)) {
                    ...
                    if (!empty($return_amount) && !empty($user_order['useStorePay']) && !empty($order['use_deposit_pay'])) {
                ...
                if ($order_status == 2 && $order['payment_method'] != 'codpay') {
                    ...
                    foreach ($return_list as $tmp) {
                        if ($tmp['profit'] > 0) {
                            ...
                            if (empty($tmp['drp_level']) && ($product['supplier_id'] == $tmp['store_id'] || empty($product['supplier_id']))) {
                                // 查看此退货商品折扣金额
                                $discount_money = $product['pro_num'] * $product['pro_price'] - $product_money;
                                $financial_record_data['income'] = -1 * $tmp['profit'] + $discount_money;
                            }
                            if ($platform_point > 0 && ($tmp['store_id'] == $store_id || $dealer_id == $tmp['store_id'])) {
                                $financial_record_data['income'] -= ($platform_point / $point2money_rate);
                                $financial_record_data['income'] = number_format($financial_record_data['income'], 2, '.', '');
                            }
                ...
                if ($return_total > 0 && !empty($user_order)) {
                 D('Store')->where(array('store_id' => $user_order['store_id']))->setDec('sales', $return_total);
             }

                //短信/通知 提醒 => 退货申请提醒通知
             import('source.class.ShopNotice');
             $return['store_content'] = $return['content'];
             ShopNotice::ReturnApplyAuth($status,$return);
    ...
    public function return_over() {
        ...
        foreach ($return_list as $tmp) {
           $return_total += ($tmp['product_money'] + $tmp['postage_money'] + ($tmp['platform_point'] / $point2money_rate));
        ...
        if ($tmp['profit'] > 0) {
         ...
         if ($return['platform_point'] > 0 && ($tmp['store_id'] == $store_id || $dealer_id == $tmp['store_id'])) {
             $financial_record_data['income'] -= ($return['platform_point'] / $point2money_rate);
             $financial_record_data['income'] = number_format($financial_record_data['income'], 2, '.', '');
         }
         $financial_record_data['type'] = 3;
         $financial_record_data['trade_no'] = date('YmdHis') . rand(100000, 999999);
 ...
 $data['status'] = 5;
        ...
        if ($return['user_return_id']) {
            D('Return')->where("user_return_id = '" . $return['user_return_id'] . "' OR id = '" . $return['user_return_id'] . "'")->data($data)->save();
        } else {
            D('Return')->where("id = '" . $return['id'] . "'")->data($data)->save();
        }
...
break;
       ...
       while($page) {
           ...
           if(count($tmp_orders) == 0) {
			//return ;
           } else if(count($tmp_orders)< $limit) {
               $page = false;				
           } else {
               $page ++;
           }	
         ...
         $return_quantity = 0;
				//if($tmp_order['status'] > 2) {
					//退货数量
         $return_quantity = M('Return_product')->returnNumber($tmp_order['order_id'], $product['pigcms_id'], false);
					//实际购买数量
         $quantity = $product['pro_num'];
         $product['pro_num'] = $quantity;
         $product['return_quantity'] = $return_quantity;

				//} 

     }
			//订单退款总额统计
     $tmp_order['order_return_money'] = D('Return')->where(array('order_id' => $tmp_order['order_id']))->sum("product_money");
     $tmp_order['order_return_postage'] = D('Return')->where(array('order_id' => $tmp_order['order_id']))->sum("postage_money");
     $tmp_order['products'] = $products;
     $tmp_order['has_my_product'] = $has_my_product;
     if (!empty($tmp_order['user_order_id'])) {
       $order_info = D('Order')->field('store_id')->where(array('order_id' => $tmp_order['user_order_id']))->find();
...
echo ' 		<td align="center" colspan="6" ><table><tr><td colspan="6" align="center" ><b>商品信息</b></td></tr><tr><td width="300" align="center"><b>商品名称</b></td><td align="center"><b>购买数量</b></td><td align="center"><b>退货数量</b></td><td align="center"><b>单价</b></td><td align="center"><b>购买的商品规格</b></td><td align="center"><b>买家购买商品的留言</b></td></tr></table></td>';
...
foreach ($orders as $k => $v) {
   ...
   if ($v['user_order_id']) {
    $user_order = D('Order')->where(array('order_id' => $v['user_order_id']))->find();
                //订单实付金额
    $v['pay_money'] = $user_order['total'];
}
...
if($v['type'] == '1') {
    ...
    $typename = "预售";
    if($v['presale_order_id'] == $v['order_id']) {
     $typename.="(".'尾款'.")订单";

     $dj_order = D('Order')->where(array('presale_order_id'=>$v['order_id']))->find();
     if($dj_order['order_no']) {
      $typename .="<br>定金订单号：".$dj_order['order_no'];
  }
} else {
 $typename.="(".'定金'.")订单";
}
...
echo ' 		<td  style="vnd.ms-excel.numberformat:@" align="center" width="190">' . $typename . '</td>';
echo '<td colspan="6"><table style="height:100%">';
$guige = "";
$comments_buyer= "";
...
$liuyan="";
echo '<tr><td align="center">'.$val['name'].$is_present.'</td><td align="center">' .$val['pro_num']. '</td><td align="center">' .$val['return_quantity']. '</td><td align="center">￥'.$val['pro_price'].'</td><td align="center">'.$guige.'</td><td align="center">'.$comments_buyer.'</td></tr>';	
}
echo '</table></td>';
...
echo ' 		<td align="center">' . ($v['cancel_method'] == 0 ? '正常' : ($v['cancel_method'] == 1 ? '卖家手动取消' : '买家手动取消')) . '</td>';
...
$v['return_yf'] = "";
if($v['order_return_postage'] > 0) {
    $v['return_yf'] = "+".$v['order_return_postage']."(邮费)";
}
echo ' 		<td align="center">' . $v['order_return_money'] . $v['return_yf'].'</td>';
   ...
   if (empty($this->store_session['drp_supplier_id'])) {
    ...
    if (!empty($orders[0]['orders'])) {
        $order_count = $orders[0]['orders'];
            ...
            //销售额
    $sql = "SELECT SUM(o.total) AS sales FROM " . option('system.DB_PREFIX') . "order o INNER JOIN " . option('system.DB_PREFIX') . "store_supplier ss ON o.store_id = ss.seller_id WHERE ss.type = 1 AND o.status IN (2,3,4,7) AND FIND_IN_SET(" . $supplier_id . ", ss.supply_chain) AND o.is_fx = 1";
    if (!empty($store_id)) {
        $sql .= " AND o.store_id = '" . $store_id . "'";
    }
...
private function _fx_bill_check_content()
{
    ...
    public function add() {
      ...
      if (IS_POST) {
       $store_user = D('User')->where(array('uid' => $this->store_session['uid']))->find();
       ...
       $point_money = $_POST['point_money'];
			...
			// 平台积分
       $platform_point_type = $_POST['platform_point_type'];
       $platform_point = $_POST['platform_point'];
       $platform_point_money = $_POST['platform_point_money'];
...
$product['wholesale_supplier_id'] = $store_id;
				...
				// 折扣金额
$discount = 10;
if ($platform_point_type != 1 && $platform_point_type != 2) {
...
if ($logistics_type != 'selffetch') {
    $postage = $_SESSION['postage'][$session_key]['postage'] + 0;
}
 ...
 foreach ($session_user_coupon_list as $tmp_store_id => $user_coupon_list) {
  ...
  foreach ($user_coupon_list as $user_coupon) {
      if ($user_coupon['id'] == $coupon_id) {
       $coupon_money += $user_coupon['face_money'];
   }
}
}
}
...
if ($point && $point_money && $platform_point_type != 1 && $platform_point_type != 2) {
				// 积分抵扣现金
    $points_money_data = Points::getPointMoney($uid, $store_id, $total_money + $postage - $reward_money - $discount_money - $coupon_money);
...
if (option('credit.platform_credit_open')) {
				// 判断用户平台积分是否充足
    if ($platform_point_type == 1 && $store['point_balance'] < $platform_point) {
     json_return(1000, '您的店铺' . option('credit.platform_credit_name') . '不够');
 } else if ($platform_point_type == 2 && $store_user['point_balance'] < $platform_point) {
     json_return(1000, '您的店铺用户的' . option('credit.platform_credit_name') . '不够');
 } else if ($platform_point_type != 1 && $user['point_balance'] < $platform_point) {
 ...
 if (option('credit.force_use_platform_credit')) {
     ...
     if ($platform_point_type != 1 && $platform_point_type != 2) {
      $platform_point_money_tmp = 0;
      if (option('credit.offline_trade_credit_type') == 1) {
       $platform_point_money_tmp = $platform_point_money;
   }

   if ($margin_balance < ($total_money + $postage - $platform_point_money_tmp) * option('credit.credit_deposit_ratio') / 100) {
       json_return(1000, '店铺保证金不够');
   }
} else {
  if ($margin_balance < ($total_money + $postage) * option('credit.credit_deposit_ratio') / 100) {
   json_return(1000, '店铺保证金不够');
}
}
}
...
if ($platform_point > 0) {
 $max_platform_point = Margin::orderPoint(0, $product_list) + $postage * option('credit.platform_credit_use_value');
 ...
 if ($max_platform_point < $platform_point) {
  json_return(1000, '此订单最多只能使用' . $max_platform_point . '个' . option('credit.platform_credit_name'));
				...
				// 现金和积分比例
if (option('credit.offline_trade_money') > 0 && $platform_point > 0) {
 if (option('credit.platform_credit_use_value') == 0) {
  json_return(1000, '此订单不能使用' . option('credit.platform_credit_name'));
} else {
						// 在未使用平台积分时，需要支付的总额
  if ($platform_point_type != 1 && $platform_point_type != 2) {
   $total = max(0, $total_money + $postage - $reward_money - $discount_money - $coupon_money - $point_money);
} else {
   $total = max(0, $total_money + $postage);
}

$platform_point_money_tmp = $platform_point / option('credit.platform_credit_use_value');
if ($platform_point_money != $platform_point_money_tmp) {
   json_return(1000, '数据不正确，请刷新页面重试');
}

if (round($total * option('credit.offline_trade_money') / 100, 2)  > round(($total - $platform_point_money), 2)) {
   $max_platform_point = ($total - $total * option('credit.offline_trade_money') / 100) * option('credit.platform_credit_use_value');
   json_return(1000, '此订单最多只能使用' . $max_platform_point . '个' . option('credit.platform_credit_name'));
}
}
}
...
$order_data['bak'] = $_POST['bak'] ? $_POST['bak'] : '线下做单订单';
$order_data['comment'] = $_POST['bak'] ? $_POST['bak'] : '线下做单订单';
$order_data['pay_money'] = $order_data['total'];
$order_data['is_check'] = 1;
$order_data['is_offline'] = 1;
$order_data['offline_type'] = $platform_point_type;
...
if (option('credit.platform_credit_open')) {
  if (!empty($product['open_return_point'])) {
   $order_product_data['return_point'] = !empty($product['return_point']) ? $product['return_point'] : 0;
} else {
   import('source.class.Margin');
   Margin::init($store_id);
							//现金兑积分
   $order_product_data['return_point'] = Margin::convert($product['pro_price'], 'point');
}
}
...
if ($pay_type == 'cash') {
  ...
  D('Product')->where(array('product_id' => $tmp_product_id))->setDec('quantity', $quantity);
				...
				// 优惠券处理
				if ($coupon_id && $platform_point_type != 1 && $platform_point_type != 2) {
					$session_user_coupon_list = array();
					if (isset($_SESSION['reward'][$session_key]['user_coupon_list']) && is_array($_SESSION['reward'][$session_key]['user_coupon_list'])) {
						$session_user_coupon_list = $_SESSION['reward'][$session_key]['user_coupon_list'];
					}
				...
				// 折扣、包邮
				if (isset($_SESSION['reward'][$session_key]['discount_list']) && is_array($_SESSION['reward'][$session_key]['discount_list']) && $platform_point_type != 1 && $platform_point_type != 2) {
				...
				// 更改订单金额
				if ($platform_point_type != 1 && $platform_point_type != 2) {
					$total = max(0, $order_data['sub_total'] + $order_data['postage'] + $order_data['float_amount'] - $money - $discount_money - $points_money_data['money'] - 0);
				} else {
					$total = max(0, $order_data['sub_total'] + $order_data['postage'] + $order_data['float_amount'] - 0);
				}
				...
				// 当小于需要审核金额时，即为审核通过
				if ($data['total'] < option('config.offline_money')) {
					$data['offline_status'] = 1;
				}
				
				//订单所属团队
				if (!empty($this->store_session['drp_team_id'])) {
					if (M('Drp_team')->checkDrpTeam($this->store_session['store_id'], true)) {
						$data['drp_team_id'] = $this->store_session['drp_team_id'];
					}
				}
				...
				if ($platform_point) {
					if ($platform_point_type != 1 && $platform_point_type != 2) {
						Margin::user_point_log($uid, $order_id, $store_id, $platform_point * -1, 1, 1, '订单使用', 1, '', false, true);
					} else if ($platform_point_type == 1) {
						Margin::store_point_log(-1 * $platform_point, 0, 0, '店铺手动做单，使用店铺积分', 1, $order_id, '', 0, false, true, false, $store_id);
					} else if ($platform_point_type == 2) {
						Margin::user_point_log($this->user_session['uid'], $order_id, $store_id, $platform_point * -1, 1, 1, '订单使用', 1, '', false, true);
					}
				}
				...
				if (Margin::check()) {
					...
					if ($margin_balance >= $total) {
						Margin::consume($total, 2, '保证金扣除', 1, $order_id);
					}
				}
		...
		$this->assign('credit_setting', $credit_setting);
	...
	private function _order_add() {
		...
		// 店铺用户积分
		$user = D('User')->where(array('uid' => $this->store_session['uid']))->find();
		
		$this->assign('pay_list', $pay_list);
		$this->assign('selffetch_list', $selffetch_list);
		$this->assign('store', $store);
		$this->assign('point_balance', $user['point_balance'] + 0);
	}
	...
	public function order_reward($product = array(), $uid = 0) {
		...
		$return['max_platform_point'] = Margin::orderPoint(0, $product_list);
...
public function repair()
{
  ...
  if (!empty($orders)) {
   foreach ($orders as $order) {
    $my_order = D('Order')->where(array('store_id' => $store_id, 'user_order_id' => $order['order_id']))->find();
    if (!empty($my_order)) {
     continue;
 }

				//订单下的商品
 $order_products = D('Order_product')->where(array('order_id' => $order['order_id']))->select();
					...
					if (!empty($value['supplier_id'])) { //非自营商品
						if (!empty($product['wholesale_product_id'])) { //批发商品
							$wholesale_products[$product['supplier_id'] . ',' . $product['store_id']][] = array(
								...
								'unified_profit'       => !empty($product['unified_profit']) ? $product['unified_profit'] : 0, //分销商店铺直销是否统一利润
...
if (!empty($wholesale_products)) {
  foreach ($wholesale_products as $tmp_key => $tmp_wholesale_products) {
   ...
   $this->_ws_order($order['order_id'], $supplier_id, $seller_id, $tmp_wholesale_products, false, $order['useStorePay']);
}
}
}
}
	...
	private function _ws_order($user_order_id, $supplier_id, $seller_id, $products, $is_seller, $useStorePay)
	{
		...
		$dealer_profit = 0;
		foreach ($products as $key => $product) {
			$properties = $this->_getProperty2Str($product['sku_data']);
			if ($properties) {
		...
		if ($fx_order_id = M('Fx_order')->add($data)) { //添加分销商订单
			...
			//获取主订单支付方式
			$master_order_info = D('Order')->field('*')->where(array('order_id' => $fx_order_info['user_order_id']))->find();
			...
			if ($order_id = M('Order')->add($order_info)) { //向供货商提交一个新订单
				...
				foreach ($products as $product) {
					...
					$owner_products[] = array(
						...
						'pro_price'            => $order_product['pro_price']
                     );
				}
				...
				if ($owner_total > 0) {
					D('Order')->where(array('order_id' => $order_id))->data(array('sale_total' => $owner_total))->save();
				}

				//满减、折扣、优惠
				$discount_amount = Order::orderDiscount($nowOrder, $order_ws_products, true);
				$order_total = $fx_order_info['cost_total'] - $discount_amount;
				D('Order')->where(array('order_id' => $order_id))->data(array('total' => $order_total, 'pay_money' => $order_total))->save();
				D('Fx_order')->where(array('fx_order_id' => $fx_order_id))->data(array('cost_total' => $order_total))->save();

				//获取批发利润(经销商)
				$profit = 0;
				if (($total - $order_total > 0)) {
					//$profit = $total - $order_total;
					if ($dealer_profit > 0) {
						D('Financial_record')->where(array('user_order_id' => $user_order_id, 'store_id' => $seller_id, 'income' => array('>', 0)))->setInc('profit', $dealer_profit);
					}
				}

				//使用保证金余额支付
				$bond_balance_pay = false;
				//if($master_order_info['useStorePay']) { //店铺收款
				if($master_order_info['payment_method'] == 'test') { //暂时测试用
					//获取经销商保证金余额
					...
					if ($bond > $order_total) {
						//保证金余额
						$bond_balance = ($bond - $order_total > 0) ? $bond - $order_total : 0;

						$bond_data['order_id']       = $order_id;
						$bond_data['order_no']       = $order_info['order_no'];
						$bond_data['transaction_no'] = date('YmdHis', time()) . mt_rand(100000, 999999);
						...
						if(D('Bond_record')->data($bond_data)->add()) { //添加扣除保证金的记录
							$bond_balance_pay = true;
							$trade_no = date('YmdHis', time()) . mt_rand(100000, 999999);
							D('Order')->where(array('order_id' => $order_id))->data(array('trade_no' => $trade_no, 'status' => 2, 'paid_time' => time(), 'third_id' => date('YmdHis', time()) . mt_rand(100000, 999999), 'use_deposit_pay' => 1))->save();
							//添加在供货商店铺的收益
							M('Store')->setIncomeInc($supplier_id, $order_total);

							//累加经销商批发利润
							if (!empty($dealer_profit) && $dealer_profit > 0) {
				...
				$data_record['income']         = (0 - $order_total);
    ...
    private function _dividends_mx_content(){
        ...
        $where = array();
        $where['sl.store_id'] = $this->store_session['store_id'];
        if (!empty($_POST['store'])) {
            $where['s.name'] = trim($_POST['store']);
        }

        $where['sl.dividends_type'] = '1';
        ...
        $record_total = $dividends_send_log->getLogsByJxsCount($where);

        import('source.class.user_page');
        $page = new Page($record_total, 20);
        
        $records = $dividends_send_log->getLogsByJxs($where, $page->firstRow, $page->listRows);      
    ...
    public function dividends_mx_rules(){

        $supplier_id = intval($_GET['supplier_id']);

        //获取该经销商对应供货商设置的分红规则说明
        $data = D('Dividends_rules')->where(array('supplier_id'=>$supplier_id))->select();
        
        $rules_des = M('Dividends_rules')->getRulesDescription($data,1);
        
        if (!$rules_des) {
            echo '<html><head></head><body>获取失败<br/><br/><font color="red">请关闭此窗口再打开重试XD。</font></body></html>';
        } else {
            $this->assign('rules_des',$rules_des);
            $this->display();
        }
    ...
    public function allorder_ajax(){ 
        ...
        if(empty($physical_id)){
            $physical_list = D('Store_physical')->where(array('store_id'=>$this->store_session['store_id']))->order('`pigcms_id` ASC')->find();
            $physical_id=$physical_list['pigcms_id'];
            $physical_name=$physical_list['name'];
        }else{
            $physical_list = D('Store_physical')->where(array('store_id'=>$this->store_session['store_id'],'pigcms_id'=>$physical_id))->order('`pigcms_id` ASC')->find();
            $physical_name=$physical_list['name'];
        }
       ...
       $results = array('err_code'=>'0', 'physical_name'=>$physical_name,'physical_id'=>$physical_id, 'physical'=>$list, 'send'=>$send,'refund'=>$refund,'seat'=>$seat,'meeting'=>$meeting,'time'=>time().'000');
   ...
   { 
    ...
    $send=D('Order')->where("store_id = '$store_id' AND status = 2")->select();
    foreach($send as $key=>$r){
      $order_product = M('Order_product');
      $send[$key]['goods']= $order_product->getProducts($r['order_id']);

  }

  $return_model = M('Return');
  $return_list = array();
  $return_list = $return_model->getList("r.store_id = '" . $store_id . "'", 500, 0);

  if($physical_id){
    $physical =  " AND physical_id = '$physical_id'";
}

$seat=D('Meal_order')->where("store_uid = '$store_id' AND status = 1 ".$physical)->select();

$meeting=D('Chahui_bm')->where("store_id = '$store_id' AND status = 1")->select();
foreach($meeting as $key=>$r){
	$chahui = D('Chahui')->where(array('pigcms_id'=>$r['cid']))->find();
	$meeting[$key]['chname']= $chahui['name'];
}
...
{ 
    ...
    if($type==1){
        $data = $_REQUEST['data'];
        $_SESSION['postdata'][$store_id] = $data;
    }else{
      $data  = $_SESSION['postdata'][$store_id];
      $results = array('err_code'=>'0', 'msg'=>$data);
      exit(json_encode($results));   
  }
