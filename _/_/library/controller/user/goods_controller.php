...
class goods_controller extends base_controller {
    ...
    public function goods_category_edit() {
        if (IS_POST) {
            ...
            $data_product_group['group_name'] = htmlspecialchars(stripcslashes($_POST['cat_name']), ENT_QUOTES);
            $data_product_group['is_show_name'] = $_POST['show_tag_title'];
            $data_product_group['first_sort'] = $_POST['first_sort'];
            $data_product_group['second_sort'] = $_POST['second_sort'];
            $data_product_group['list_style_size'] = $_POST['size'];
    ...
    public function create() {
        ...
        if (IS_POST) {
            ...
            $data['after_subscribe_discount'] = isset($_POST['after_subscribe_discount']) ? floatval(trim($_POST['after_subscribe_discount'])) : 10; //关注后折扣
            ...
            $data['open_return_point'] = !empty($_POST['open_return_point']) ? intval($_POST['open_return_point']) : 0; //是否自定义赠送平台积分
            $data['return_point'] = !empty($_POST['return_point']) ? intval($_POST['return_point']) : 0; //赠送平台积分数

            if (!empty($data['open_return_point'])) {
                import('source.class.Margin');
                if (!Margin::check()) {
                    $data['open_return_point'] = 0;
                    $data['return_point'] = 0;
                }
            } else {
                $data['return_point'] = 0;
			...
			$images2 = isset($_POST['images2']) ? $_POST['images2'] : array(); //图片
            ...
            if (!empty($images)) {
              ...
              if (!empty($images2)) {
                foreach ($images2 as &$image2) {
                    $image2 = getAttachment($image2);
                }
                $data['image'] = $images2; //商品主图
            }
                ...
                if (!empty($tmp_skus)) {
                    ...
                    foreach ($tmp_skus as $sku) {
                        ...
                        $skus[] = array(
                            ...
                            'price' => $sku['price'],
                        	'point' => $sku['point'] + 0,
//                            'after_subscribe_discount' => $sku['after_subscribe_discount'],
                            'after_subscribe_price' => $sku['after_subscribe_price'],
                        	'weight' => $sku['weight'],
...
//                    exit;
				...
				// 关联商品
				$relation_product_id = $_POST['relation_product_id'];
				if ($relation_product_id && is_array($relation_product_id)) {
					$i = 0;
					$relation_product_id = array_unique($relation_product_id);
					foreach ($relation_product_id as $tmp_product_id) {
						// 禁止关联其它店铺的产品
						$relation_product = D('Product')->where(array('product_id' => $tmp_product_id, 'uid' => $this->user_session['uid']))->field()->find();
						if ($relation_product) {
							$product_relation_data = array();
							$product_relation_data['product_id'] = $product_id;
							$product_relation_data['relation_id'] = $tmp_product_id;
							$product_relation_data['sort'] = $i;
							
							D('Product_relation')->data($product_relation_data)->add();
							$i++;
						}
					}
				}
	...
	//修改商品
		...
		if (IS_POST) {
			...
			$data['buyer_quota'] = isset($_POST['buyer_quota']) ? intval(trim($_POST['buyer_quota'])) : 0; //每人限购
			$data['sold_time'] = !empty($_POST['sold_time']) ? strtotime(trim($_POST['sold_time'])) : 0; //开售时间
			$data['allow_discount'] = isset($_POST['discount']) ? intval(trim($_POST['discount'])) : 0; //会员折扣
			$data['invoice'] = isset($_POST['invoice']) ? intval(trim($_POST['invoice'])) : 0; //发票
			$data['warranty'] = isset($_POST['warranty']) ? intval(trim($_POST['warranty'])) : 0; //保修
			$data['status'] = $_POST['status']; //销售状态 下架 上架
            ...
            //价格浮动
            //$fluctuationPrice = $data['price'] - $product_detail['price'];
			...
			if (!empty($skus)) {
				// 没有添加和删除商品属性
				if (empty($_POST['is_edit_sku'])) {

                    //$this->setFxProductsPrice($product_id,$fluctuationPrice);
                    //修改库存
					$product_sku->edit($product_id, $skus);
					// 修改已批发商品库存信息
                    if(!empty($original_product['is_wholesale'])) {
                        if (count($wholesaleProductList) > 0){
                            foreach ($wholesaleProductList as $productInfo){
                                unset($data['uid']);
                                unset($data['is_wholesale']);
                                unset($data['is_fx']);
                                $product->edit(array('product_id' => $productInfo['product_id']), $data);
                                $product_sku->wholesale_Skus($productInfo['product_id'], $skus);
                            }
                        }
                    }
					...
					if (!empty($sku_exists_data)) {
                        ...
                        // 查询以外的库存
                        $resultSku = D('Product_sku')->where(array ('product_id' => $product_id, 'sku_id' => array ('not in', $sku_id_list)))->select();

                        $del_sku_data = array();
                        foreach($resultSku as $result){
                            $del_sku_data[] =  "'".$result["properties"]."'";
                        }
                        // 删除以外的库存
                        if($del_sku_data){
                        ...
                        if(!empty($original_product['is_wholesale'])){
                            ...
                            if (count($wholesaleProductList) > 0) {
                                foreach ($wholesaleProductList as $productInfo) {
                                    $product_sku->edit($productInfo['product_id'], $sku_exists_data,true);
                                    ...
                                    // 删除以外的库存
                                    if($del_sku_data){
                                        D('Product_sku')->where(array ('product_id' => $productInfo['product_id'], 'properties' => array ('in', $del_sku_data)))->delete();
                                    }
                                }
                            }
                        }
					...
					if (!empty($sku_not_exists_data)) {
                        ...
                        $tmp_skus=$product_sku->add($product_id, $sku_not_exists_data);

                        $tmp_sku = $tmp_skus;
                        if(!empty($original_product['is_wholesale']))  {
                            if (count($wholesaleProductList) > 0)   {
                                foreach ($wholesaleProductList as $productInfo) {
                                    $product_sku->add($productInfo['product_id'], $sku_not_exists_data,$tmp_sku);
                                }
                            }
                        }
					}
                ...
                if(!empty($original_product['is_wholesale'])){
                    if (count($wholesaleProductList) > 0){
                        foreach ($wholesaleProductList as $productInfo){
                            ...
                            unset($data['is_wholesale']);
                            unset($data['is_fx']);
                            $product->edit(array('wholesale_product_id' => $product_id), $data);
                            $where = array('product_id' => $productInfo['product_id'], 'store_id' => $productInfo['store_id']);
                            D('Product_to_property')->where($where)->delete();
                            D('Product_to_property_value')->where($where)->delete();
                            D('Product_sku')->where(array('product_id'=>$productInfo['product_id']))->delete();
			...
			// 关联商品
			$relation_product_id = $_POST['relation_product_id'];
			D('Product_relation')->where(array('product_id' => $product_id))->delete();
			if ($relation_product_id && is_array($relation_product_id)) {
				$i = 0;
				$relation_product_id = array_unique($relation_product_id);
				foreach ($relation_product_id as $tmp_product_id) {
					...
					// 禁止关联其它店铺的产品
					$relation_product = D('Product')->where(array('product_id' => $tmp_product_id, 'uid' => $this->user_session['uid']))->field('product_id')->find();
					if ($relation_product) {
						$product_relation_data = array();
						$product_relation_data['product_id'] = $product_id;
						$product_relation_data['relation_id'] = $tmp_product_id;
    ...
    /*private function setFxProductsPrice($product_id,$fluctuationPrice){
        //商品详细
        $productInfo = D('Product')->where(array('product_id'=>$product_id))->find();
        $productSkuInfo = D('Product_sku')->where(array('product_id'=>$product_id))->select();

        //更新商品价
        D('Product')->where(array('product_id'=>$productInfo['product_id']))
            ->data(array( 'drp_level_1_cost_price'=>$productInfo['drp_level_1_cost_price'] + $fluctuationPrice,
                          'drp_level_2_cost_price'=>$productInfo['drp_level_2_cost_price'] + $fluctuationPrice,
                          'drp_level_3_cost_price'=>$productInfo['drp_level_3_cost_price'] + $fluctuationPrice,
                          'drp_level_1_price'=>$productInfo['drp_level_1_cost_price'] + $fluctuationPrice,
                          'drp_level_2_price'=>$productInfo['drp_level_2_cost_price'] + $fluctuationPrice,
                          'drp_level_3_price'=>$productInfo['drp_level_3_cost_price'] + $fluctuationPrice ))
            ->save();
        //更新库存
        foreach($productSkuInfo as $product){
            D('Product_sku')->where(array('product_id' => $product['product_id']))
                ->data(array( 'drp_level_1_cost_price' => $product['drp_level_1_cost_price'] + $fluctuationPrice,
                    'drp_level_2_cost_price' => $product['drp_level_2_cost_price'] + $fluctuationPrice,
                    'drp_level_3_cost_price' => $product['drp_level_3_cost_price'] + $fluctuationPrice,
                    'drp_level_1_price' => $product['drp_level_1_price'] + $fluctuationPrice,
                    'drp_level_2_price' => $product['drp_level_2_price'] + $fluctuationPrice,
                    'drp_level_3_price' => $product['drp_level_3_price'] + $fluctuationPrice,
                    'cost_price' => $product['cost_price'] + $fluctuationPrice,
                    'min_fx_price' => $product['min_fx_price'] + $fluctuationPrice,
                    'max_fx_price' => $product['max_fx_price'] + $fluctuationPrice ))
                ->save();
        }
    }*/
    ...
    public function goods_load() {
        ...
        if ($_POST['page'] == 'category_content') {
            $group_list = M('Product_group')->get_catall_list($this->store_session['store_id']);
...
//    }
    ...
    private function _create_content()
    {
        //平台是否开启送积分
        import('source.class.Margin');
        $this->assign('open_platform_point', Margin::check());
...
//        exit;
		...
		if (!empty($skus)) {
			foreach ($skus as $tmp_sku) {
				...
				$tmp['point'] = $tmp_sku['return_point'];
//				$tmp['after_subscribe_discount'] = $tmp_sku['after_subscribe_discount'];
        ...
        if (!empty($pids[0]['pid'])) {
        	$open_point_checked = false;
        	if ($product['open_return_point'] == 1) {
        		$open_point_checked = true;
        	}
        	$this->assign('open_point_checked', $open_point_checked);
            $pid = $pids[0]['pid'];
            ...
            if (!empty($pids[1]['pid']) && !empty($pids[2]['pid'])) {
                ...
                $html .= '        <th class="th-price"><input type="checkbox" class="js-open_point" ' . ($open_point_checked ? 'checked="checked"' : '') . ' /> 消费积分</th>';
                    ...
                    foreach ($vids1 as $key1 => $vid1) {
                        ...
                        foreach ($vids2 as $key2 => $vid2) {
                            ...
                            if(empty($product['wholesale_product_id'])){
                                ...
                                $html .= '        <td class="text-right"><input type="text" name="sku_point" class="js-sku_point input-mini" ' . ($open_point_checked ? '' : 'readonly="readonly"') . ' value="' . ($open_point_checked ? $sku['return_point'] : '') . '" maxlength="10"></td>';
                                ...
                                $html .= '        <td class="text-right"><input type="text" data-saleminprice="'.$sku['sale_min_price'].'" data-salemaxprice="'.$sku['sale_max_price'].'" name="sku_price" class="js-price input-mini" value="' . $sku['price'] . '" maxlength="10"></td>';
                                $html .= '        <td class="text-right"><input type="text" name="sku_point" class="js-sku_point input-mini" ' . ($open_point_checked ? '' : 'readonly="readonly"') . ' value="' . ($open_point_checked ? $sku['return_point'] : '') . '" maxlength="10"></td>';
               ...
               // $html .= '</tbody><tfoot><tr><td colspan="6"><div class="batch-opts">批量设置： <span class="js-batch-type"><a class="js-batch-price" href="javascript:;">价格</a>&nbsp;&nbsp;<a class="js-batch-stock" href="javascript:;">库存</a></span><span class="js-batch-form" style="display:none;"><input type="text" class="js-batch-txt input-mini" placeholder=""><a class="js-batch-save" href="javascript:;">保存</a> <a class="js-batch-cancel" href="javascript:;">取消</a><p class="help-desc"></p></span></div></td></tr></tfoot>';
                ...
                $html .= '        <th class="th-price text-right">价格（元）</th>';
                $html .= '        <th class="th-price"><input type="checkbox" class="js-open_point" ' . ($open_point_checked ? 'checked="checked"' : '') . ' /> 消费积分</th>';
//                $html .= '        <th class="th-after-subscribe-discount text-right">关注后折扣</th>';
                $html .= '        <th class="th-stock text-right">库存</th>';
                $html .= '        <th class="th-stock text-right">重量(克)</th>';
                $html .= '        <th class="th-code">商品编码</th>';
                if(empty($product['wholesale_product_id'])) {
		...
		// 关联商品
		$product_relation_list = M('Product_relation')->getRelationProduct($id);
		$this->assign('product_relation_list', $product_relation_list);
    }
    ...
    private function _comment_store_list() {
        $where['relation_id'] = $this->store_session['store_id'];
        $where['delete_flg'] = 0;
        $where['type'] = 'STORE';
        $comment_model = M('Comment');
        $count = $comment_model->getCount($where);
    ...
    public function del_comment() {
    	if (option('config.is_allow_comment_control') != 1) {
    		json_return(1000, '您不能删除评论');
    	}
    	$type = $_POST['type'];
    	if (!in_array($type, array('store', 'goods')) || empty($_POST['comment_id'])) {
    		json_return(1000, '缺少基本参数');
    	}
        ...
        if (!empty($comment_id)) {
			if ($type == 'store') {
				$comment->delete(array('id' => array('in', $comment_id), 'relation_id' => $this->store_session['store_id'], 'type' => 'STORE'));
			} else {
				$comment->delete(array('id' => array('in', $comment_id), 'store_id' => $this->store_session['store_id'], 'type' => 'PRODUCT'));
			}
            json_return(0, '删除成功');
        } else {
            json_return(1001, '删除失败');
    ...
    public function set_comment_status() {
        ...
        if (!in_array($type, array('store', 'goods')) || empty($comment_id) || empty($status)) {
        	json_return(1000, '缺少基本参数');
        }
...
//    		echo ' 		<td align="center">' . $v['after_subscribe_discount'] . '</td>';
//    		echo ' 		<td align="center">' . $v['after_subscribe_price'] . '</td>';
	...
	public function _subject_edit() {
		...
		if($_POST['is_ajax']) {
			...
			$used_product_id_arr = array();
			$subject_product_array = D('Subject_product') ->where("subject_id = '".$subject_id."'")->select();
			if(is_array($subject_product_array)) {
				foreach($subject_product_array as $k=>$v) {
					$used_product_id_arr[] = $v['product_id'];
				}
			}
			$allow_delete_arr = array_diff ( $used_product_id_arr ,  $product_id_arr );
			...
			if($subject_id) {
				...
				D('Subject_product')->where(array('product_id'=>array('in',$allow_delete_arr)))->delete();
				$ks = 0;
				foreach($piclist as $k=>$v) {
					$vs['piclist'] = implode("^",$v);
					$vs['product_id'] = $k;
					...
					$ks++;
				 $last_sql[] = D('Subject_product')->last_sql;//exit;
				}
				
				json_return(0, '修改专题成功！'.json_encode($product_id_arr));
			} else {
    ...
    function checkProduct(){
        ...
        if(!empty($product_id)){
            $checkResuct = D('Product')->where(array('wholesale_product_id'=>$product_id))->select();
            if (empty($checkResuct)){
                json_return(1001, '此商品没有被批发');
            } else{
                json_return(0, '此商品已批发');
            }
        }
    }
}
