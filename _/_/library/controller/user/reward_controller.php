...
class reward_controller extends base_controller {
	...
	private function _product_list() {
		...
		if (!empty($group_id)) {
			$product_to_group = D('Product_to_group')->where(array('group_id' => $group_id))->field('GROUP_CONCAT(product_id) as product_id_str')->find();
			if (empty($product_to_group['product_id_str'])) {
				$where['product_id'] = 0;
			} else {
				$where['_string'] = 'product_id in (' . $product_to_group['product_id_str'] . ')';
			}
		}
		if (!empty($title)) {
			if ($type == 'title') {
