...
class store_controller extends base_controller{
	...
	public function index() {
		...
		if($wx_img = $this->update_qcode($id)) {
			$store['qcode'] = $wx_img;
        ...
        //获取店铺的横幅广告
        $adver_banners = D("Adver")->where(array('store_id'=>$id,'cat_id'=>14,'status'=>1))->limit(3)->select();
		// 抽出店铺产品
		$where = array();
		$where['store_id'] = $id;
		$where['status'] = 1;
		...
		$this->assign('adver_banners',$adver_banners);
	...
	public function store_list(){
		...
		$where_sql = '`s`.`status` = 1 and  `s`.is_point_mall=0';
...
/* 		if (!empty($id)) {
	...
	function couponlist(){
		...
		if (empty($this->user_session['uid'])) {
			...
			echo json_encode(array('err_code'=>2,'err_msg'=>'/index.php?c=account&a=login&referer='.$referer));
			exit;
		}
	...
	function coupon_detail() {
		...
		$isajax = (int)$_GET['isajax'];
		if($isajax){
			$coupon_detail['starttime_formate'] = date('Y-m-d',$coupon_detail['start_time']);
			$coupon_detail['endtime_formate'] = date('Y-m-d',$coupon_detail['end_time']);
			echo json_encode(array('err_code'=>0,'err_msg'=>$coupon_detail));
			exit;
		}
	...
	/*@description：全站商铺ajax检测二维码过期	
	 */
	 public function check_qcode() {
			$store_id = $_POST['store_id'];
			
			if($wx_img = $this->update_qcode($store_id)) {
				exit(json_encode(array('msg'=>'二维码更新成功！','code'=>'0','wx_img'=>$wx_img)));
			}
	 }

	/*@description：全站商铺检测二维码过期	
	 */	
	private function update_qcode($store_id) {
			//$store_id = $_POST['store_id'];
			$time = time();
			...
			if($store_id>0) {
				...
				if($info['qcode_starttime'] >$time_7days_before) {
					...
					//下载二维码图片 @simon
					$local_file = M('Recognition')->download_img_from_weixin($qid,'./upload/store');
					...
					*/
					...
					if($qcode['error_code']=='0') {
						...
						D('Store')->data($datas)->where(array('store_id'=>$store_id))->save();
