	...
	public function check_status(){
		...
		if(!empty($now_qrcode)){
			if($now_qrcode['status'] 	== 1 && $now_qrcode['openid'] !='') {
				...
				$user 	= M('User')->get_user('openid',$now_qrcode['openid']);
				if(!$now_qrcode['phone'] && $user['weixin_bind'] == 1) {
					echo json_encode(array('errcode'=>110,'errmsg'=>'to_mobile_bind'));
					exit;
				}
				...
				if(!$_SESSION['user']) {
					...
					if($result['user']) {
						if(!$result['user']['openid']) {
							echo json_encode(array('errcode'=>1,'errmsg'=>'to_mobile_bind'));
							exit;
						}
					}
					if($result['user']['status']!=1){
						echo json_encode(array('errcode'=>1,'errmsg'=>'no_power_to_login'));
						exit;
					}
					session_start();
