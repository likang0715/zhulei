...
class goods_controller extends base_controller{
	...
	public function index() {
		...
		$wheres = array(
				'ss.supplier_id' => $this->product['store_id'],
				's.status' => array('>',0),
		);
		$count_user1 = M('Store_supplier')->seller_count($wheres);
			
		$sellerList = M('Store_supplier')->getNextSellers($this->product['store_id'], 'all');
		...
		$this->product['drp_seller_qty'] = $count_user;
		...
		if (Margin::pre_recharge()) {
			...
			//商品赠送的积分
			$return_point = Margin::productPoint($this->product['product_id']);
			//平台保证金服务费
        ...
        $this->assign('store_online_trade', $store_online_trade);
		...
		$this->assign('constraint_sold_out', $constraint_sold_out);
		$this->display();
	}
