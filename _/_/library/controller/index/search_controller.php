...
class search_controller extends base_controller{
	...
	function store() {
		...
		if (!empty($id)) {
			$where_sql[] = " (`s`.`sale_category_fid` = '" . $id . "' or `s`.`sale_category_id` = '" . $id . "')";
		}
		...
		if(!empty($p_c_a)){		
			$p_c_a_arr = explode(',', $p_c_a);

			if($p_c_a_arr[0]){
				$this->assign('province_code', $p_c_a_arr[0]);
			}

			if($p_c_a_arr[1]){
				$this->assign('city_code', $p_c_a_arr[1]);
			}

			if($p_c_a_arr[2]){
				$this->assign('area_code', $p_c_a_arr[2]);
			}
				
		}else{

			$WebUserInfo = show_distance();
			if($WebUserInfo['province_code']){
				$this->assign('province_code', $WebUserInfo['province_code']);
			}

			if($WebUserInfo['city_code']){
				$this->assign('city_code', $WebUserInfo['city_code']);
			}

			if($WebUserInfo['area_code']){
				$this->assign('area_code', $WebUserInfo['area_code']);
			}
		}


		$count = M('Store')->getStoreByRoundDistanceCount_new($where_sql,$distance,$is_distance_order,$p_c_a_arr);
		//echo $count;
		...
		if ($count > 0) {
			...
			$store_list = M('Store')->getStoreByRoundDistance_new($where_sql,$limit,$offset,$orders,$sort,$distance,$is_distance_order,$p_c_a_arr);
		...
		if (!empty($tag_id)) {
			$param_collect['tag_id'] = $tag_id;
			$param['tag_id'] = $tag_id;
			$param_search['tag_id'] = $tag_id;
			$param_distance['tag_id'] = $tag_id;
		}
 		...
 		if(!empty($p_c_a)){
 			...
 			$param_collect['p_c_a'] = $p_c_a;
			$param['p_c_a'] = $p_c_a;
			$param_search['p_c_a'] = $p_c_a;
			$param_distance['p_c_a'] = $p_c_a;
		...
		$param_new = $param;
		if($order == 'date'){
			$param_new['sort'] = $sort == 'asc' ? 'desc' : 'asc';
		}
		$param_new['order'] = 'date';
		...
		foreach ($store_tags as $key => $value) {
			if($value['tag_id'] == $tag_id){
				$store_tag_name = $value['name'];
				break;
			}
		}
	
		//优质店铺
		$excellent_list = M('Store')->supplieStore(0,6,true);
		//优质供应商
		...
		$this->assign('param_new', $param_new);
		...
		$this->assign('store_tags', $store_tags);
		$this->assign('store_tag_name', $store_tag_name);
		$this->assign('p_c_a', $p_c_a);
		$this->display();
	}
	...
	function goods() {
		...
		$where_sql[] = "`p`.`status` = 1 and `p`.`name` like '%" . $keyword . "%'";
