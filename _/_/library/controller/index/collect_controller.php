...
class collect_controller extends base_controller {
    ...
    function add() {
        ...
        // 查看是否已经收藏过
        $user_collect = D('User_collect')->where(array('user_id' => $this->user_session['uid'], 'type' => $type,'store_id'=>$store_id, 'dataid' => $dataid))->find();
        if (!empty($user_collect)) {
            echo json_encode(array('status' => false, 'msg' => '已经收藏过了'));
            exit;
        }

        M('User_collect')->add($this->user_session['uid'], $dataid, $type,$store_id);
    ...
    public function attention() {
        ...
        $attention_info = D('User_attention')->where(array('user_id' => $this->user_session['uid'],'store_id'=>$store_id, 'data_type' => $data_type, 'data_id' => $data_id))->find();
        if (!empty($attention_info)) {
            echo json_encode(array('status' => false, 'msg' => '已经关注过了'));
            exit;
        } else {
            M('User_attention')->add($this->user_session['uid'], $data_id, $data_type,$store_id);
    ...
    function attention_cancel() {
    	...
    	// 查看是否已经收藏过
    	$user_attention = D('User_attention')->where(array('user_id' => $this->user_session['uid'], 'store_id'=>$store_id, 'data_type' => $data_type, 'data_id' => $data_id))->find();
    	if (empty($user_attention)) {
    		echo json_encode(array('status' => false, 'msg' => '未找到您的关注'));
    		exit;
    	}
    
    	M('User_attention')->cancel($this->user_session['uid'], $data_id, $data_type, $store_id);
    ...
    function cancel() {
        ...
        // 查看是否已经收藏过
        $user_collect = D('User_collect')->where(array('user_id' => $this->user_session['uid'],'store_id'=>$store_id, 'type' => $type, 'dataid' => $dataid))->find();
        if (empty($user_collect)) {
            echo json_encode(array('status' => false, 'msg' => '未找到您的收藏'));
            exit;
        }

        M('User_collect')->cancel($this->user_session['uid'], $dataid, $type,$store_id);
