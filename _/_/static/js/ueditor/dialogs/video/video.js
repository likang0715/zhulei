...
(function(){
    ...
    function convert_url(s){
        if (!s || !UE.utils.isString(s)) {
            return;
        }
        var id,iframe_url;
        if (s.indexOf('v.qq.com') >= 0) {
            id = s.match(/vid\=([^\&]*)($|\&)/);
            if  (id) {
                iframe_url = 'http://v.qq.com/iframe/player.html?vid='+id[1]+'&tiny=0&auto=0';
            } else {
                id = s.match(/\/([0-9a-zA-Z]+).html/);
                if(id) {
                    iframe_url = 'http://v.qq.com/iframe/player.html?vid='+id[1]+'&tiny=0&auto=0';
                }
            }
        ...
        } else if (s.indexOf('v.youku.com') >= 0) {
            id = s.match(/id_(.*)\.html/);
            iframe_url = 'http://player.youku.com/embed/' + id[1];
        } else if (s.indexOf('//player.youku.com/embed/') >= 0) {
            iframe_url = s.match(/src=\"([^"]*)\"/)[1];
        } else if (s.indexOf('tudou.com') >= 0) {
            id = s.match(/\/(\w*)\.html/)[1];
    ...
    function createPreviewVideo(url){
		...
		if(conUrl){
			$G("preview").innerHTML = '<iframe class="edui-faked-video" src="' + conUrl + '" width="' + 542 + '" height="' + 280 + '" allowfullscreen="true" ></ifame>';
		}else {
            $G("preview").innerHTML = '请复制腾讯、优酷视频地址输入框。';
        }
    }
    ...
    function insertUpload(){
        var videoObjs=[],
            ...
            width = $G('upload_width').value || 300,
            height = $G('upload_height').value || 225,
