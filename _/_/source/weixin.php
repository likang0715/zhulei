...
// 	$user_name = $data['FromUserName'];
	...
	if ($data['MsgType'] == 'event') {
		...
		switch (strtoupper($data['Event'])) {
			...
			return scan($id, $data['FromUserName']);
				break;
			case 'CLICK':
					...
					switch ($menu['type']) {
						...
						case 1:
							...
							foreach ($images as $im) {
								...
								if(empty($im['url'])){
								   $im['url']=$config['wap_site_url'].'/imagetxt.php?id='.$im['pigcms_id'];
								   }   
...
function scan($id, $openid = ''){

         // 参数二维码
        if ($id > 1800000000 && $id < 1900000000) {
            $qid = $id - 1800000000;
   
           $relation = D('Reply_relation')->where(array('q_id'=>$qid))->limit(1)->order('rand()')->select();
