...
class Weixinapp{
	...
	public function pay(){
	$this->pay_config['pay_weixin_appid']='wx8917d124e0d8d9ff';
	$this->pay_config['pay_weixin_mchid']='1358077502';
	...
	///	$unifiedOrder->setParameter("openid",$this->openid);//商品描述
		...
		$unifiedOrder->setParameter("notify_url",option('config.wap_site_url').'/appwxnotice.php');//通知地址
		...
		$Common = new Common_util_pub($this->pay_config['pay_weixin_appid'],$this->pay_config['pay_weixin_mchid'],$this->pay_config['pay_weixin_key']);
		
		
        $results=array();   
        $results['appid']=$this->pay_config['pay_weixin_appid'];
        $results['partnerid']=$this->pay_config['pay_weixin_mchid'];
        $results['prepayid']=$prepay_result['prepay_id'];
		$results['package']='Sign=WXPay';
		$results['noncestr']=$Common->createNoncestr();
		$time_stamp = time();
		$results['timestamp']="$time_stamp";
		$results['sign']=$Common->getSign($results);
...
public function cashier_pay(){
		...
		$unifiedOrder = new UnifiedOrder_pub($this->pay_config['pay_weixin_appid'],$this->pay_config['pay_weixin_mchid'],$this->pay_config['pay_weixin_key']);
		$unifiedOrder->setParameter("openid",$this->openid);//商品描述
		$unifiedOrder->setParameter("body",$this->order_info['store_name']);//商品描述
		//自定义订单号，此处仅作举例
		$unifiedOrder->setParameter("out_trade_no",$this->order_info['pay_no']);//商户订单号
		$unifiedOrder->setParameter("total_fee",round($this->order_info['price']*100));//总金额
		$unifiedOrder->setParameter("notify_url",option('config.wap_site_url').'/cashiernotice.php');//通知地址
...
public function cashier_notice(){
		$array_data = json_decode(json_encode(simplexml_load_string($GLOBALS['HTTP_RAW_POST_DATA'], 'SimpleXMLElement', LIBXML_NOCDATA)), true);
		
		if($array_data){
			$nowOrder = D('Order_cashier')->field('`store_id`')->where(array('pay_no'=>$array_data['out_trade_no']))->find();
			$weixin_bind_info = D('Weixin_bind')->where(array('store_id'=>$nowOrder['store_id']))->find();
			if(!empty($weixin_bind_info['wxpay_mchid']) && !empty($weixin_bind_info['wxpay_key']) && !empty($weixin_bind_info['authorizer_appid'])){
