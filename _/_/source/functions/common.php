...
function R($group,$mode,$action){
	...
	if(file_exists($mode_file)){
		...
		if(!class_exists($mode_name)){
			error_tips($mode_name.' 类不存在。','none');
		}
		...
		if(DEBUG){
			error_tips($mode_file.' 控制器文件不存在。','none');
		}else{
			error_tips($group.'分组 控制器 '.$mode.' 文件不存在。','none');
		}
	}
}
/**
...
function display($tpl=''){
	...
	if(file_exists(TPL_PATH.$tpl_file)){
		...
		if(DEBUG){
			error_tips('模板文件 template/'.$tpl_file.' 文件不存在。','none');
		}else{
...
//重定向店铺url
function redirect_store($store_id, $url = '') {
	if (empty($url)) {
		$url = 'http://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
	}
	if (preg_match('/home\.php\?id\=\d/i', $url)) {
		$url = preg_replace('/home\.php\?id\=\d+/i', 'home.php?id=' . $store_id, $url);
	} else if (preg_match('/ucenter\.php\?id\=\d+/i', $url)) {
		$url = preg_replace('/ucenter\.php\?id\=\d+/i', 'ucenter.php?id=' . $store_id, $url);
	} else if (preg_match('/cart\.php\?id\=\d+/i', $url)) {
		$url = preg_replace('/cart\.php\?id\=\d+/i', 'cart.php?id=' . $store_id, $url);
	} else if (preg_match('/order\.php\?id\=\d+/i', $url)) {
		$url = preg_replace('/order\.php\?id\=\d+/i', 'order.php?id=' . $store_id, $url);
	} else if (preg_match('/store_id\=\d+/i', $url)) {
		$url = preg_replace('/store_id\=\d+/i', 'store_id=' . $store_id, $url);
	} else if (preg_match('/good.php/i', $url)) {
...
// 生成活动wap端url地址
function activityUrl($id, $type = 'tuan', $store_id = 0) {
	if (empty($id) || !in_array($type, array('tuan', 'presale', 'bargin', 'seckill', 'unitary', 'cutprice'))) {
		return '';
	}
	
	$url = option('config.site_url');
	if ($type == 'tuan') {
		$url .= '/webapp/groupbuy/#/details/' . $id;
	} else if ($type == 'presale') {
		$url .= '/wap/presale.php?id=' . $id;
	} else if ($type == 'bargin') {
		$url .= '/wap/bargain.php?action=detail&id=' . $id .'&store_id=' . $store_id;
	} else if ($type == 'seckill') {
		$url .= '/wap/seckill.php?seckill_id=' . $id;
	} else if ($type == 'unitary') {
		$url .= '/webapp/snatch/#/main/' . $id;
	} else if ($type == 'cutprice') {
		$url .= '/wap/cutprice.php?action=detail&store_id=' . $store_id . '&id=' . $id;
	}
