...
<form id="myform" method="post" action="{pigcms{:U('Product/category_edit')}" enctype="multipart/form-data">
  <table cellpadding="0" cellspacing="0" class="frame_form" width="100%">
    ...
    <if condition="$category['cat_pic']">
      <tr>
        <th width="80">wap分类现图</th>
        <td><img src="{pigcms{$category.cat_pic}" style="width:60px;height:60px;" class="view_msg"/></td>
      </tr>
    </if>
     <if condition="$category['cat_pc_pic']">
      <tr>
        <th width="80">pc分类现图</th>
        <td><img src="{pigcms{$category.cat_pc_pic}" style="width:60px;height:60px;" class="view_msg"/></td>
      </tr>
    </if>   
    <tr>
      <th width="80">wap分类图片</th>
      <td><input type="file" class="input fl" name="pic" style="width:175px;" placeholder="请上传图片" tips="不修改请不上传！上传新图片，老图片会被自动删除！二级分类建议上传"/></td>
    </tr>
    <tr>
      <th width="80">pc分类图片</th>
      <td><input type="file" class="input fl" name="pcpic"   style="width:175px;" placeholder="请上传图片" tips="不修改请不上传！上传新图片，老图片会被自动删除！二级分类建议上传"/></td>
    </tr>    
    <tr>
      <th width="80">分类排序</th>
    ...
    <tr>
      ...
      <if condition="$edit_type eq '1'">
        <td><foreach name="property_type" item="propertytype" key="k"> <div style="padding-bottom: 5px" 
            ...
            <select validate="required:true"  name="property[]" class="property" id="cate_menu_select">
