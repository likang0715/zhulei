	...
	<form id="myform" method="post" action="{pigcms{:U('Search_hot/amend')}" frame="true" refresh="true">
		...
		<table cellpadding="0" cellspacing="0" class="frame_form" width="100%">
			...
			<tr>
				<th width="80">类别</th>
				<td>
					<select name="cat_type" class="valid">
						<option value="1" <if condition="$search_hot['cat_type']  eq 1">selected</if>>茶·活动</option>
						<option value="2" <if condition="$search_hot['cat_type']  eq 2">selected</if>>茶·空间</option>
						<option value="3" <if condition="$search_hot['cat_type']  eq 3">selected</if>>茶·产品</option>
					</select>
				</td>
			</tr>
			<tr>
