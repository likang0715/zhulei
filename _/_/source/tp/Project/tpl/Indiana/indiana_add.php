...
<div class="mainbox">
    <div id="nav" class="mainnav_title">
        <ul>
            <a href="javascript:void(0);" class="on">一元夺宝活动列表</a>
        </ul>
    </div>
    <table class="search_table" width="100%">
        <tr>
            <td>
                <form action="{pigcms{:U('Indiana/indiana_add')}" method="get">
                    <input type="hidden" name="c" value="Indiana"/>
                    <input type="hidden" name="a" value="indiana_add"/>
                    <input type="hidden" name="cat_id" value="{pigcms{$_REQUEST['cat_id']}"/>
                    筛选: <input type="text" name="keyword" class="input-text" value="{pigcms{$_GET['keyword']}"/>
                    <select name="type">
                        <option value="activity_id" <if condition="$_GET['type'] eq 'tuan_id'">selected="selected"</if>>活动名称</option>
    ...
    <form name="myform" id="myform" action="{pigcms{:U('Indiana/rcmd_indiana_all')}" method="post">
        <div class="table-list">
            ...
            <table width="100%" cellspacing="0">
                ...
                <tbody>
                <if condition="!empty($activity_lists)">
                    <volist name="activity_lists" id="vo">
                    <tr>
                        <td>
                        ...
                        <td><a class="button" href="{pigcms{:U('Indiana/rcmd_indiana',array('tid'=>$vo['id'],'cat_id'=>$cat_id,'product_id'=>$vo['product_id']))}" style="cursor: pointer;text-decoration:none;">添加推荐</a></td>
                    </tr>
                    </volist>
