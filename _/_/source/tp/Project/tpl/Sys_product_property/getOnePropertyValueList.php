	...
	$(function() {
		$('.status-enable > .cb-enable').click(function(){
			if (!$(this).hasClass('selected') ) {
				...
				$.post("<?php echo U('Sys_product_property/propertyvalue_status'); ?>",{'status': 1, 'vid': pid}, function(data){
		...
		$('.status-disable > .cb-disable').click(function(){
			if (!$(this).hasClass('selected') && parseFloat($(this).data('status')) == 1) {
				...
				var pid = $(this).data('id');
				if (!$(this).hasClass('selected')) {
					$.post("<?php echo U('Sys_product_property/propertyvalue_status'); ?>", {'status': 0, 'vid': pid}, function (data) {
						window.location.href = url;
					})
				}
			}
                    ...
                    <tbody>
                        <if condition="is_array($propertyValues)">
                            <volist name="propertyValues" id="propertyValue">
                                <tr>
                                    <td><a url="<?php echo U('Sys_product_property/propertyValue_del', array('vid' => $propertyValue['vid'])); ?>"  class="delete_row"><img src="{pigcms{$static_path}images/icon_delete.png" width="18" title="删除" alt="删除" /></a> | <a href="javascript:void(0);" onclick="window.top.artiframe('{pigcms{:U('Sys_product_property/propertyValue_edit', array('vid' => $propertyValue['vid']))}','修改商品属性值 - {pigcms{$property.name}',480,<if condition="$property['cat_pic']">390<else/>310</if>,true,false,false,editbtn,'edit',true);"><img src="{pigcms{$static_path}images/icon_edit.png" width="18" title="修改" alt="修改" /></a></td>
