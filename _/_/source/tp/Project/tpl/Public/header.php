...
<html xmlns="http://www.w3.org/1999/xhtml" xmlns="http://www.w3.org/1999/html"><head>
		...
		<script type="text/javascript" src="{pigcms{$static_path}js/plugin/jquery-ui.js"></script>
		<script type="text/javascript" src="{pigcms{$static_path}js/plugin/jquery-ui-timepicker-addon.js"></script>
		...
		<script type="text/javascript" src="{pigcms{$static_path}js/date.js"></script>
			<if condition="$withdrawal_count gt 0">
				<script type="text/javascript">
					$(function(){
						$('#nav_4 > dd > #leftmenu_Order_withdraw', parent.document).html('提现记录 <label style="color:red">(' + {pigcms{$withdrawal_count} + ')</label>')
					})
				</script>
			</if>
			<if condition="$unprocessed gt 0">
				<script type="text/javascript">
					$(function(){
						if ($('#leftmenu_Credit_returnRecord', parent.document).length > 0) {
							...
							menu_html += ' <label style="color:red">({pigcms{$unprocessed})</label>';
		...
		<body width="100%" 
		<if condition="$bg_color">
		style="background:{pigcms{$bg_color};"</if>
> 