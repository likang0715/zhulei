...
<form id="myform" method="post" action="{pigcms{:U('Admin/group_menu')}" frame="true" refresh="true">
		...
		<tbody>
			<volist name="system_menu" id="vo">
				<tr data-tt-id="{pigcms{$vo['id']}" <if condition="$vo.fid neq 0">data-tt-parent-id="{pigcms{$vo['fid']}"</if> >
					<td>
						<input type="checkbox" id="ipt_{pigcms{$vo.id}" class="js-check-all" name="menu[]" value="{pigcms{$vo.id}" <if condition="in_array($vo['id'], $all_filter)">checked=checked</if>>
				...
				<volist name="vo['menu_list']" id="voo">
					<tr data-tt-id="{pigcms{$voo['id']}" data-tt-parent-id="{pigcms{$voo['fid']}">
						<td>
							├─ &nbsp; <input type="checkbox" id="ipt_{pigcms{$voo.id}" class="fid_{pigcms{$voo['fid']}" name="menu[]" value="{pigcms{$voo['id']}" <if condition="in_array($voo['id'], $all_filter)">checked=checked</if>>
