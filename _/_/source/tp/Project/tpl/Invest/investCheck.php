...
function investLook(uid){
  var url='{pigcms{:U('Invest/investLook')}&uid='+uid;
        layer.open({
            type: 2,
            title: '投资人审核',
            fix: false,
	...
	$(".table-list input").blur(function(){
		...
		<div class="mainbox">
			<div id="nav" class="mainnav_title">
				<ul>
					<a href="{pigcms{:U('Invest/investCheck')}" class="on">投资人列表</a>
				</ul>
			</div>
							...
							<if condition="is_array($user_list)">
								<volist name="user_list" id="vo">
									<tr class="set">
										...
										<td>
										<input  style="border-radius: 2px;width: 70px;padding: 3px;" type="text" name="beilv" value="<?php echo $vo['beilv']; ?>"  kid="<?php echo $vo['uid'];  ?>"  />
										...
										<td class="textcenter">
											<a href="javascript:void(0);" onclick="investLook(<?php echo $vo['uid']; ?>)">审核</a>&nbsp;&nbsp;
											<a href="{pigcms{:U('Invest/investDel',array('uid'=>$vo['uid']))}" >删除</a>
										</td>
									</tr>
								</volist>
