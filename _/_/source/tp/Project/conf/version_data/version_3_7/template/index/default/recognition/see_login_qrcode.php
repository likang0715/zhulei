	...
	<body>
		...
		<script>
			...
			function ajax_weixin_login(){
				...
				$.get("<?php dourl('index:wxlogin:ajax_weixin_bind_login');?>",{qrcode_id:<?php echo $id;?>},function(result){
					if(result == 'reg_user'){
						...
						$('#login_status').html('您的账号已被禁止登陆！').css('color','red').show();
						window.top.location.href = redirect_url;
					} else if(result == 'to_mobile_bind') {
						$('#login_status').html('请前往微信客户端绑定用户信息！').css('color','red').show();
						ajax_weixin_login();
					}else if(result!=='true'){
						ajax_weixin_login();
					}else{
		...
		<?php if($config['web_login_show'] == 0){ ?>
