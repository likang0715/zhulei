...
<html>
	...
	<body>
		...
		<p style="<?php if(empty($_GET['mt'])){ ?>margin-top:50px;<?php } ?>margin-bottom:0px;text-align:center;">请使用微信扫描二维码绑定注册</p>
		...
		<script>
			...
			function ajax_weixin_login(){
				...
				$.get("<?php dourl('index:wxlogin:ajax_weixin_bind_register');?>",{qrcode_id:<?php echo $id;?>},function(result){
					if(result == 'reg_user'){
						$('#login_status').html('已扫描！请在微信公众号中点击授权登录。').css('color','green').show();
						ajax_weixin_login();
					}else if(result == 'no_user'){
						$('#login_status').html('请扫描二维码,注册用户！').css('color','red').show();
						window.location.href = redirect_url;
					} else if(result == 'no_power_to_login'){
						$('#login_status').html('用户被锁定！').css('color','red').show();
						ajax_weixin_login();
					}else if(result!=='true'){
						ajax_weixin_login();
					}else{
						$('#login_status').html('绑定注册成功！正在跳转。').css('color','green').show();
