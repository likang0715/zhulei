...
//        }
        ...
        $where = array('property_type_id' => $property_type_id,'status'=>1);
    ...
    public function property_edit() {
        ...
        if (IS_POST) {
            ...
            if ($productProperty_info = $productProperty->where(array('name' => $data['name'], 'pid' => array('neq', $pid)))->find()) {
                if (D('Product_property_type')->where(array('type_id' => $type_id))->find()) {
    ...
    public function propertyvalue_status() {

        $systemPropertyValue = M('systemPropertyValue');
        $vid = $this->_post('vid', 'trim,intval');
        $status = $this->_post('status', 'trim,intval');
        $systemPropertyValue->where(array('vid' => $vid))->save(array('status' => $status));
    }
...
*/
			$detail = $productPropertyValue->where(array( 'vid' => array('eq', $vid)))->find();
			if ($productPropertyValue->where(array('value' => $data['value'],'pid'=>$detail['pid'], 'vid' => array('neq', $vid)))->find()) {
                $this->frame_submit_tips(0, '该属性下此属性值已经存在！');
            }
