...
class ConfigAction extends BaseAction {
	...
	//平台公众号 appid 和 appsecret 检测
	public function check_wechat() {
		
		if(IS_POST) {
			$wechat_appid = $this->_post('wechat_appid');
			$wechat_appsecret = $this->_post('wechat_appsecret');
			$database_config = D('Config');
			$condition_config['gid'] = 8;
			$condition_config['status'] = '1';
			$tmp_config_list = $database_config->where($condition_config)->order('`id` asc')->select();
			$return = 0;
			foreach($tmp_config_list as $k=>$v) {
				if($v['name'] == 'wechat_appid') {
					if($v['value'] != $wechat_appid) {
						$return = '1';
					}
				}
				
				if($v['name'] == 'wechat_appsecret') {
					if($v['value'] != $wechat_appsecret) {
						$return = '2';
					}	
				}
			...
			echo 
				json_encode(array('status'=>$return));
			exit;
	...
	public function sendmsg() {
		$sysuser_id = $this->system_session['id'];
		$mobile = $this->_post("mobile");
		$sms_topmain = $this->_post("topmain");
		$sms_key = $this->_post("skey");
		$sms_price = $this->_post("sprice");
		$sms_sign = $this->_post("ssign");
		$content = "";
		if(!$sysuser_id) {
			exit(json_encode(array('error' => '1','message' =>'非法操作')));
		}
		if (!preg_match ( "/^1[3|5|7|8|9]{1}[0-9]{9}$/i", $mobile )) {
			echo json_encode ( array ('error' => '2','msg' => '该手机号不正确！'));
			exit();
		}
		if(!$sms_key || !$sms_price || !sms_sign || !$sms_topmain) {
			echo json_encode ( array ('error' => '3','msg' => '短信配置填写不完整！'));
			exit();			
		}
		...
		$params['sms'] = array('mobile'=>$mobile,'token'=>'','content'=>'管理员测试短信发送，发送时间：'.$date.'。','sendType'=>1);
		$return = MessageFactory::method($params, array('SmsMessage'));
		//$return = D('SmsByCode')->sendbycontent($sms_topmain,$sms_key,$mobile,$content,$sms_sign,$sms_price);
		if($return['SmsMessage']){
			exit(json_encode(array('error' => '9999','message' =>'短信发送失败，请检查配置！'.$return['SmsMessage'])));
		}else{
			exit(json_encode(array('error' => '0','message' =>'测试短信发送成功，请留意手机！')));
		}
