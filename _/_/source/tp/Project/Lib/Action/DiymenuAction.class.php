...
class DiymenuAction extends BaseAction
{
	...
	{
		$class = M('Diymenu_class')->where(array('pid'=>0, 'store_id' =>0))->order('`sort` desc')->select();
		foreach ($class as $key => $vo) {
			$c =M('Diymenu_class')->where(array('pid' => $vo['id'], 'store_id' =>0))->order('`sort` desc')->select();
			$class[$key]['class'] = $c;
		}
	...
	public function class_add()
	{
		if (IS_POST) {
			...
			$count = M('Diymenu_class')->where(array('pid' => $data['pid'], 'store_id' => 0))->count();
...
// 			}
			...
			if ($data==false) {
				...
				$class=M('Diymenu_class')->where(array('pid'=>0, 'store_id' => 0))->order('sort desc')->select();//dump($class);
				$this->assign('class',$class);
	...
	public function  class_del()
	{
		...
		$count = M('Diymenu_class')->where(array('pid' => $id, 'store_id' => 0))->count();
		if (empty($count)) {
	...
	public function class_send()
	{
		if(IS_GET){
			...
			$class=M('Diymenu_class')->where(array('pid'=>0,'is_show'=>1, 'store_id' => 0))->limit(3)->order('sort asc')->select();//dump($class);
			$kcount=M('Diymenu_class')->where(array('pid'=>0,'is_show'=>1, 'store_id' => 0))->count();
			$k=1;
			
			foreach($class as $key=>$vo){
				//主菜单
				$data.='{"name":"'.$vo['title'].'",';
				$c=M('Diymenu_class')->where(array('pid'=>$vo['id'],'is_show'=>1, 'store_id' => 0))->limit(5)->order('sort asc')->select();
				$count=M('Diymenu_class')->where(array('pid'=>$vo['id'],'is_show'=>1, 'store_id' => 0))->count();
				//子菜单
